...
} = require('./macos-notarize-with-retry.cjs')
 ...
 *
 * Signed release builds notarize here (not via electron-builder's built-in hook)
 * so stapling can retry longer when Apple's CloudKit ticket API is slow.
 */
async function afterSign(context) {
  ...
  if (isMacNotarizeConfigured()) {
    await notarizeMacAppWithResilientStaple(appPath)
  }
}
