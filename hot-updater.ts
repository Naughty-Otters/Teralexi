...
const start = async () => {
  ...
  if (buildConfig.asar) {
    errorLog(
      `${chalk.red(
        'pack:resources requires build.json asar=false because it replaces unpacked dist/ files. Use electron-updater for asar builds, or set "asar": false in build.json.',
      )}\n`,
    )
    return
  }
