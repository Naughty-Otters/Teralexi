...
function getopenfdeDbPath() {
  const path15 = (0, import_path.join)(getopenfdeDbDir(), openfde_DB_FILENAME);
  ensureParentDirForFile(path15);
  return path15;
}
...
function ensureDir(path15) {
  (0, import_fs2.mkdirSync)(redirectLegacyChannelDataPath(path15), { recursive: true });
}
...
function isLoadableSkillFolder(skillsDir, entry) {
  ...
  const skillFolder = (0, import_path2.join)(skillsDir, entry);
  try {
    if (!(0, import_fs3.statSync)(skillFolder).isDirectory()) return false;
  } catch {
    return false;
  }
  return (0, import_fs3.existsSync)((0, import_path2.join)(skillFolder, SKILL_FILES.SKILL_MD));
}
function resolveBundledSkillsDirectory() {
  if (import_electron.app?.isPackaged) {
    return (0, import_path2.join)(import_electron.app.getAppPath(), "skills");
  }
  return (0, import_path2.join)(process.cwd(), "skills");
}
...
function resolveBundledToolSetDirectory() {
  if (import_electron.app?.isPackaged) {
    return (0, import_path2.join)(import_electron.app.getAppPath(), SKILL_FILES.TOOL_SET_DIR);
  }
  return (0, import_path2.join)(process.cwd(), SKILL_FILES.TOOL_SET_DIR);
}
...
function resolvePropertiesRaw(skillId, skillFolder, skillRaw) {
  const propertiesFile = (0, import_path2.join)(skillFolder, SKILL_FILES.PROPERTIES_MD);
  const propertiesFromFile = (0, import_fs3.existsSync)(propertiesFile) ? (0, import_fs3.readFileSync)(propertiesFile, "utf-8") : "";
...
var import_fs3, import_path2, import_electron, RESERVED_SKILL_DIR_NAMES, PROPERTY_KEY_ORDER;
...
function skillModuleCacheDir() {
  return (0, import_path3.join)(process.cwd(), SKILL_MODULE.CACHE_DIR);
}
...
function fingerprintFromMetafile(metafile) {
  ...
  for (const inputPath of Object.keys(metafile.inputs)) {
    if (!(0, import_fs4.existsSync)(inputPath)) continue;
    inputs[inputPath] = (0, import_fs4.statSync)(inputPath).mtimeMs;
  }
  return { inputs };
}
function isSkillModuleBundleStale(fingerprint) {
  for (const [inputPath, recordedMtime] of Object.entries(fingerprint.inputs)) {
    if (!(0, import_fs4.existsSync)(inputPath)) return true;
    if ((0, import_fs4.statSync)(inputPath).mtimeMs !== recordedMtime) return true;
  }
  return false;
}
function readSkillModuleBundleFingerprint(outJs) {
  const fingerprintPath = skillModuleFingerprintPath(outJs);
  if (!(0, import_fs4.existsSync)(fingerprintPath)) return null;
  try {
    return JSON.parse(
      (0, import_fs4.readFileSync)(fingerprintPath, "utf8")
    );
  } catch {
    return null;
  }
...
function shouldRebuildSkillModuleBundle(outJs) {
  if (!(0, import_fs4.existsSync)(outJs)) return true;
...
function esbuildPathAliases() {
  ...
  return {
    "@main": (0, import_path3.resolve)(root, "src/main"),
    "@config": (0, import_path3.resolve)(root, "config"),
    "@shared": (0, import_path3.resolve)(root, "src/shared"),
    "@toolSet": (0, import_path3.resolve)(root, "toolSet"),
    "@logging": (0, import_path3.resolve)(root, "src/logging"),
    "@openfde-ai": (0, import_path3.resolve)(root, "src/openfde-ai")
  };
}
var import_crypto, import_fs4, import_path3, SKILL_MODULE_FINGERPRINT_SUFFIX;
...
async function requireModule(filepath) {
  if (filepath.endsWith(".ts")) {
    try {
      ...
      const outJs = (0, import_path4.join)(cacheDir, `${cacheKey}.js`);
...
async function loadSkillActionsFromDirectory(actionsDir) {
  if (!(0, import_fs5.existsSync)(actionsDir)) return [];
  const indexCandidates = [
    actionsDir,
    (0, import_path4.join)(actionsDir, "index"),
    (0, import_path4.join)(actionsDir, "index.ts"),
    (0, import_path4.join)(actionsDir, "index.js"),
    (0, import_path4.join)(actionsDir, "index.mjs"),
    (0, import_path4.join)(actionsDir, "index.cjs")
  ];
  ...
  let entries;
  try {
    entries = (0, import_fs5.readdirSync)(actionsDir, { withFileTypes: true });
  } catch {
    return [];
  }
  for (const entry of entries) {
    const entryPath = (0, import_path4.join)(actionsDir, entry.name);
...
async function loadSkillActions(skillFolder, declaredToolNames) {
  const actionsDir = (0, import_path4.join)(skillFolder, SKILL_FILES.ACTIONS_DIR);
...
async function loadToolSetToolsFromDirectory(toolSetDir) {
  if (!(0, import_fs5.existsSync)(toolSetDir)) return [];
  ...
  const moduleFiles = (0, import_fs5.readdirSync)(toolSetDir).filter((name) => {
    if (name.startsWith("index.")) return false;
    const ext = (0, import_path4.extname)(name);
    return ext === ".ts" || ext === ".js" || ext === ".mjs" || ext === ".cjs";
  }).map((name) => ({
    name,
    filePath: (0, import_path4.join)(toolSetDir, name),
    moduleTag: (0, import_path4.basename)(name, (0, import_path4.extname)(name))
  }));
  ...
  const candidates = [
    (0, import_path4.join)(toolSetDir, "index.ts"),
    (0, import_path4.join)(toolSetDir, "index.js"),
    (0, import_path4.join)(toolSetDir, "index.mjs"),
    (0, import_path4.join)(toolSetDir, "index")
  ];
...
var import_fs5, import_path4, ACTION_MODULE_EXT_RE, ACTION_INDEX_RE;
...
function toolLoopOutputRelBase(scopeOrStepKey) {
  ...
  if (!pathScope) {
    return (0, import_path5.join)("output", TOOL_LOOP_OUTPUT_SEGMENT);
  }
  ...
  return (0, import_path5.join)("output", TOOL_LOOP_OUTPUT_SEGMENT, ...instanceSegments);
}
function getOutputResultsRelPrefix(scope) {
  const active = scope?.trim() || getSandboxOutputScopeFromEnv();
  if (active) {
    return (0, import_path5.join)(toolLoopOutputRelBase(active), "results");
  }
  return (0, import_path5.join)("output", "results");
}
function getOutputScriptsRelPrefix(scope) {
  const active = scope?.trim() || getSandboxOutputScopeFromEnv();
  if (active) {
    return (0, import_path5.join)(toolLoopOutputRelBase(active), "scripts");
  }
  return (0, import_path5.join)("output", "scripts");
}
function remapLegacyPlanRelativePath(userPath) {
  ...
  if (seg.length >= 2 && seg[0] === "output" && seg[1] === "plans") {
    return (0, import_path5.join)("plans", ...seg.slice(2));
  }
  return stripped;
}
function remapLegacySharedOutputPath(userPath) {
  ...
  if (seg.length >= 2 && seg[0] === "output" && seg[1] === "results") {
    return (0, import_path5.join)(getOutputResultsRelPrefix(), ...seg.slice(2));
  }
  if (seg.length >= 2 && seg[0] === "output" && seg[1] === "scripts") {
    return (0, import_path5.join)(getOutputScriptsRelPrefix(), ...seg.slice(2));
  }
  return stripped;
}
var import_path5, OTTER_AGENT_SANDBOX_OUTPUT_SCOPE_ENV, SANDBOX_OUTPUT_SCOPE_GLOBAL_KEY, TOOL_LOOP_OUTPUT_SEGMENT;
...
var init_run_context = __esm({
  "src/main/agent/sandbox/run-context.ts"() {
    ...
    log2 = createLogger("sandbox.run-context");
...
function resolvePath(ctx, path15) {
  const parts = path15.trim().split(".");
  let cur = ctx;
...
var log3, AppCache, appCache;
var init_app_cache = __esm({
  "src/main/cache/app-cache.ts"() {
    init_logger();
    log3 = createLogger("agent.cache");
    AppCache = class {
      constructor() {
      ...
      setAgents(userId, agents) {
        ...
        log3.debug("Cache set: agents", { userId, count: agents.length });
      }
      invalidateAgents(userId) {
        if (this.agentsMap.delete(userId)) {
          log3.debug("Cache invalidated: agents", { userId });
        }
      }
      invalidateAllAgents() {
        this.agentsMap.clear();
        log3.debug("Cache invalidated: all agents");
      }
      // ── credentials ─────────────────────────────────────────────────────────
      getCredentials() {
      ...
      setCredentials(credentials) {
        ...
        log3.debug("Cache set: credentials");
      }
      invalidateCredentials() {
        if (this.credentialsEntry) {
          this.credentialsEntry = null;
          log3.debug("Cache invalidated: credentials");
        }
      }
      // ── MCP tools ───────────────────────────────────────────────────────────
      ...
      setPersona(userId, memoryAgentId, block) {
        ...
        log3.debug("Cache set: persona", { userId, memoryAgentId });
      }
      invalidatePersona(userId, memoryAgentId) {
        const key = `${userId}:${memoryAgentId ?? ""}`;
        if (this.personaMap.delete(key)) {
          log3.debug("Cache invalidated: persona", { userId, memoryAgentId });
        }
      }
      invalidateAllPersona(userId) {
        ...
        log3.debug("Cache invalidated: all persona", { userId });
      }
      // ── diagnostics ─────────────────────────────────────────────────────────
      stats() {
        return {
...
var import_fs6, import_path6, ConversationStore;
  ...
  "src/main/services/conversation-store/store.ts"() {
    ...
    ConversationStore = class {
      ...
      saveDataToFile(userId, relativePath, data) {
        ...
        const workspaceRoot = (0, import_path6.resolve)(workspacePath);
        const targetPath = (0, import_path6.resolve)(workspaceRoot, cleanRelativePath);
        const workspacePrefix = workspaceRoot.endsWith(import_path6.sep) ? workspaceRoot : `${workspaceRoot}${import_path6.sep}`;
        ...
        (0, import_fs6.mkdirSync)((0, import_path6.dirname)(targetPath), { recursive: true });
        (0, import_fs6.writeFileSync)(targetPath, content, "utf-8");
        return targetPath;
      }
      // ── Token usage ──────────────────────────────────────────────────────────
...
function getConversationStore() {
  if (!_store) {
    _store = instrumentInstanceMethods2(new ConversationStore(), log4);
  }
  return _store;
}
var log4, _store;
var init_conversation_store = __esm({
  "src/main/services/conversation-store/index.ts"() {
    ...
    init_types();
    init_store();
    log4 = createLogger("services.conversation-store");
    _store = null;
  }
});
...
var init_plan_mode_state_machine = __esm({
  "src/main/agent/coding/plan-mode-state-machine.ts"() {
    ...
    log5 = createLogger("agent.plan-mode.state");
    PlanModeStateMachine = class _PlanModeStateMachine {
      ...
      consumeEnterReminder(meta = defaultMeta("injection:consumeEnterReminder")) {
        ...
        if (consumed) {
          log5.debug("Consumed enter plan reminder", {
      ...
      consumeExecuteReminder(meta = defaultMeta("injection:consumeExecuteReminder")) {
        ...
        if (consumed) {
          log5.debug("Consumed execute plan reminder", {
            conversationId: this.conversationId,
            trigger: meta.trigger
          });
        }
      ...
      logUpdate(from, fromStatus, toStatus, meta) {
        ...
        if (statusChanged) {
          log5.info("Explore mode status transition", payload);
        } else {
          log5.info("Explore mode state updated", payload);
        }
      }
    };
  }
...
function pruneStalePlanMarkdownFiles(plansDirAbs, canonicalPlanAbs) {
  if (!(0, import_node_fs2.existsSync)(plansDirAbs)) return;
  const canonical = (0, import_node_path2.resolve)(canonicalPlanAbs);
  let entries;
  try {
    entries = (0, import_node_fs2.readdirSync)(plansDirAbs);
  } catch {
    return;
  }
  for (const name of entries) {
    ...
    try {
      (0, import_node_fs2.unlinkSync)(abs);
    } catch {
    }
  }
}
...
function ensurePlanModePlansDir(plansDirAbs) {
  if (!(0, import_node_fs2.existsSync)(plansDirAbs)) {
    (0, import_node_fs2.mkdirSync)(plansDirAbs, { recursive: true });
  }
}
...
function migrateLegacyPlanArtifacts(storage) {
  ...
  if (!(0, import_node_fs2.existsSync)(legacyDir)) return;
  ...
  if (!(0, import_node_fs2.existsSync)(storage.planFile.absolutePath) && (0, import_node_fs2.existsSync)(legacyPlan)) {
    try {
      (0, import_node_fs2.renameSync)(legacyPlan, storage.planFile.absolutePath);
    } catch {
      (0, import_node_fs2.copyFileSync)(legacyPlan, storage.planFile.absolutePath);
    }
  }
  if (!(0, import_node_fs2.existsSync)(storage.todosFile.absolutePath) && (0, import_node_fs2.existsSync)(legacyTodos)) {
    try {
      (0, import_node_fs2.renameSync)(legacyTodos, storage.todosFile.absolutePath);
    } catch {
      (0, import_node_fs2.copyFileSync)(legacyTodos, storage.todosFile.absolutePath);
    }
  }
}
function readLegacySandboxTodoList(sandboxRoot) {
  const legacy = (0, import_node_path2.join)(sandboxRoot, "todos.json");
  if (!(0, import_node_fs2.existsSync)(legacy)) return null;
  try {
    return parseTodoList(JSON.parse((0, import_node_fs2.readFileSync)(legacy, "utf8")));
  } catch {
    return null;
  }
}
function readPlanModeTodoList(conversationId, options) {
  ...
  if ((0, import_node_fs2.existsSync)(file)) {
    try {
      return parseTodoList(JSON.parse((0, import_node_fs2.readFileSync)(file, "utf8")));
    } catch {
...
function syncPlanMarkdownFromTodoList(storage, list) {
  ...
  (0, import_node_fs2.writeFileSync)(storage.planFile.absolutePath, markdown2, "utf8");
}
...
function bootstrapPlanModeStorage(conversationId, title, options) {
  ...
  ensurePlanModePlansDir(storage.plansDirAbs);
  pruneStalePlanMarkdownFiles(storage.plansDirAbs, storage.planFile.absolutePath);
  if (!(0, import_node_fs2.existsSync)(storage.planFile.absolutePath)) {
...
function seedTodosFromPlanMarkdown(conversationId, planPath, options) {
  let content = "";
  try {
    content = (0, import_node_fs2.readFileSync)(planPath, "utf8");
  } catch {
    return { seeded: 0 };
  }
...
var import_node_fs2, import_node_path2, import_node_crypto2, PLAN_MODE_PLANS_DIR;
...
function resolveFileToolPermissionKey(toolName) {
  return FILE_TOOL_PERMISSION_KEYS[toolName];
}
function extractFileToolPaths(toolName, input) {
  const paths = [];
  const add = (value) => {
...
var log6, READ_FILE_PRUNE_PREVIEW_CHARS;
var init_context_overflow_guard = __esm({
  "src/main/agent/expr/context-overflow-guard.ts"() {
    init_logger();
    init_thread_tagger();
    log6 = createLogger("agent.expr.context-overflow-guard");
...
function requireActiveSandbox() {
  ...
  if (!root) {
    return {
      ok: false,
      message: "No active agent sandbox. File tools are only available during an agent run with a sandbox."
    };
  }
  return { ok: true, root };
}
...
function isPathInFilesystemContext(sandboxRoot, workspacePath, absPath) {
...
function resolvePathAllowingOutside(sandboxRoot, userPath, workspacePath) {
  const trimmed = userPath.trim();
  if (!trimmed) {
    throw new Error("Empty path");
  }
  try {
    ...
    if (import_path7.default.isAbsolute(trimmed)) {
      return import_path7.default.normalize(import_path7.default.resolve(trimmed));
    }
    throw err;
  }
}
function assertMoveAllowed(sandboxRoot, workspacePath, sourceAbs, destAbs) {
...
function getWorkspacePathFromEnv() {
...
function sandboxPathError(error) {
...
function normalizeToolInputForDedupeKey(toolName, input, ctx) {
  ...
  switch (toolName) {
    case "read_file": {
      const path15 = typeof raw.path === "string" ? raw.path : "";
      return {
        path: normalizeToolPathForKey(path15, ctx),
...
function loadConversationWorkspace(conversationId) {
  ...
  const path15 = row?.workspacePath?.trim() || null;
  cache.set(id, path15);
  return path15;
}
...
function readFileCacheKey(path15, offset, limit) {
  return `${path15}\0${offset ?? 1}\0${limit ?? ""}`;
}
...
var import_node_fs3, MAX_MANIFEST_FILE_ENTRIES, MAX_MANIFEST_SEARCH_ENTRIES, MAX_MANIFEST_RESOURCE_ENTRIES, MANIFEST_TOOLS;
...
var import_node_fs4, import_zod2, exitPlanMode;
  ...
  "toolSet/planning/exit-plan-mode.ts"() {
    ...
    exitPlanMode = {
      ...
      async execute(input) {
        ...
        const planExists = storage != null && (0, import_node_fs4.existsSync)(storage.planFile.absolutePath);
        let planContent = "";
        if (planExists && storage) {
          try {
            planContent = (0, import_node_fs4.readFileSync)(storage.planFile.absolutePath, "utf8");
          } catch {
            planContent = "";
          }
        }
...
var import_node_fs5, FILESYSTEM_PREVIEW_CONVERSATION_ID;
...
function isSubAgentSandboxRoot(path15) {
...
// src/shared/tool-result/terminal-capture.ts
...
function isCaptureLike(filePath, captureAbs) {
  if (captureAbs && import_path19.default.normalize(filePath) === import_path19.default.normalize(captureAbs)) {
    return true;
  }
  const base = import_path19.default.basename(filePath).toLowerCase();
  return base.startsWith("capture-") && base.endsWith(".txt");
}
function sandboxRelativePosix(sandboxRoot, absPath) {
  return import_path19.default.relative(import_path19.default.resolve(sandboxRoot), import_path19.default.resolve(absPath)).split(import_path19.default.sep).join("/");
}
function classifySandboxArtifactPath(absPath, sandboxRoot) {
  const rel = sandboxRelativePosix(sandboxRoot, absPath);
  const segments = rel.split("/").filter(Boolean);
  const base = import_path19.default.basename(absPath).toLowerCase();
...
var import_path19, MAX_PRIMARY_PREVIEW_BYTES;
var init_run_script_artifacts = __esm({
  "toolSet/run-script-artifacts.ts"() {
    import_path19 = __toESM(require("path"));
    MAX_PRIMARY_PREVIEW_BYTES = 48 * 1024;
  }
});

// src/shared/file-change/types.ts
function isFileChangeToolName(name) {
  return FILE_CHANGE_TOOL_NAMES.includes(name);
}
var FILE_CHANGE_TOOL_NAMES;
...
__export(file_system_exports, {
  ...
  FILE_TOOL_PERMISSION_KEYS: () => FILE_TOOL_PERMISSION_KEYS,
  ...
  editFile: () => editFile,
  extractFileToolPaths: () => extractFileToolPaths,
  ...
  listFiles: () => listFiles,
  moveFile: () => moveFile,
  parseCommandArgv: () => parseCommandArgv,
  planPromoteArtifact: () => planPromoteArtifact,
  previewFileChange: () => previewFileChange,
  previewPromoteArtifact: () => previewPromoteArtifact,
  promoteArtifact: () => promoteArtifact,
  readFile: () => readFile,
  replace: () => replace,
  resolveFileToolPermissionKey: () => resolveFileToolPermissionKey,
...
// toolSet/file-system/apply-patch.ts
var import_fs11 = require("fs");
var import_path11 = __toESM(require("path"));
var import_zod7 = require("zod");
init_sandbox_paths();

// toolSet/file-system/constants.ts
var FILE_SYSTEM_TAG = ["file-system"];
var WORKSPACE_PATH_HINT = "Paths may be workspace-relative (e.g. src/foo.ts), absolute under the project root, or absolute paths returned by list_files/grep_files. Always call this tool \u2014 do not assume host paths are inaccessible.";
var SANDBOX_ARTIFACT_HINT = " Paths under output/, scripts/, refs/, or skills/ resolve to the agent sandbox, not the user repo.";
var DUAL_ROOT_PATH_HINT = `${WORKSPACE_PATH_HINT}${SANDBOX_ARTIFACT_HINT}`;
var MAX_GREP_MATCHES = 100;
var MAX_GLOB_PATHS = 100;
var MAX_LINE_CHARS = 2e3;
var MAX_READ_LINES = 2e3;
var MAX_READ_OUTPUT_BYTES = 50 * 1024;

// toolSet/file-system/file-io-utils.ts
var import_fs10 = require("fs");
var import_path10 = __toESM(require("path"));
var import_diff = require("diff");
async function movePath(sourcePath, destPath, overwrite) {
  if (!overwrite) {
    try {
      await import_fs10.promises.access(destPath);
  ...
  await import_fs10.promises.mkdir(import_path10.default.dirname(destPath), { recursive: true });
  try {
    await import_fs10.promises.rename(sourcePath, destPath);
  } catch (err) {
    const code = err.code;
    if (code === "EXDEV" || code === "EPERM") {
      await import_fs10.promises.copyFile(sourcePath, destPath);
      await import_fs10.promises.unlink(sourcePath);
      return;
    }
    throw err;
  }
...
async function withFileLock(filePath, fn) {
  const resolved = import_path10.default.resolve(filePath);
...
function createDiffMetadata(filePath, contentOld, contentNew) {
  const diff = trimDiff(
    (0, import_diff.createTwoFilesPatch)(
      filePath,
      filePath,
  ...
  let additions = 0;
  let deletions = 0;
  for (const change of (0, import_diff.diffLines)(contentOld, contentNew)) {
    if (change.added) additions += change.count ?? 0;
    if (change.removed) deletions += change.count ?? 0;
  }
  return { diff, additions, deletions };
}
function toToolAbsolutePath(absolutePath) {
  return import_path10.default.resolve(absolutePath);
}
function toToolDisplayPath(absolutePath, sandboxRoot, workspacePath) {
  if (workspacePath) {
    const rel2 = import_path10.default.relative(import_path10.default.resolve(workspacePath), import_path10.default.resolve(absolutePath));
    if (rel2 && !rel2.startsWith("..") && !import_path10.default.isAbsolute(rel2)) {
      return rel2.split(import_path10.default.sep).join("/");
    }
  }
  const rel = import_path10.default.relative(import_path10.default.resolve(sandboxRoot), import_path10.default.resolve(absolutePath));
  if (rel && !rel.startsWith("..") && !import_path10.default.isAbsolute(rel)) {
    return rel.split(import_path10.default.sep).join("/");
  }
  return import_path10.default.basename(absolutePath);
}
function matchPathToDisplayPath(matchPath, searchRoot, _sandboxRoot, _workspacePath) {
  const abs = import_path10.default.isAbsolute(matchPath) ? import_path10.default.normalize(matchPath) : import_path10.default.normalize(import_path10.default.join(searchRoot, matchPath));
  return toToolAbsolutePath(abs);
}
function computeDisplayPath(absolutePath, sandboxRoot, workspacePath) {
  return toToolDisplayPath(absolutePath, sandboxRoot, workspacePath);
}
function buildFileChangePreview(sandboxRoot, absolutePath, contentOld, contentNew, extras) {
  ...
  return {
    ...
    moveFrom: extras?.moveFrom ? computeDisplayPath(extras.moveFrom, sandboxRoot, workspacePath) : void 0,
    workspacePath: workspacePath ?? void 0
  };
}
async function readTextFileIfExists(filePath) {
  try {
    return await import_fs10.promises.readFile(filePath, "utf-8");
  } catch (err) {
    if (err.code === "ENOENT") {
      return null;
    }
...
// toolSet/file-system/patch-parse.ts
function parsePatchHeader(lines, startIdx) {
  ...
  if (line.startsWith("*** Update File:")) {
    ...
    if (nextIdx < lines.length && lines[nextIdx].startsWith("*** Move to:")) {
      movePath2 = lines[nextIdx].slice("*** Move to:".length).trim();
      nextIdx++;
    }
    return filePath ? { filePath, movePath: movePath2, nextIdx } : null;
  }
  return null;
}
...
// toolSet/file-system/apply-patch.ts
function resolveHunkPath(sandboxRoot, workspacePath, relativePath) {
  return resolvePathInContext(sandboxRoot, workspacePath, relativePath);
}
async function applyHunk(sandboxRoot, workspacePath, hunk) {
  switch (hunk.type) {
    case "add": {
      const targetPath = resolveHunkPath(sandboxRoot, workspacePath, hunk.path);
      await import_fs11.promises.mkdir(import_path11.default.dirname(targetPath), { recursive: true });
      await import_fs11.promises.writeFile(targetPath, hunk.contents, "utf-8");
      return { path: targetPath, action: "A", newContent: hunk.contents };
    }
    case "delete": {
      const targetPath = resolveHunkPath(sandboxRoot, workspacePath, hunk.path);
      const previousContent = await import_fs11.promises.readFile(targetPath, "utf-8");
    ...
    case "update": {
      ...
      const previousContent = await import_fs11.promises.readFile(sourcePath, "utf-8");
      ...
      if (hunk.move_path) {
        ...
        await import_fs11.promises.mkdir(import_path11.default.dirname(destPath), { recursive: true });
        await import_fs11.promises.writeFile(destPath, newContent, "utf-8");
        await import_fs11.promises.unlink(sourcePath);
        return {
          path: destPath,
          action: "M",
          previousContent,
      ...
      await import_fs11.promises.writeFile(sourcePath, newContent, "utf-8");
...
async function rollbackChanges(changes) {
  for (const change of [...changes].reverse()) {
    try {
      if (change.action === "A") {
        await import_fs11.promises.unlink(change.path).catch(() => void 0);
      } else if (change.action === "D" && change.previousContent !== void 0) {
        await import_fs11.promises.mkdir(import_path11.default.dirname(change.path), { recursive: true });
        await import_fs11.promises.writeFile(change.path, change.previousContent, "utf-8");
      } else if (change.action === "M") {
        if (change.moveFrom && change.previousContent !== void 0) {
          await import_fs11.promises.writeFile(change.moveFrom, change.previousContent, "utf-8");
          await import_fs11.promises.unlink(change.path).catch(() => void 0);
        } else if (change.previousContent !== void 0) {
          await import_fs11.promises.writeFile(change.path, change.previousContent, "utf-8");
        }
      }
    } catch {
...
var applyPatch = {
  ...
  description: `Apply a multi-file patch in the user project (*** Begin Patch ... *** End Patch). Rolls back on first failure. ${WORKSPACE_PATH_HINT}`,
  inputSchema: import_zod7.z.object({
    patch_text: import_zod7.z.string().min(1)
  }),
  needsApproval: true,
  async execute(input) {
    ...
    for (const hunk of hunks) {
      const paths = hunk.type === "update" && hunk.move_path ? [hunk.path, hunk.move_path] : [hunk.path];
    ...
    try {
      for (const hunk of hunks) {
        ...
        applied.push(change);
        summary.push(`${change.action} ${import_path11.default.relative(sandbox.root, change.path)}`);
        const oldContent = change.action === "A" ? "" : change.previousContent ?? await readTextFileIfExists(change.path) ?? "";
        const newContent = change.action === "D" ? "" : change.newContent ?? await readTextFileIfExists(change.path) ?? "";
        const action = change.action === "A" ? "create" : change.action === "D" ? "delete" : change.moveFrom ? "rename" : "modify";
...
// toolSet/file-system/copy-move.ts
...
var moveFile = {
  name: "move_file",
  tags: [...FILE_SYSTEM_TAG],
  description: `Move a file or directory. Destination must be in the user project or sandbox. ${WORKSPACE_PATH_HINT}${SANDBOX_ARTIFACT_HINT}`,
  inputSchema: import_zod8.z.object({
    source: import_zod8.z.string().min(1),
    destination: import_zod8.z.string().min(1),
    overwrite: import_zod8.z.boolean().optional()
  }),
  needsApproval: true,
  async execute(input) {
    const sandbox = requireActiveSandbox();
    ...
    try {
      const stats = await import_fs12.promises.stat(sourcePath).catch(() => null);
      const contentOld = stats?.isFile() ? await readTextFileIfExists(sourcePath) ?? "" : "";
...
var copyFile = {
  name: "copy_file",
  tags: [...FILE_SYSTEM_TAG],
  description: `Copy a file. Destination must be in the user project or sandbox. ${WORKSPACE_PATH_HINT}${SANDBOX_ARTIFACT_HINT}`,
  inputSchema: import_zod8.z.object({
    source: import_zod8.z.string().min(1),
    destination: import_zod8.z.string().min(1),
    overwrite: import_zod8.z.boolean().optional()
  ...
  async execute(input) {
    ...
    try {
      if (!overwrite) {
        try {
          await import_fs12.promises.access(destinationPath);
          return { error: `Destination already exists: ${destinationPath}` };
        } catch {
        }
      }
      const sourceStats = await import_fs12.promises.stat(sourcePath);
      if (!sourceStats.isFile()) {
        return { error: `Source is not a file: ${sourcePath}` };
      }
      const contentNew = await readTextFileIfExists(sourcePath) ?? "";
      await import_fs12.promises.mkdir(import_path12.default.dirname(destinationPath), { recursive: true });
      await import_fs12.promises.copyFile(sourcePath, destinationPath);
      const fileChange = buildFileChangePreview(
        sandbox.root,
        destinationPath,
        "",
...
// toolSet/file-system/delete-file.ts
...
var deleteFile = {
  name: "delete_file",
  tags: [...FILE_SYSTEM_TAG],
  description: `Delete a single file in the user project. ${WORKSPACE_PATH_HINT} Directories are not supported.`,
  ...
  async execute(input) {
    ...
    try {
      const stats = await import_fs13.promises.stat(targetPath);
      if (!stats.isFile()) {
        return { error: `Path is not a file: ${targetPath}` };
      }
      const contentOld = await import_fs13.promises.readFile(targetPath, "utf-8");
      await import_fs13.promises.unlink(targetPath);
      const fileChange = buildFileChangePreview(
        sandbox.root,
        targetPath,
        contentOld,
...
// toolSet/file-system/edit-file.ts
...
var editFile = {
  name: "edit_file",
  tags: [...FILE_SYSTEM_TAG],
  description: `Apply a search/replace edit to a text file in the user project. Prefer this over write_file for partial edits; set old_string empty to create or overwrite. ${WORKSPACE_PATH_HINT}`,
  inputSchema: import_zod10.z.object({
    path: import_zod10.z.string().min(1),
    old_string: import_zod10.z.string(),
    new_string: import_zod10.z.string(),
    replace_all: import_zod10.z.boolean().optional().default(false)
  }),
  needsApproval: true,
  async execute(input) {
    const sandbox = requireActiveSandbox();
    ...
    try {
      return await withFileLock(targetPath, async () => {
        if (oldString === "") {
          const contentOld2 = await readTextFileIfExists(targetPath) ?? "";
          await import_fs14.promises.mkdir(import_path13.default.dirname(targetPath), { recursive: true });
          await import_fs14.promises.writeFile(targetPath, newString, "utf-8");
          const fileChange2 = buildFileChangePreview(
            sandbox.root,
            targetPath,
            contentOld2,
            newString,
            { action: contentOld2 ? "modify" : "create", workspacePath }
          );
          return {
            written: true,
            path: targetPath,
            sandboxRoot: sandbox.root,
            workspacePath: workspacePath ?? void 0,
            diff: fileChange2.diff,
            additions: fileChange2.additions,
            deletions: fileChange2.deletions,
            files: [fileChange2]
          };
        }
        const stats = await import_fs14.promises.stat(targetPath).catch(() => null);
        if (!stats) {
        ...
        const contentOld = await import_fs14.promises.readFile(targetPath, "utf-8");
        ...
        await import_fs14.promises.writeFile(targetPath, contentNew, "utf-8");
        const fileChange = buildFileChangePreview(
          sandbox.root,
          targetPath,
          contentOld,
...
// toolSet/file-system/run-workspace-command.ts
...
// src/main/agent/background/background-task-manager.ts
var import_node_crypto3 = require("node:crypto");
var tasks = /* @__PURE__ */ new Map();
var shellHandles = /* @__PURE__ */ new Map();
function appendOutput(id, chunk) {
  const t = tasks.get(id);
  if (!t) return;
  t.output = (t.output + chunk).slice(-2e5);
}
function createBackgroundTask(args) {
  const task = {
...
function completeBackgroundTask(id, status, error) {
  const t = tasks.get(id);
  if (!t) return null;
  t.status = status;
...
function registerShellTask(task, process2) {
  shellHandles.set(task.id, { task, process: process2 });
  process2.stdout?.on("data", (buf) => appendOutput(task.id, String(buf)));
  process2.stderr?.on("data", (buf) => appendOutput(task.id, String(buf)));
}
...
function requireWorkspace() {
  ...
  if (!p) {
    return {
      ok: false,
      error: "No workspace folder is set for this conversation. Ask the user to select a workspace folder before running project commands."
    };
  }
...
function normalizeArgv(input) {
  if (Array.isArray(input)) {
    const argv = input.filter((x) => typeof x === "string" && x.length > 0);
    if (argv.length === 0) {
...
function resolveShellInvocation(commandStr) {
...
var runWorkspaceCommand = {
  name: "run_workspace_command",
  tags: [...FILE_SYSTEM_TAG, "workspace"],
  description: 'Run a command in the user project folder. Requires a selected workspace; do not use for sandbox paths (output/, scripts/). Default: argv via execFile, no shell (e.g. ["npm","test"] or a quoted string). Set `shell: true` to run a full shell command string with pipes, &&, redirects, and globs (e.g. "npm run build && npm test 2>&1 | tail -50") \u2014 runs through the OS shell in the workspace. Optional cwd is relative to the workspace root. Requires approval.',
  inputSchema: import_zod11.z.object({
    command: import_zod11.z.union([import_zod11.z.array(import_zod11.z.string().min(1)), import_zod11.z.string().min(1)]),
    shell: import_zod11.z.boolean().optional().default(false).describe(
      "Run `command` through the OS shell (supports pipes, &&, redirects, globs). `command` must be a string in shell mode."
    ),
    cwd: import_zod11.z.string().optional(),
    timeoutMs: import_zod11.z.number().int().min(100).max(6e5).optional().default(12e4),
    background: import_zod11.z.boolean().optional().default(false).describe(
  ...
  async execute(input) {
    ...
    const timeoutMs = typeof input["timeoutMs"] === "number" && input["timeoutMs"] > 0 ? input["timeoutMs"] : 12e4;
    const rawCwd = input["cwd"];
    let cwd = ws.path;
    if (typeof rawCwd === "string" && rawCwd.trim()) {
      const resolved = import_path14.default.resolve(ws.path, rawCwd.trim());
      const rel = import_path14.default.relative(ws.path, resolved);
      if (rel.startsWith("..") || import_path14.default.isAbsolute(rel)) {
    ...
    let displayCommand;
    if (useShell) {
      const commandStr = Array.isArray(input["command"]) ? input["command"].join(" ") : String(input["command"] ?? "").trim();
      if (!commandStr) {
    ...
    if (input["background"] === true) {
      const label = typeof displayCommand === "string" ? displayCommand.slice(0, 120) : displayCommand.join(" ").slice(0, 120);
      ...
      child.on("close", (code, signal) => {
        completeBackgroundTask(
          ...
          code !== 0 ? `exit ${code ?? signal ?? "unknown"}` : void 0
        );
      });
      child.on("error", (err) => {
...
// toolSet/file-system/glob-files.ts
...
// toolSet/file-system/ripgrep.ts
var import_child_process2 = require("child_process");
var import_path15 = __toESM(require("path"));
var import_util = require("util");
init_logger();
var execFileAsync = (0, import_util.promisify)(import_child_process2.execFile);
var log18 = createLogger("toolSet:ripgrep");
function formatExecError(err) {
  if (err instanceof Error) return err.message;
  return String(err);
}
function toAbsoluteRipgrepPath(cwd, filePath) {
  const trimmed = filePath.trim();
  if (!trimmed) return trimmed;
  return import_path15.default.isAbsolute(trimmed) ? import_path15.default.normalize(trimmed) : import_path15.default.resolve(cwd, trimmed);
}
async function runRipgrepJson(args, cwd) {
  try {
    ...
    log18.info("ripgrep content search completed", {
      cwd,
      args,
    ...
    log18.warn("ripgrep content search failed; use Node fallback", {
      cwd,
      args,
      error
    });
...
async function runRipgrepFiles(args, cwd) {
  try {
    const { stdout } = await execFileAsync("rg", ["--files", ...args], {
      cwd,
      maxBuffer: 10 * 1024 * 1024
    });
    const paths = stdout.split("\n").map((line) => line.trim()).filter(Boolean).map((rel) => toAbsoluteRipgrepPath(cwd, rel));
    log18.info("ripgrep file listing completed", {
      cwd,
      args,
    ...
    const error = formatExecError(err);
    log18.warn("ripgrep --files failed; use Node fallback", {
      cwd,
      args,
      error
    });
...
// toolSet/file-system/workspace-scan-filters.ts
var import_zod12 = require("zod");
var PACKAGE_SKIP_DIR_NAMES = /* @__PURE__ */ new Set([
  "node_modules",
  "bower_components",
  "vendor",
  ".yarn",
...
var includePackageFilesField = import_zod12.z.boolean().optional().default(false).describe(
  "When false (default), exclude node_modules, Python package dirs, and hidden files from results."
);
function parseIncludePackageFiles(input) {
  return Boolean(input["include_package_files"]);
}
function shouldSkipListingEntry(name, isDirectory, includePackageFiles) {
  if (includePackageFiles) return false;
  if (name.startsWith(".")) return true;
  if (isDirectory && PACKAGE_SKIP_DIR_NAMES.has(name)) return true;
  return false;
}
function shouldSkipRelativePath(relPath, includePackageFiles) {
  if (includePackageFiles) return false;
...
function ripgrepExcludeGlobArgs(includePackageFiles) {
...
function fastGlobIgnorePatterns(includePackageFiles) {
  if (includePackageFiles) return [];
  return [
...
// toolSet/file-system/glob-files.ts
var log19 = createLogger("toolSet:glob_files");
async function nodeGlobFallback(root, pattern, includePackageFiles) {
  ...
  const withMtime = await Promise.all(
    matches.map(async (rel) => {
      try {
        const stats = await import_fs15.promises.stat(import_path16.default.join(root, rel));
...
var globFiles = {
  name: "glob_files",
  tags: [...FILE_SYSTEM_TAG],
  description: `Find files by glob pattern in the user project (e.g. "**/*.ts"). ${WORKSPACE_PATH_HINT}${SANDBOX_ARTIFACT_HINT} By default skips node_modules, Python package dirs, and hidden paths. Returns up to 100 paths sorted by modification time (newest first).`,
  inputSchema: import_zod13.z.object({
    pattern: import_zod13.z.string().min(1),
    path: import_zod13.z.string().min(1).optional(),
  ...
  async execute(input) {
    ...
    try {
      ...
      if (!rgResult.available) {
        log19.warn("glob_files switching to Node fallback", {
          searchPath: resolvedPath,
          pattern,
        ...
        log19.info("glob_files Node fallback completed", {
          searchPath: resolvedPath,
          pattern,
          pathCount: paths.length
        });
        ...
        const withMtime = await Promise.all(
          paths.map(async (filePath) => {
            const abs = import_path16.default.isAbsolute(filePath) ? filePath : import_path16.default.join(resolvedPath, filePath);
            try {
              const stats = await import_fs15.promises.stat(abs);
      ...
      const limited = paths.slice(0, MAX_GLOB_PATHS).map(
        (absPath) => matchPathToDisplayPath(absPath, resolvedPath, sandbox.root, workspacePath)
      );
      const result = {
      ...
      if (usedFallback) {
        result.note = [result.note, "Used Node fallback (ripgrep unavailable)."].filter(Boolean).join(" ");
      }
      log19.info("glob_files completed", {
        searchPath: resolvedPath,
        pattern,
        engine,
...
// toolSet/file-system/grep-files.ts
...
// toolSet/file-system/format-tool-output.ts
function truncateLine(line, maxChars = MAX_LINE_CHARS) {
  if (line.length <= maxChars) return line;
  return `${line.slice(0, maxChars)}\u2026 [truncated ${line.length - maxChars} chars]`;
}
function formatNumberedLines(lines, startLineNumber) {
  const parts = [];
  let byteCount = 0;
  let truncated = false;
  let linesShown = 0;
...
// toolSet/file-system/grep-files.ts
var log20 = createLogger("toolSet:grep_files");
async function nodeGrepSingleFile(filePath, displayRoot, pattern, include) {
  let regex;
  try {
  ...
  const rel = import_path17.default.relative(displayRoot, filePath) || import_path17.default.basename(filePath);
  if (include) {
    const includeGlob = include.replace(/\*/g, ".*");
    const includeRegex = new RegExp(`^${includeGlob}$`);
    if (!includeRegex.test(rel) && !includeRegex.test(import_path17.default.basename(filePath))) {
      return [];
    }
  }
  const matches = [];
  try {
    const stats = await import_fs16.promises.stat(filePath);
    const content = await import_fs16.promises.readFile(filePath, "utf-8");
    const lines = content.split("\n");
    for (let i = 0; i < lines.length; i++) {
...
async function nodeGrepFallback(root, pattern, include, includePackageFiles) {
  let regex;
  try {
    regex = new RegExp(pattern);
  } catch (err) {
  ...
  async function scan(dir) {
    ...
    for (const entry of entries) {
      if (matches.length >= MAX_GREP_MATCHES) return;
      const entryPath = import_path17.default.join(dir, entry.name);
      if (entry.isDirectory()) {
        if (shouldSkipListingEntry(
          entry.name,
          true,
          includePackageFiles
        )) {
      ...
      if (!entry.isFile()) continue;
      if (shouldSkipListingEntry(entry.name, false, includePackageFiles)) {
        continue;
      }
      const rel = import_path17.default.relative(root, entryPath);
      if (!includeRegex.test(rel) && !includeRegex.test(entry.name)) continue;
      try {
        const stats = await import_fs16.promises.stat(entryPath);
        const content = await import_fs16.promises.readFile(entryPath, "utf-8");
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
          if (matches.length >= MAX_GREP_MATCHES) return;
          if (regex.test(lines[i])) {
  ...
  await scan(root);
  log20.info("Node grep fallback scan completed", {
    root,
    pattern,
...
var grepFiles = {
  name: "grep_files",
  tags: [...FILE_SYSTEM_TAG],
  description: `Search file contents with a regex in the user project. ${WORKSPACE_PATH_HINT}${SANDBOX_ARTIFACT_HINT} By default skips node_modules, Python package dirs, and hidden paths. Returns up to 100 matches sorted by modification time (newest first).`,
  inputSchema: import_zod14.z.object({
    pattern: import_zod14.z.string().min(1),
    path: import_zod14.z.string().min(1).optional().describe("Directory or single file to search (default: workspace root)."),
    include: import_zod14.z.string().optional(),
    include_package_files: includePackageFilesField
  }),
  needsApproval: false,
  async execute(input) {
    ...
    try {
      const stat = await import_fs16.promises.stat(resolvedPath);
      const searchFile = stat.isFile();
      const searchRoot = searchFile ? import_path17.default.dirname(resolvedPath) : resolvedPath;
      ...
      let engine = "ripgrep";
      if (!rgResult.available) {
        log20.warn("grep_files switching to Node fallback", {
          searchPath: resolvedPath,
          pattern,
          include: include ?? null,
          ripgrepError: rgResult.error ?? "unknown"
        });
        const fallback = searchFile ? await nodeGrepSingleFile(resolvedPath, displayRoot, pattern, include) : await nodeGrepFallback(
          resolvedPath,
          pattern,
          include,
          includePackageFiles
        ...
        const withMtime = await Promise.all(
          matches.map(async (m) => {
            const abs = import_path17.default.isAbsolute(m.path) ? m.path : import_path17.default.join(searchRoot, m.path);
            try {
              const stats = await import_fs16.promises.stat(abs);
      ...
      log20.info("grep_files completed", {
        searchPath: resolvedPath,
        pattern,
        engine,
        matchCount: limited.length,
...
// toolSet/file-system/list-files.ts
...
var listFiles = {
  name: "list_files",
  tags: [...FILE_SYSTEM_TAG],
  description: `List files and directories in the user project at the given path. ${WORKSPACE_PATH_HINT} By default skips node_modules, Python package dirs, and hidden files; set include_package_files=true to include them.`,
  inputSchema: import_zod15.z.object({
    path: import_zod15.z.string().min(1),
    recursive: import_zod15.z.boolean().optional().default(false),
    maxDepth: import_zod15.z.number().min(1).optional().default(1),
    include_package_files: includePackageFilesField
  }),
  needsApproval: false,
  async execute(input) {
    ...
    async function listDir(dir, depth) {
      const entries = await import_fs17.promises.readdir(dir, { withFileTypes: true });
      const results = [];
      for (const entry of entries) {
        if (shouldSkipListingEntry(
          entry.name,
        ...
        const entryPath = import_path18.default.join(dir, entry.name);
        const item = {
          name: entry.name,
        ...
        if (entry.isFile()) {
          const stats = await import_fs17.promises.stat(entryPath);
...
var storageCheck = {
  name: "storage_check",
  tags: [...FILE_SYSTEM_TAG],
  description: `Check available disk space for a path in the user project or sandbox. ${WORKSPACE_PATH_HINT}`,
  inputSchema: import_zod15.z.object({
    path: import_zod15.z.string().min(1).optional()
  }),
  needsApproval: false,
  async execute(input) {
    const sandbox = requireActiveSandbox();
    ...
    if (typeof input["path"] === "string" && input["path"].trim() !== "") {
      try {
        targetPath = resolvePathInContext(sandbox.root, workspacePath, input["path"]);
      } catch (err) {
        return sandboxPathError(err);
      }
    }
    try {
      const { stdout } = await execFileAsync2("df", ["-k", targetPath], {
        shell: true,
...
var fileStatus = {
  name: "file_status",
  tags: [...FILE_SYSTEM_TAG],
  description: `Retrieve metadata for a file or directory in the user project. ${WORKSPACE_PATH_HINT}`,
  inputSchema: import_zod15.z.object({
    path: import_zod15.z.string().min(1)
  }),
  needsApproval: false,
  async execute(input) {
    ...
    try {
      const stats = await import_fs17.promises.stat(resolvedPath);
      return {
        path: toToolAbsolutePath(resolvedPath),
        sandboxRoot: sandbox.root,
        workspacePath: workspacePath ?? void 0,
        ...
        permissions: (stats.mode & 511).toString(8)
      };
    } catch (error) {
      return { error: String(error) };
    }
...
// toolSet/file-system/promote-artifact.ts
var import_fs18 = require("fs");
var import_path20 = __toESM(require("path"));
var import_zod16 = require("zod");
init_sandbox_paths();
init_run_script_artifacts();
var promoteInputSchema = import_zod16.z.object({
  from: import_zod16.z.string().min(1),
  to: import_zod16.z.string().min(1),
  mode: import_zod16.z.enum(["copy", "move"]).optional().default("copy"),
  overwrite: import_zod16.z.boolean().optional().default(false),
  allowTemp: import_zod16.z.boolean().optional().default(false)
});
function requireWorkspacePath() {
  ...
  if (!workspacePath?.trim()) {
    return {
      ok: false,
      error: "promote_artifact requires a workspace folder. Ask the user to select their project folder first."
    };
  }
  return { ok: true, path: workspacePath.trim() };
}
function resolveSandboxSourcePath(sandboxRoot, userPath) {
  const trimmed = userPath.trim();
  return resolvePathMustBeInside(
    sandboxRoot,
    remapLegacySharedOutputPath(trimmed)
...
function planPromoteArtifact(input) {
  ...
  if (isPathInsideWorkspace(ws.path, sourcePath)) {
    return {
      ok: false,
      error: "Source is already in the workspace. Use edit_file or write_file for in-repo changes."
    };
  }
  ...
  if (disposition === "non_promotable") {
    return {
      ok: false,
      error: `Cannot promote ${from}: sandbox scripts and read-only dirs are not promotable. Pick a file under output/ or step results/.`
    };
  }
  if (disposition === "temp" && !allowTemp) {
    return {
      ok: false,
      error: `Cannot promote ${from}: classified as temporary scratch output. Use a deliverable under results/, or set allowTemp: true if intentional.`
    };
  }
  return {
    ok: true,
...
async function buildPromotePreview(plan) {
  const sourceStats = await import_fs18.promises.stat(plan.sourcePath).catch(() => null);
  if (!sourceStats?.isFile()) {
    throw new Error(`Source is not a file: ${plan.sourceDisplayPath}`);
  }
  const destExists = await import_fs18.promises.stat(plan.destPath).then(() => true).catch(() => false);
  ...
  const fileChange = buildFileChangePreview(
    ...
    {
      action,
      moveFrom: plan.mode === "move" ? plan.sourcePath : void 0,
...
var promoteArtifact = {
  ...
  tags: [...FILE_SYSTEM_TAG],
  description: `Copy or move a sandbox step deliverable into the user workspace. Use after run_script when a file under output/toolLoop/.../results/ should become part of the project. ${WORKSPACE_PATH_HINT} Source must be a sandbox path (e.g. output/toolLoop/step-1/results/report.html). Destination is workspace-relative (e.g. src/generated.ts). Temp/scratch files require allowTemp: true. ${SANDBOX_ARTIFACT_HINT}`,
  ...
  async execute(input) {
    ...
    try {
      const { fileChange, destExists, sourceContent, destOld } = await buildPromotePreview(plan);
      if (plan.mode === "move") {
      ...
      await import_fs18.promises.mkdir(import_path20.default.dirname(plan.destPath), { recursive: true });
      await import_fs18.promises.copyFile(plan.sourcePath, plan.destPath);
...
async function previewPromoteArtifact(input) {
  const planned = planPromoteArtifact(input);
...
// toolSet/file-system/read-file.ts
...
var readFile = {
  name: "read_file",
  tags: [...FILE_SYSTEM_TAG],
  description: `Read a file or list a directory in the user project. Returns line-numbered content with optional offset/limit. ${WORKSPACE_PATH_HINT}`,
  inputSchema: import_zod17.z.object({
    path: import_zod17.z.string().min(1),
    encoding: import_zod17.z.enum(["utf8", "ascii", "base64", "hex", "latin1"]).optional().default("utf8"),
    offset: import_zod17.z.number().min(1).optional().default(1),
    limit: import_zod17.z.number().min(1).optional().default(MAX_READ_LINES),
    reason: import_zod17.z.string().trim().min(1).optional().describe(
  ...
  async execute(input) {
    ...
    const encoding = typeof rawEncoding === "string" && rawEncoding.trim() !== "" ? rawEncoding : "utf8";
    try {
      await import_fs19.promises.access(targetPath);
      const stats = await import_fs19.promises.stat(targetPath);
      if (stats.isDirectory()) {
        const entries = await import_fs19.promises.readdir(targetPath);
        entries.sort();
        return {
      ...
      if (encoding !== "utf8") {
        const content = await import_fs19.promises.readFile(targetPath, { encoding });
        return {
          path: toToolAbsolutePath(targetPath),
          sandboxRoot: sandbox.root,
          workspacePath: workspacePath ?? void 0,
      ...
      const buffer = await import_fs19.promises.readFile(targetPath);
...
// toolSet/file-system/search-files.ts
...
var searchFiles = {
  name: "search_files",
  tags: [...FILE_SYSTEM_TAG],
  description: `Search for files by name or content in the user project tree. ${WORKSPACE_PATH_HINT} By default skips node_modules, Python package dirs, and hidden paths. Prefer grep_files for regex content search and glob_files for filename patterns.`,
  inputSchema: import_zod18.z.object({
    path: import_zod18.z.string().min(1),
    query: import_zod18.z.string().min(1),
    matchContent: import_zod18.z.boolean().optional(),
    maxDepth: import_zod18.z.number().min(1).optional(),
    include_package_files: includePackageFilesField
  }),
  needsApproval: false,
  async execute(input) {
    ...
    async function scan(dir, depth) {
      const entries = await import_fs20.promises.readdir(dir, { withFileTypes: true });
      for (const entry of entries) {
        if (shouldSkipListingEntry(
          entry.name,
          entry.isDirectory(),
        ...
        const entryPath = import_path21.default.join(dir, entry.name);
        ...
        if (matchContent && entry.isFile()) {
          try {
            const content = await import_fs20.promises.readFile(entryPath, "utf-8");
...
// toolSet/file-system/write-file.ts
...
var writeFile = {
  name: "write_file",
  tags: [...FILE_SYSTEM_TAG],
  description: `Write text data to a file in the user project. Prefer edit_file for partial edits; use write_file for new files or full rewrites. ${WORKSPACE_PATH_HINT}`,
  inputSchema: import_zod19.z.object({
    path: import_zod19.z.string().min(1),
    data: import_zod19.z.string(),
    overwrite: import_zod19.z.boolean().optional().default(false),
    createDirectories: import_zod19.z.boolean().optional().default(true),
    encoding: import_zod19.z.enum(["utf8", "ascii", "base64", "hex", "latin1"]).optional().default("utf8")
  }),
  needsApproval: true,
  async execute(input) {
    const sandbox = requireActiveSandbox();
    ...
    const overwrite = Boolean(input["overwrite"]);
    const createDirectories = input["createDirectories"] === void 0 ? true : Boolean(input["createDirectories"]);
    const rawEncoding = input["encoding"];
    if (typeof rawPath !== "string" || rawPath.trim() === "") {
      return { error: "Invalid path: expected a non-empty string." };
    }
    ...
    const encoding = typeof rawEncoding === "string" && rawEncoding.trim() !== "" ? rawEncoding : "utf8";
    try {
      const contentOld = encoding === "utf8" ? await readTextFileIfExists(targetPath) ?? "" : "";
      if (!overwrite) {
        try {
          await import_fs21.promises.access(targetPath);
          return { error: `File already exists: ${targetPath}` };
        } catch {
        }
      }
      if (createDirectories) {
        await import_fs21.promises.mkdir(import_path22.default.dirname(targetPath), { recursive: true });
      }
      await import_fs21.promises.writeFile(targetPath, rawData, { encoding });
      const stats = await import_fs21.promises.stat(targetPath);
      const result = {
        written: true,
        path: targetPath,
        sandboxRoot: sandbox.root,
...
// toolSet/file-system/file-change-preview.ts
var import_fs22 = require("fs");
init_types4();
init_sandbox_paths();
async function previewEditFile(input) {
  const sandbox = requireActiveSandbox();
  if (!sandbox.ok) return { ok: false, error: sandbox.message };
  const rawPath = input.path;
  const oldString = input.old_string;
  ...
  if (oldString === "") {
    const contentOld2 = await readTextFileIfExists(targetPath) ?? "";
    const action = contentOld2 ? "modify" : "create";
    return {
      ok: true,
      files: [
        buildFileChangePreview(sandbox.root, targetPath, contentOld2, newString, {
          action,
          workspacePath
        })
      ]
  ...
  const stats = await import_fs22.promises.stat(targetPath).catch(() => null);
  if (!stats) {
  ...
  const contentOld = await import_fs22.promises.readFile(targetPath, "utf-8");
  const ending = detectLineEnding(contentOld);
  const old = convertToLineEnding(normalizeLineEndings(oldString), ending);
  const replacement = convertToLineEnding(normalizeLineEndings(newString), ending);
  try {
...
async function previewWriteFile(input) {
  const sandbox = requireActiveSandbox();
  if (!sandbox.ok) return { ok: false, error: sandbox.message };
  const rawPath = input.path;
  const rawData = input.data;
  ...
  const exists = await import_fs22.promises.stat(targetPath).then(() => true).catch(() => false);
  if (exists && !overwrite) {
...
async function previewHunk(sandboxRoot, workspacePath, hunk) {
  switch (hunk.type) {
    case "add": {
    ...
    case "delete": {
      const targetPath = resolvePathInContext(sandboxRoot, workspacePath, hunk.path);
      const previousContent = await import_fs22.promises.readFile(targetPath, "utf-8");
      return {
    ...
    case "update": {
      ...
      const previousContent = await import_fs22.promises.readFile(sourcePath, "utf-8");
      const newContent = deriveNewContentsFromChunks(sourcePath, hunk.chunks, previousContent);
      if (hunk.move_path) {
...
async function previewApplyPatch(input) {
  const sandbox = requireActiveSandbox();
  if (!sandbox.ok) return { ok: false, error: sandbox.message };
  const patchText = input.patch_text;
  if (typeof patchText !== "string" || !patchText.trim()) {
...
async function previewDeleteFile(input) {
  const sandbox = requireActiveSandbox();
  if (!sandbox.ok) return { ok: false, error: sandbox.message };
  const rawPath = input.path;
  if (typeof rawPath !== "string" || !rawPath.trim()) {
...
async function previewMoveFile(input) {
  const sandbox = requireActiveSandbox();
  if (!sandbox.ok) return { ok: false, error: sandbox.message };
  const source = input.source;
  const destination = input.destination;
  ...
  const stats = await import_fs22.promises.stat(sourcePath).catch(() => null);
  ...
  const contentOld = await import_fs22.promises.readFile(sourcePath, "utf-8");
  return {
    ok: true,
    files: [
...
async function previewCopyFile(input) {
  const sandbox = requireActiveSandbox();
  if (!sandbox.ok) return { ok: false, error: sandbox.message };
  const source = input.source;
  const destination = input.destination;
  ...
  const sourceStats = await import_fs22.promises.stat(sourcePath).catch(() => null);
  if (!sourceStats?.isFile()) {
    return { ok: false, error: `Source is not a file: ${sourcePath}` };
  }
  const destExists = await import_fs22.promises.stat(destinationPath).then(() => true).catch(() => false);
  if (destExists && !overwrite) {
    return { ok: false, error: `Destination already exists: ${destinationPath}` };
  }
  const contentNew = await import_fs22.promises.readFile(sourcePath, "utf-8");
  return {
    ok: true,
    files: [
...
async function previewFileChange(toolName, input) {
...
//# sourceMappingURL=f46235f2d12286d9724fd2fc8152a23df8553eb3.js.map
