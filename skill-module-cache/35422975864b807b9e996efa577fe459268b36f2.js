...
function getopenfdeAccountsDir() {
  const dir = (0, import_path.join)(getopenfdeHome(), "accounts");
  ensureDir(dir);
  return dir;
}
...
function ensureDir(path2) {
  (0, import_fs.mkdirSync)(redirectLegacyChannelDataPath(path2), { recursive: true });
}
function ensureParentDirForFile(filePath) {
  ensureDir((0, import_path.dirname)((0, import_path.resolve)(filePath)));
}
function initializeopenfdeHome(app3) {
  const home = (0, import_path.resolve)(resolveopenfdeHomePath());
  if (initialized) {
...
var import_module, import_fs, import_os, import_path, import_meta, openfde_HOME_DIRNAME, openfde_DB_FILENAME, LEGACY_OTTERS_HOME_DIRNAME, LEGACY_OTTERS_DB_FILENAME, openfde_APP_DIRS, openfde_CHANNEL_DATA_DIRS, openfde_DB_DIRNAME, initialized, openfdeHomePath;
...
var import_fs2, CONFIG_PROPERTIES_FILENAME, DEFAULT_SYSTEM_PROPERTIES;
...
function entryCacheKey(filepath) {
  ...
  return (0, import_crypto2.createHash)("sha256").update(filepath).update(String(st.mtimeMs)).digest("hex").slice(0, 40);
}
...
var import_crypto2, import_fs5, import_path4, SKILL_MODULE_FINGERPRINT_SUFFIX;
...
var init_run_context = __esm({
  "src/main/agent/sandbox/run-context.ts"() {
    ...
    log3 = createLogger("sandbox.run-context");
...
var log4, AppCache, appCache;
var init_app_cache = __esm({
  "src/main/cache/app-cache.ts"() {
    init_logger();
    log4 = createLogger("agent.cache");
    AppCache = class {
      constructor() {
      ...
      setAgents(userId, agents) {
        ...
        log4.debug("Cache set: agents", { userId, count: agents.length });
      }
      invalidateAgents(userId) {
        if (this.agentsMap.delete(userId)) {
          log4.debug("Cache invalidated: agents", { userId });
        }
      }
      invalidateAllAgents() {
        this.agentsMap.clear();
        log4.debug("Cache invalidated: all agents");
      }
      // ── credentials ─────────────────────────────────────────────────────────
      getCredentials() {
      ...
      setCredentials(credentials) {
        ...
        log4.debug("Cache set: credentials");
      }
      invalidateCredentials() {
        if (this.credentialsEntry) {
          this.credentialsEntry = null;
          log4.debug("Cache invalidated: credentials");
        }
      }
      // ── MCP tools ───────────────────────────────────────────────────────────
      ...
      setPersona(userId, memoryAgentId, block) {
        ...
        log4.debug("Cache set: persona", { userId, memoryAgentId });
      }
      invalidatePersona(userId, memoryAgentId) {
        const key = `${userId}:${memoryAgentId ?? ""}`;
        if (this.personaMap.delete(key)) {
          log4.debug("Cache invalidated: persona", { userId, memoryAgentId });
        }
      }
      invalidateAllPersona(userId) {
        ...
        log4.debug("Cache invalidated: all persona", { userId });
      }
      // ── diagnostics ─────────────────────────────────────────────────────────
      stats() {
        return {
...
var import_fs7, import_path7, ConversationStore;
  ...
  "src/main/services/conversation-store/store.ts"() {
    ...
    ConversationStore = class {
      ...
      saveDataToFile(userId, relativePath, data) {
        ...
        const workspaceRoot = (0, import_path7.resolve)(workspacePath);
        const targetPath = (0, import_path7.resolve)(workspaceRoot, cleanRelativePath);
        const workspacePrefix = workspaceRoot.endsWith(import_path7.sep) ? workspaceRoot : `${workspaceRoot}${import_path7.sep}`;
        ...
        (0, import_fs7.mkdirSync)((0, import_path7.dirname)(targetPath), { recursive: true });
...
function getConversationStore() {
  if (!_store) {
    _store = instrumentInstanceMethods2(new ConversationStore(), log5);
  }
  return _store;
}
var log5, _store;
var init_conversation_store = __esm({
  "src/main/services/conversation-store/index.ts"() {
    ...
    init_types();
    init_store();
    log5 = createLogger("services.conversation-store");
    _store = null;
  }
});
...
var init_plan_mode_state_machine = __esm({
  "src/main/agent/coding/plan-mode-state-machine.ts"() {
    ...
    log6 = createLogger("agent.plan-mode.state");
    PlanModeStateMachine = class _PlanModeStateMachine {
      ...
      consumeEnterReminder(meta = defaultMeta("injection:consumeEnterReminder")) {
        ...
        if (consumed) {
          log6.debug("Consumed enter plan reminder", {
      ...
      consumeExecuteReminder(meta = defaultMeta("injection:consumeExecuteReminder")) {
        ...
        if (consumed) {
          log6.debug("Consumed execute plan reminder", {
            conversationId: this.conversationId,
            trigger: meta.trigger
          });
        }
      ...
      logUpdate(from, fromStatus, toStatus, meta) {
        ...
        if (statusChanged) {
          log6.info("Explore mode status transition", payload);
        } else {
          log6.info("Explore mode state updated", payload);
        }
      }
    };
  }
...
var log7, READ_FILE_PRUNE_PREVIEW_CHARS;
var init_context_overflow_guard = __esm({
  "src/main/agent/expr/context-overflow-guard.ts"() {
    init_logger();
    init_thread_tagger();
    log7 = createLogger("agent.expr.context-overflow-guard");
...
function resolvePathForContainment(filePath) {
  const abs = import_path8.default.resolve(filePath);
  let current = abs;
  const tail = [];
  for (; ; ) {
    try {
      const real = (0, import_fs8.realpathSync)(current);
      return tail.length > 0 ? import_path8.default.join(real, ...tail) : real;
    } catch {
      const parent = import_path8.default.dirname(current);
      if (parent === current) return abs;
      tail.unshift(import_path8.default.basename(current));
      current = parent;
    }
  }
}
function isPathInsideSandbox(sandboxRoot, absPath) {
  ...
  return rel === "" || !rel.startsWith("..") && !import_path8.default.isAbsolute(rel);
}
...
function isPseudoAbsoluteProjectPath(userPath) {
  ...
  if (!import_path8.default.isAbsolute(trimmed)) return false;
...
function normalizeWorkspaceRelativePath(workspacePath, userPath) {
  const trimmed = userPath.trim();
  if (!trimmed) return trimmed;
  if (import_path8.default.isAbsolute(trimmed) && !isPseudoAbsoluteProjectPath(trimmed)) {
    return trimmed;
  }
  const relativePart = isPseudoAbsoluteProjectPath(trimmed) ? trimmed.replace(/^[/\\]+/, "") : trimmed.replace(/^[/\\]+/, "");
  const wsBase = import_path8.default.basename(import_path8.default.resolve(workspacePath));
...
function resolveRelativeInsideRoot(root, userPath) {
  const base = import_path8.default.resolve(root);
  const trimmed = userPath.trim();
  if (!trimmed) {
  ...
  const resolved = import_path8.default.normalize(import_path8.default.resolve(base, ...segments));
  const rel = import_path8.default.relative(base, resolved);
  if (rel.startsWith("..") || import_path8.default.isAbsolute(rel)) {
...
function resolvePathMustBeInside(sandboxRoot, userPath) {
  ...
  if (import_path8.default.isAbsolute(trimmed)) {
    const abs = import_path8.default.normalize(import_path8.default.resolve(trimmed));
...
function formatPathInContextError(userPath, sandboxRoot, workspacePath) {
  if (!workspacePath?.trim()) {
    if (import_path8.default.isAbsolute(userPath.trim())) {
...
var log8, DEFAULT_RESPONSE_LANGUAGE, DEFAULT_USER_ID, ANTHROPIC_MODELS, SYSTEM_PROP_KEYS, buildToolPromptDescription, withResponseLanguageInstruction, normalizeBaseURL, todoStatusIcon;
var init_config = __esm({
  "src/main/agent/config/config.ts"() {
    init_logger();
    init_constants4();
    init_llm_constants2();
    log8 = createLogger("agent.config");
...
function getMcpServerManager() {
  if (!_manager) {
    _manager = instrumentInstanceMethods2(new McpServerManager(), log9);
  }
  return _manager;
}
var log9, McpServerManager, _manager;
var init_mcp_server_manager = __esm({
  "src/main/services/mcp-server-manager.ts"() {
    init_mcp();
    init_logger();
    init_mcp_server_runtime();
    log9 = createLogger("services.mcp-server-manager");
...
var init_thread_context_builder = __esm({
  "src/main/agent/expr/thread-context-builder.ts"() {
    ...
    init_conversation_store();
    init_thread_tagger();
    log10 = createLogger("agent.expr.thread-context-builder");
  }
});
...
var init_llm_debug_writer = __esm({
  "src/main/agent/llm/llm-debug-writer.ts"() {
    ...
    log11 = createLogger("agent.llm.debug");
...
var log12, ProviderAdapter, OllamaAdapter, LlamaCppAdapter, OpenAIAdapter, AnthropicAdapter, GeminiAdapter, DeepSeekAdapter, ZhipuAdapter, MoonshotAdapter, QwenAdapter, HuggingFaceAdapter, OpenAiCompatibleProviderAdapter, PROVIDER_ADAPTERS;
var init_adapters = __esm({
  "src/main/agent/providers/adapters.ts"() {
    init_openfde_ai();
    init_logger();
    log12 = createLogger("agent.providers.adapters");
    ...
    PROVIDER_ADAPTERS = {
      ollama: instrumentInstanceMethods2(new OllamaAdapter(), log12.child({ provider: "ollama" })),
      llamacpp: instrumentInstanceMethods2(
        new LlamaCppAdapter(),
        log12.child({ provider: "llamacpp" })
      ),
      openai: instrumentInstanceMethods2(new OpenAIAdapter(), log12.child({ provider: "openai" })),
      anthropic: instrumentInstanceMethods2(
        new AnthropicAdapter(),
        log12.child({ provider: "anthropic" })
      ),
      gemini: instrumentInstanceMethods2(new GeminiAdapter(), log12.child({ provider: "gemini" })),
      deepseek: instrumentInstanceMethods2(
        new DeepSeekAdapter(),
        log12.child({ provider: "deepseek" })
      ),
      zhipu: instrumentInstanceMethods2(new ZhipuAdapter(), log12.child({ provider: "zhipu" })),
      moonshot: instrumentInstanceMethods2(new MoonshotAdapter(), log12.child({ provider: "moonshot" })),
      qwen: instrumentInstanceMethods2(new QwenAdapter(), log12.child({ provider: "qwen" })),
      bytedance: openAiCompatibleAdapter("bytedance", "bytedance"),
      huggingface: instrumentInstanceMethods2(
        new HuggingFaceAdapter(),
        log12.child({ provider: "huggingface" })
      ),
...
async function loadSkillsFromDirectory(skillsDir, options) {
  ...
  for (const entry of entries) {
    ...
    const skillFolder = (0, import_path9.join)(skillsDir, entry);
    const skillFile = (0, import_path9.join)(skillFolder, SKILL_FILES.SKILL_MD);
    try {
      ...
      log13.warn(SKILL_LOADER_LOG.SKIPPED_INVALID, {
        skillId: entry,
        folder: skillFolder
      });
    } catch (err) {
      log13.warn(SKILL_LOADER_LOG.SKIPPED_FAILED, {
        skillId: entry,
        folder: skillFolder,
        err
      });
  ...
  log13.info(SKILL_LOADER_LOG.LOADED, {
    skillsDir,
    count: skills.length,
...
var import_fs9, import_path9, log13;
var init_skills_directory_loader = __esm({
  "src/main/skills/skills-directory-loader.ts"() {
    ...
    init_skill_workspace_tool_defaults();
    init_conversation_store();
    log13 = createLogger("skills.loader");
  }
});
...
var log14, getSkillsDir, parseSkillMarkdown2, skillToAgent;
var init_skills = __esm({
  "src/main/skills/skills.ts"() {
    ...
    init_actions();
    init_skill_visibility();
    log14 = createLogger("skills.skills");
    ...
    skillToAgent = traceFunction2(log14, "skillToAgent", skillToAgent2);
  }
});
...
function syncSandboxOutputViewImpl(event, args) {
  ...
  if (!view) {
    view = new import_electron4.WebContentsView({
      webPreferences: {
    ...
    if (!view.webContents.isDestroyed()) {
      view.webContents.on("did-fail-load", (_event, errorCode, errorDesc, url) => {
        log16.error("Sandbox output view failed to load", {
          errorCode,
          errorDesc,
          url
        });
...
var import_electron4, sandboxResultsViews, closedHandlersRegistered, log16, syncSandboxOutputView;
var init_output_view = __esm({
  "src/main/agent/sandbox/output-view.ts"() {
    ...
    log16 = createLogger("sandbox.output-view");
...
function realpathSafe(p) {
  const r = (0, import_path10.resolve)(p.trim());
  try {
...
function isRemovableopenfdeSandboxPathImpl(candidatePath) {
  ...
  let name;
  try {
    const absReal = (0, import_fs10.realpathSync)((0, import_path10.resolve)(candidatePath.trim()));
    parentReal = (0, import_path10.dirname)(absReal);
    name = (0, import_path10.basename)(absReal);
  } catch {
    const abs = (0, import_path10.resolve)(candidatePath.trim());
    name = (0, import_path10.basename)(abs);
    parentReal = realpathSafe((0, import_path10.dirname)(abs));
  }
  if (parentReal === openfdeSandboxReal) return true;
  try {
    const absReal = (0, import_fs10.realpathSync)((0, import_path10.resolve)(candidatePath.trim()));
    const rel = (0, import_path10.relative)(openfdeSandboxReal, absReal);
    if (rel && !rel.startsWith("..") && !rel.startsWith(`..${import_path10.sep}`) && (rel.startsWith("sub-agents") || rel.includes("/sub-agents/"))) {
      return true;
    }
  } catch {
...
async function removeSandboxDirectoriesImpl(paths) {
  for (const p of paths) {
    const abs = (0, import_path10.resolve)(p.trim());
    if (!isRemovableopenfdeSandboxPath(abs)) {
      log17.warn("Skipped sandbox removal outside allowed sandbox roots", {
        path: p,
    ...
    try {
      ...
      log17.info("Removed sandbox directory", { path: abs });
    } catch (err) {
      log17.error("Failed to remove sandbox directory", {
        path: abs,
        err
      });
    }
...
var import_fs10, import_promises, import_os2, import_path10, log17, isRemovableopenfdeSandboxPath, removeSandboxDirectories;
var init_cleanup = __esm({
  "src/main/agent/sandbox/cleanup.ts"() {
    ...
    init_logger();
    log17 = createLogger("sandbox.cleanup");
...
var init_sandbox_globals_lock = __esm({
  "src/main/agent/sandbox/sandbox-globals-lock.ts"() {
    ...
    init_paths();
    init_run_context();
    log18 = createLogger("sandbox.globals-lock");
...
// skills/google-workspace/actions/index.ts
var index_exports = {};
__export(index_exports, {
  default: () => index_default,
  googleCalendarCreateEvent: () => googleCalendarCreateEvent,
  googleCalendarListCalendars: () => googleCalendarListCalendars,
  googleCalendarListEvents: () => googleCalendarListEvents,
  googleDriveDownload: () => googleDriveDownload,
  googleDriveGetFile: () => googleDriveGetFile,
  googleDriveListFiles: () => googleDriveListFiles,
  googleGmailGetMessage: () => googleGmailGetMessage,
  googleGmailListMessages: () => googleGmailListMessages,
  googleGmailSend: () => googleGmailSend,
  googleWorkspaceApi: () => googleWorkspaceApi,
  googleWorkspaceAuthStatus: () => googleWorkspaceAuthStatus,
  googleWorkspaceRequest: () => googleWorkspaceRequest,
  googleWorkspaceTools: () => googleWorkspaceTools,
  tools: () => tools
});
module.exports = __toCommonJS(index_exports);

// skills/google-workspace/actions/google-workspace.ts
...
// config/google-oauth-defaults.ts
var BUNDLED_GOOGLE_OAUTH_CLIENT_ID = "254852600906-ta5em3q6et8963rtbep7phqmmnsk3dks.apps.googleusercontent.com";
var BUNDLED_GOOGLE_OAUTH_CLIENT_SECRET = "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";
...
var log2 = createLogger("services.google-oauth");
function resolveGoogleOAuthCredentials() {
  const clientId = getSystemPropValue("app.google.clientId", "").trim() || process.env.GOOGLE_CLIENT_ID?.trim() || BUNDLED_GOOGLE_OAUTH_CLIENT_ID;
  const clientSecret = getSystemPropValue("app.google.clientSecret", "").trim() || process.env.GOOGLE_CLIENT_SECRET?.trim() || BUNDLED_GOOGLE_OAUTH_CLIENT_SECRET;
...
function getTokenFilePath() {
  ...
  return (0, import_path2.join)(dir, "google-account.json");
}
function loadStoredAccountImpl() {
  try {
...
function persistAccount(account) {
  ...
  if (!(0, import_fs3.existsSync)(dir)) (0, import_fs3.mkdirSync)(dir, { recursive: true });
  (0, import_fs3.writeFileSync)(filePath, JSON.stringify(account, null, 2), "utf-8");
}
function clearStoredAccountImpl() {
  try {
    const filePath = getTokenFilePath();
    if ((0, import_fs3.existsSync)(filePath)) {
      (0, import_fs3.writeFileSync)(filePath, "", "utf-8");
    }
  } catch {
  }
}
function httpsPost(url, params) {
  return new Promise((resolve6, reject) => {
    ...
    const parsed = (0, import_url.parse)(url);
...
function waitForOAuthCallback(port, state) {
  return new Promise((resolve6, reject) => {
    const server = (0, import_http.createServer)((req, res) => {
      const reqUrl = (0, import_url.parse)(req.url ?? "", true);
      const code = reqUrl.query.code;
      const returnedState = reqUrl.query.state;
      const error = reqUrl.query.error;
      const html = (msg) => `<html><body style="font-family:sans-serif;padding:40px"><h2>${msg}</h2><p>You can close this window.</p></body></html>`;
      if (error) {
...
async function startGoogleSignInImpl() {
  log2.info("Starting Google sign-in flow");
  const { clientId, clientSecret } = getCredentials();
  const port = 7779;
  const redirectUri = `http://127.0.0.1:${port}`;
  const state = (0, import_crypto.randomBytes)(16).toString("hex");
  const codeVerifier = base64Url((0, import_crypto.randomBytes)(64));
  const codeChallenge = base64Url(
    (0, import_crypto.createHash)("sha256").update(codeVerifier).digest()
  );
  ...
  const tokens = {
    ...
    expires_at: Date.now() + Number(tokenResponse.expires_in ?? 3600) * 1e3,
  ...
  const userInfo = {
    ...
    picture: userInfoRaw.picture ?? ""
  };
  const account = { tokens, userInfo };
  persistAccount(account);
  return account;
}
async function refreshGoogleTokenImpl(account) {
  log2.info("Refreshing Google access token");
  const { clientId, clientSecret } = getCredentials();
  ...
  account.tokens.access_token = tokenResponse.access_token;
  account.tokens.expires_at = Date.now() + Number(tokenResponse.expires_in ?? 3600) * 1e3;
  if (tokenResponse.refresh_token) {
    account.tokens.refresh_token = tokenResponse.refresh_token;
  }
  persistAccount(account);
  return account;
}
async function getValidAccessTokenImpl() {
  ...
  if (Date.now() >= account.tokens.expires_at - 6e4) {
    account = await refreshGoogleToken(account);
  }
  return account.tokens.access_token;
}
var loadStoredAccount = traceFunction2(
  log2,
  "loadStoredAccount",
  loadStoredAccountImpl
);
var clearStoredAccount = traceFunction2(
  log2,
  "clearStoredAccount",
  clearStoredAccountImpl
);
var startGoogleSignIn = traceFunction2(
  log2,
  "startGoogleSignIn",
  startGoogleSignInImpl
);
var refreshGoogleToken = traceFunction2(
  log2,
  "refreshGoogleToken",
  refreshGoogleTokenImpl
);
var getValidAccessToken = traceFunction2(
  log2,
  "getValidAccessToken",
...
function formatAuthHint(error) {
  const base = "Ask the user to sign in via Settings \u2192 Google Account. If they already signed in before Workspace was enabled, sign out and sign in again.";
  return error ? `${error} ${base}` : base;
}
async function googleWorkspaceRequest(options) {
  ...
  try {
    ...
    if (!res.ok) {
      const apiMessage = typeof data === "object" && data !== null && "error" in data && typeof data.error?.message === "string" ? data.error.message : res.statusText;
      const scopeHint = res.status === 403 ? " Insufficient OAuth scopes \u2014 sign out and sign in again in Settings \u2192 Google Account." : "";
      return {
        success: false,
...
function encodeGmailRaw(message) {
  return Buffer.from(message, "utf-8").toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
}
function buildMimeMessage(input) {
  const fromLine = input.from?.trim() ? `From: ${input.from.trim()}\r
` : "";
  return `${fromLine}To: ${input.to.trim()}\r
Subject: ${input.subject.trim()}\r
Content-Type: text/plain; charset=utf-8\r
\r
${input.body}`;
}
function defineGoogleWorkspaceTool(config) {
  const tool = {
    name: config.name,
...
var googleWorkspaceAuthStatus = defineGoogleWorkspaceTool({
  name: "google_workspace_auth_status",
  description: "Check Google sign-in for Workspace tools (Gmail, Calendar, Drive). Read-only. Run first if other google_* tools fail.",
...
var googleGmailListMessages = defineGoogleWorkspaceTool({
  name: "google_gmail_list_messages",
  description: "List Gmail message ids matching a search query (Gmail search syntax). Read-only.",
  inputSchema: import_zod7.z.object({
    query: import_zod7.z.string().optional().describe("Gmail search query (e.g. is:unread from:alice)."),
    maxResults: import_zod7.z.number().int().min(1).max(100).optional().default(20),
    labelIds: import_zod7.z.array(import_zod7.z.string()).optional().describe("Filter by label ids (e.g. INBOX, UNREAD).")
  }),
  async execute(input) {
...
var googleGmailGetMessage = defineGoogleWorkspaceTool({
  ...
  inputSchema: import_zod7.z.object({
    messageId: import_zod7.z.string().min(1),
    format: import_zod7.z.enum(["minimal", "full", "raw", "metadata"]).optional().default("metadata").describe("Gmail API format parameter.")
  }),
  async execute(input) {
...
var googleGmailSend = defineGoogleWorkspaceTool({
  ...
  inputSchema: import_zod7.z.object({
    ...
    body: import_zod7.z.string().min(1),
    from: import_zod7.z.string().optional().describe("Optional From header (defaults to signed-in account).")
  }),
  needsApproval: true,
  async execute(input) {
...
var googleCalendarListCalendars = defineGoogleWorkspaceTool({
  name: "google_calendar_list_calendars",
  description: "List calendars on the signed-in Google account. Read-only.",
  inputSchema: import_zod7.z.object({
    maxResults: import_zod7.z.number().int().min(1).max(250).optional().default(50)
  }),
  async execute(input) {
...
var googleCalendarListEvents = defineGoogleWorkspaceTool({
  name: "google_calendar_list_events",
  description: "List events on a calendar within a time range. Read-only.",
  inputSchema: import_zod7.z.object({
    calendarId: import_zod7.z.string().optional().default("primary").describe("Calendar id (default: primary)."),
    timeMin: import_zod7.z.string().optional().describe("RFC3339 lower bound (inclusive)."),
    timeMax: import_zod7.z.string().optional().describe("RFC3339 upper bound (exclusive)."),
    maxResults: import_zod7.z.number().int().min(1).max(250).optional().default(25),
    q: import_zod7.z.string().optional().describe("Free-text search query.")
  }),
  async execute(input) {
    const params = new URLSearchParams({
...
var googleCalendarCreateEvent = defineGoogleWorkspaceTool({
  name: "google_calendar_create_event",
  description: "Create a calendar event. Requires approval.",
  inputSchema: import_zod7.z.object({
    calendarId: import_zod7.z.string().optional().default("primary"),
    summary: import_zod7.z.string().min(1),
    description: import_zod7.z.string().optional(),
    start: import_zod7.z.string().min(1).describe("RFC3339 start datetime."),
    end: import_zod7.z.string().min(1).describe("RFC3339 end datetime."),
    timeZone: import_zod7.z.string().optional().describe("IANA time zone (e.g. America/Los_Angeles)."),
    attendees: import_zod7.z.array(import_zod7.z.string().email()).optional()
  }),
  needsApproval: true,
  async execute(input) {
    const body = {
...
var googleDriveListFiles = defineGoogleWorkspaceTool({
  name: "google_drive_list_files",
  description: "List or search Google Drive files. Read-only.",
  inputSchema: import_zod7.z.object({
    query: import_zod7.z.string().optional().describe('Drive query (e.g. name contains "report" and mimeType = "application/pdf").'),
    pageSize: import_zod7.z.number().int().min(1).max(100).optional().default(20),
    fields: import_zod7.z.string().optional().default("files(id,name,mimeType,modifiedTime,webViewLink,parents)")
  }),
  async execute(input) {
    const params = new URLSearchParams({
...
var googleDriveGetFile = defineGoogleWorkspaceTool({
  name: "google_drive_get_file",
  description: "Get Google Drive file metadata by id. Read-only.",
  inputSchema: import_zod7.z.object({
    fileId: import_zod7.z.string().min(1),
    fields: import_zod7.z.string().optional().default("id,name,mimeType,size,modifiedTime,webViewLink,parents")
  }),
  async execute(input) {
...
var googleDriveDownload = defineGoogleWorkspaceTool({
  name: "google_drive_download",
  description: "Download a Drive file into the agent sandbox (export Google Docs/Sheets or download binary). Requires approval.",
  inputSchema: import_zod7.z.object({
    fileId: import_zod7.z.string().min(1),
    destination: import_zod7.z.string().min(1).describe("Sandbox-relative path to write the file (e.g. output/toolLoop/step/results/doc.pdf)."),
    exportMimeType: import_zod7.z.string().optional().describe(
  ...
  async execute(input) {
    ...
    const url = input.exportMimeType?.trim() ? `${DRIVE_BASE}/files/${fileId}/export?mimeType=${encodeURIComponent(input.exportMimeType.trim())}` : `${DRIVE_BASE}/files/${fileId}?alt=media`;
    try {
      ...
      (0, import_fs11.mkdirSync)((0, import_path11.dirname)(destAbs), { recursive: true });
      (0, import_fs11.writeFileSync)(destAbs, buffer);
      return {
        success: true,
...
var googleWorkspaceApi = defineGoogleWorkspaceTool({
  name: "google_workspace_api",
  description: "Call a Google Workspace REST API (Gmail, Calendar, Drive, etc.) with the signed-in account. Mutating methods require approval. Prefer dedicated google_* tools when available.",
  inputSchema: import_zod7.z.object({
    method: import_zod7.z.enum(["GET", "POST", "PATCH", "PUT", "DELETE"]).optional().default("GET"),
    url: import_zod7.z.string().min(1).describe("Full HTTPS URL on googleapis.com (must start with https://)."),
    body: import_zod7.z.record(import_zod7.z.string(), import_zod7.z.unknown()).optional().describe("JSON body for POST/PATCH/PUT.")
  }),
...
// skills/google-workspace/actions/index.ts
var tools = googleWorkspaceTools;
...
//# sourceMappingURL=35422975864b807b9e996efa577fe459268b36f2.js.map
