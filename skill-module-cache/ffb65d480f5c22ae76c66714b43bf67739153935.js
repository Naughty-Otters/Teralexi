...
function redirectLegacyChannelDataPath(targetPath) {
  ...
  for (const name of openfde_CHANNEL_DATA_DIRS) {
    ...
    if (resolved === legacyRoot || resolved.startsWith(`${legacyRoot}${import_path.sep}`)) {
      const relative3 = resolved.slice(legacyRoot.length).replace(/^[/\\]+/, "");
      return relative3 ? (0, import_path.join)(channelsRoot, name, relative3) : (0, import_path.join)(channelsRoot, name);
    }
  }
  return targetPath;
}
...
function filterByDeclared(tools2, declaredNames) {
  if (declaredNames.length === 0) return tools2;
  const nameSet = new Set(declaredNames);
  return tools2.filter((t) => nameSet.has(t.name));
}
async function loadSkillActionsFromDirectory(actionsDir) {
  ...
  for (const candidate of indexCandidates) {
    ...
    if (!loaded) continue;
    const tools2 = collectToolsFromModule(loaded);
    if (tools2.length > 0) return tools2;
  }
  const allTools = [];
  let entries;
  try {
...
async function loadToolSetToolsFromDirectory(toolSetDir) {
  ...
  const withDefaultTags = (tools2, defaultTag) => {
...
async function loadToolSetTools() {
  ...
  for (const toolSetDir of resolveToolSetSourceRoots()) {
    const tools2 = await loadToolSetToolsFromDirectory(toolSetDir);
    for (const tool of tools2) {
...
var init_app_cache = __esm({
  "src/main/cache/app-cache.ts"() {
    ...
    AppCache = class {
      ...
      setMcpTools(userId, agentId, tools2) {
        this.mcpToolsMap.set(`${userId}:${agentId}`, { tools: tools2 });
        log3.debug("Cache set: mcpTools", { userId, agentId, count: tools2.length });
      }
...
function filterToolsByAvailableSet(tools2, options) {
  if (!options.availableSetTouched) {
    return tools2.map((tool) => tool.name);
  }
...
function normalizeExecutionSteps(agent) {
  ...
  const tools2 = agent.availableSkillTools ?? existing?.toolLoop?.tools ?? [];
...
function applySubAgentSettingsToExecutionSteps(agent) {
  ...
  const tools2 = prevToolLoop.tools ?? agent.availableSkillTools ?? [];
  const maxIterations = resolveToolLoopMaxIterations(
  ...
  if (agent.allowSubAgents === false) {
    ...
    if (Object.keys(rest).length > 0 || tools2.length > 0) {
      steps.toolLoop = { ...rest, tools: tools2, maxIterations };
    } else {
  ...
  "src/main/services/mcp-server-manager.ts"() {
    ...
    McpServerManager = class {
      ...
      async callTool(server, toolName, input, context) {
        ...
        const targetTool = tools2[toolName];
        if (!targetTool) {
...
async function loadMcpToolsForAgentFromServers(userId, agent) {
  ...
  for (const server of enabledServers) {
    ...
    try {
      const tools2 = await getMcpServerManager().listTools(server);
      for (const tool of tools2) {
        mcpTools.push({
          name: tool.name,
...
async function loadMcpToolsForAgent(userId, agent) {
  ...
  if (cached) return cached;
  const tools2 = await loadMcpToolsForAgentFromServers(userId, agent);
  appCache.setMcpTools(userId, agent.id, tools2);
  return tools2;
}
...
function tagToolsForSkill(tools2, skillId) {
...
// src/main/skills/skill-attachment-dirs.ts
function parsePropertiesKeyValues2(raw) {
  const out = {};
  for (const line of raw.split("\n")) {
    const m = line.match(/^([\w-]+):\s*(.+)$/);
    if (!m) continue;
...
function splitAttachmentDirPropertyValue(raw) {
  return raw.split(",").map((part) => part.trim()).filter(Boolean);
}
function tryNormalizeAttachmentDir(raw) {
...
function parseAttachmentDirList(raw, fallback) {
  const parts = raw?.trim() ? splitAttachmentDirPropertyValue(raw) : [];
  const normalized = parts.map((part) => tryNormalizeAttachmentDir(part)).filter((dir) => dir.length > 0);
  const unique = [...new Set(normalized)];
  return unique.length > 0 ? unique : [fallback];
}
function parseAttachmentDirsFromProperties(propertiesRaw) {
  const kv = parsePropertiesKeyValues2(propertiesRaw);
  return {
...
function resolveSkillAttachmentDirs(skillFolder) {
  const propertiesFile = (0, import_path10.join)(skillFolder, SKILL_FILES.PROPERTIES_MD);
  if (!(0, import_fs10.existsSync)(propertiesFile)) {
    return {
  ...
  try {
    return parseAttachmentDirsFromProperties(
      (0, import_fs10.readFileSync)(propertiesFile, "utf-8")
    );
  } catch {
    return {
      ref: [...DEFAULT_SKILL_ATTACHMENT_DIRS.ref],
...
var import_fs10, import_path10, DEFAULT_SKILL_ATTACHMENT_DIRS, RESERVED_ATTACHMENT_DIR_SEGMENTS;
var init_skill_attachment_dirs = __esm({
  "src/main/skills/skill-attachment-dirs.ts"() {
    ...
    RESERVED_ATTACHMENT_DIR_SEGMENTS = /* @__PURE__ */ new Set([
      ...SKILLS_RESERVED_DIR_NAMES,
...
// skills/documents/actions/index.ts
var index_exports = {};
__export(index_exports, {
  createPresentation: () => createPresentation,
  createSpreadsheet: () => createSpreadsheet,
  createWordDoc: () => createWordDoc,
...
// src/main/skills/skill-attachments.ts
var import_fs11 = require("fs");
var import_path11 = require("path");
init_constants();
init_skill_attachment_dirs();
init_skill_path();
...
function walkFiles(dir, rootDir, out) {
  ...
  for (const entry of entries) {
    ...
    let st;
    try {
      st = (0, import_fs11.statSync)(full);
    } catch {
      continue;
    }
    if (st.isDirectory()) {
    ...
    out.push((0, import_path11.relative)(rootDir, full).split("\\").join("/"));
  }
}
function listCategoryFiles(skillFolder, category, dirs) {
  const paths = [];
  for (const categoryDir of dirs[category]) {
    const dir = (0, import_path11.join)(skillFolder, categoryDir);
    if (!(0, import_fs11.existsSync)(dir)) continue;
    const files = [];
...
function listSkillAttachments(skillId) {
  const id = skillId.trim();
  if (!id) return [];
  const { bundled, user } = resolveSkillsSources();
  const roots = [
    { source: "bundled", dir: (0, import_path11.join)(bundled, id) },
    { source: "user", dir: (0, import_path11.join)(user, id) }
  ];
  const merged = /* @__PURE__ */ new Map();
  for (const { source, dir } of roots) {
    if (!(0, import_fs11.existsSync)((0, import_path11.join)(dir, SKILL_FILES.SKILL_MD))) continue;
    const attachmentDirs = resolveSkillAttachmentDirs(dir);
    for (const category of Object.keys(
      attachmentDirs
    )) {
      for (const rel of listCategoryFiles(dir, category, attachmentDirs)) {
        const absolutePath = (0, import_path11.join)(dir, rel);
        let sizeBytes = 0;
        try {
          sizeBytes = (0, import_fs11.statSync)(absolutePath).size;
        } catch {
          continue;
        }
        merged.set(rel, {
          category,
          relativePath: rel,
          fileName: (0, import_path11.basename)(rel),
          absolutePath,
          source,
          sizeBytes
        });
...
function resolveSkillAttachment(skillId, relativePath) {
  const normalized = relativePath.replace(/^[/\\]+/, "").split("\\").join("/");
  return listSkillAttachments(skillId).find((a) => a.relativePath === normalized) ?? null;
}
function readSkillAttachment(skillId, relativePath) {
  const entry = resolveSkillAttachment(skillId, relativePath);
  if (!entry) {
    throw new Error(`Attachment not found: ${relativePath}`);
  }
  const lower = entry.fileName.toLowerCase();
  const asText = TEXT_EXTENSIONS.has(lower) || lower.endsWith(".form.md") || [...TEXT_EXTENSIONS].some((ext) => lower.endsWith(ext));
  if (asText) {
    return {
      content: (0, import_fs11.readFileSync)(entry.absolutePath, "utf-8"),
      encoding: "utf8",
      mimeType: guessMimeType(entry.fileName, true)
    };
  }
  return {
    content: (0, import_fs11.readFileSync)(entry.absolutePath).toString("base64"),
    encoding: "base64",
    mimeType: guessMimeType(entry.fileName, false)
  };
}
function guessMimeType(fileName, text) {
...
// skills/documents/actions/template-core/types.ts
var DOCUMENTS_SKILL_ID = "documents";

// skills/documents/actions/template-core/load-themes.ts
var PPT_THEMES_PATH = "templates/styles/ppt-themes.json";
var EXCEL_THEMES_PATH = "templates/styles/excel-themes.json";
function loadJsonAttachment(relativePath) {
  const { content } = readSkillAttachment(DOCUMENTS_SKILL_ID, relativePath);
  return JSON.parse(content);
}
function loadPptThemes() {
  return loadJsonAttachment(PPT_THEMES_PATH);
}
function loadExcelThemes() {
  return loadJsonAttachment(EXCEL_THEMES_PATH);
}
function resolvePptTheme(themeKey) {
  const themes = loadPptThemes();
  return themes[themeKey] ?? themes.navy;
}
function resolveExcelTheme(themeKey) {
...
function slugify(name) {
  return name.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "") || "document";
}
var createSpreadsheet = {
  name: "create_spreadsheet",
  description: "Create a formatted Excel spreadsheet (.xlsx) from column headers and row data. Writes directly to the sandbox output/results/ directory. Returns the output path. Supports optional charts (bar, line, pie).",
  inputSchema: void 0,
  // described in skill.md — no zod needed
  async execute(input) {
    const sandbox = requireActiveSandbox();
    if (!sandbox.ok) return { error: sandbox.message };
    const resultsDir = import_path12.default.join(sandbox.root, getOutputResultsRelPrefix());
    import_fs12.default.mkdirSync(resultsDir, { recursive: true });
    ...
    const columns = Array.isArray(rawColumns) ? rawColumns.map(String) : String(rawColumns ?? "").split(",").map((s) => s.trim()).filter(Boolean);
    if (!columns.length) return { error: "columns is required" };
    const rows = parseRows(input["rows"]);
    const excelTheme = input["excel_theme"] ?? resolveExcelTheme("corporate-blue");
    const headerColor = String(
      input["header_color"] ?? excelTheme.header.fill ?? "2E75B6"
    );
    const addChart = Boolean(input["add_chart"]);
    const chartType = String(input["chart_type"] ?? "bar");
    const chartColumn = String(input["chart_column"] ?? columns[1] ?? columns[0]);
    const outputPath = import_path12.default.join(resultsDir, `${filename}.xlsx`);
    ...
    rows.forEach((row, rowIdx) => {
      ...
      dataRow.eachCell((cell) => {
        ...
        if (typeof cell.value === "number") {
          ...
          cell.alignment = {
            horizontal: excelTheme.numbers?.align ?? "right"
          };
        }
      });
    });
    ...
    if (addChart && rows.length > 0) {
      ...
      const labelColIdx = 1;
      if (chartType === "bar") {
        const chart = wb.addChart("bar", { top5: false });
        void chart;
      }
      try {
        const chart = wb.addChart("bar", {});
        chart.title = { name: chartColumn };
        ws.addImage(ws.getRow(1).getCell(dataColIdx), {
...
var readSpreadsheet = {
  ...
  description: "Read an existing Excel (.xlsx) file and return its contents as structured JSON (columns array + rows array). Use this before update operations to see the current data.",
  async execute(input) {
    const sandbox = requireActiveSandbox();
    if (!sandbox.ok) return { error: sandbox.message };
    ...
    const sheetName = input["sheet_name"] ? String(input["sheet_name"]) : void 0;
    ...
    if (!ws) {
      return {
        error: sheetName ? `Sheet "${sheetName}" not found. Available: ${wb.worksheets.map((s) => s.name).join(", ")}` : "No worksheets found"
      };
    }
    const headerRow = ws.getRow(1);
    const columns = [];
    ...
    return {
      ...
      message: `Read ${rows.length} rows \xD7 ${columns.length} columns from "${ws.name}"`
    };
  }
};
...
init_sandbox_paths();
function resolveThemeFromInput(input) {
  const themeKey = String(input["theme"] ?? "navy");
  const fromInput = input["ppt_theme"];
  const theme = fromInput ?? resolvePptTheme(themeKey);
  const customColor = input["theme_color"] ? String(input["theme_color"]).replace("#", "") : void 0;
  if (customColor) theme.bg = customColor;
  return { themeKey, theme };
}
function slugify2(name) {
  return name.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "") || "presentation";
}
var createPresentation = {
  name: "create_presentation",
  description: "Create a PowerPoint presentation (.pptx) from a list of slide definitions. Each slide has a title and optional bullet points. Writes to sandbox output/results/<filename>.pptx.",
  async execute(input) {
    const sandbox = requireActiveSandbox();
    if (!sandbox.ok) return { error: sandbox.message };
    const resultsDir = import_path13.default.join(sandbox.root, getOutputResultsRelPrefix());
    import_fs13.default.mkdirSync(resultsDir, { recursive: true });
    const filename = slugify2(String(input["output_filename"] ?? "presentation"));
    const presTitle = String(input["presentation_title"] ?? "Presentation");
    const presenter = String(input["presenter"] ?? "");
    const { themeKey, theme } = resolveThemeFromInput(input);
      ...
      slides = rawSlides.map((s) => {
        if (typeof s === "object" && s !== null) {
          ...
          return {
            ...
            bullets: Array.isArray(obj["bullets"]) ? obj["bullets"].map(String) : typeof obj["bullets"] === "string" ? obj["bullets"].split("|").map((b) => b.trim()).filter(Boolean) : [],
            notes: obj["notes"] ? String(obj["notes"]) : void 0
          };
        }
        return { title: String(s), bullets: [] };
      });
    } else if (typeof rawSlides === "string") {
      slides = rawSlides.trim().split("\n").filter(Boolean).map((line) => {
    ...
    const outputPath = import_path13.default.join(resultsDir, `${filename}.pptx`);
    const prs = new import_pptxgenjs.default();
...
var HEADER_COLOR = "2E75B6";
var HEADER_TEXT = "FFFFFF";
var ALT_ROW_BG = "EBF3FB";
var createWordDoc = {
  name: "create_word_doc",
  description: "Create a formatted Word document (.docx) from a list of sections (heading + body text). Optionally includes a data table, author, and title page. Writes to sandbox output/results/<filename>.docx.",
  async execute(input) {
    const sandbox = requireActiveSandbox();
    if (!sandbox.ok) return { error: sandbox.message };
    const resultsDir = import_path14.default.join(sandbox.root, getOutputResultsRelPrefix());
    import_fs14.default.mkdirSync(resultsDir, { recursive: true });
    const filename = slugify3(String(input["output_filename"] ?? "document"));
    ...
    const outputPath = import_path14.default.join(resultsDir, `${filename}.docx`);
    const children = [];
    children.push(
      new import_docx.Paragraph({
        children: [new import_docx.TextRun({ text: docTitle, bold: true, size: 56, color: HEADER_COLOR })],
    ...
    if (author) {
      children.push(
        new import_docx.Paragraph({
          children: [
            new import_docx.TextRun({ text: `${author}  \xB7  ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}`, size: 22, color: "777777" })
          ],
          alignment: import_docx.AlignmentType.CENTER,
          spacing: { after: 480 }
        })
    ...
    children.push(
      new import_docx.Paragraph({
        text: "",
        border: { bottom: { style: import_docx.BorderStyle.SINGLE, size: 6, color: HEADER_COLOR } },
    ...
    sections.forEach((section) => {
      children.push(
        new import_docx.Paragraph({
          ...
          heading: section.level === 2 ? import_docx.HeadingLevel.HEADING_2 : import_docx.HeadingLevel.HEADING_1,
      ...
      if (paragraphs.length) {
        paragraphs.forEach((para) => {
          children.push(
            new import_docx.Paragraph({
              children: [new import_docx.TextRun({ text: para.replace(/\n/g, " "), size: 24 })],
              alignment: isFormal ? import_docx.AlignmentType.JUSTIFIED : import_docx.AlignmentType.LEFT,
        ...
        children.push(
          new import_docx.Paragraph({
            children: [new import_docx.TextRun({ text: section.body, size: 24 })],
            alignment: isFormal ? import_docx.AlignmentType.JUSTIFIED : import_docx.AlignmentType.LEFT,
            spacing: { before: 0, after: 160 }
          })
        );
    ...
    if (tableData && tableData.headers.length > 0) {
      ...
      const colWidth = Math.floor(9360 / tableData.headers.length);
      const headerCells = tableData.headers.map(
        (h) => new import_docx.TableCell({
          children: [
            new import_docx.Paragraph({
              children: [new import_docx.TextRun({ text: h, bold: true, color: HEADER_TEXT, size: 22 })],
              alignment: import_docx.AlignmentType.CENTER
            })
          ],
          shading: { type: import_docx.ShadingType.SOLID, color: HEADER_COLOR },
          width: { size: colWidth, type: import_docx.WidthType.DXA }
        })
      );
      const dataRows = tableData.rows.map(
        (row, rowIdx) => new import_docx.TableRow({
          children: tableData.headers.map(
            (_, colIdx) => new import_docx.TableCell({
              children: [
                new import_docx.Paragraph({
                  children: [new import_docx.TextRun({ text: row[colIdx] ?? "", size: 22 })]
                })
              ],
              shading: rowIdx % 2 === 0 ? { type: import_docx.ShadingType.SOLID, color: ALT_ROW_BG } : void 0,
              width: { size: colWidth, type: import_docx.WidthType.DXA }
            })
          )
        })
      ...
      children.push(
        new import_docx.Table({
          rows: [new import_docx.TableRow({ children: headerCells }), ...dataRows],
      ...
      sections: [
        {
          properties: {
            page: {
              margin: {
                top: (0, import_docx.convertInchesToTwip)(1),
                right: (0, import_docx.convertInchesToTwip)(1.2),
                bottom: (0, import_docx.convertInchesToTwip)(1),
                left: (0, import_docx.convertInchesToTwip)(1.2)
              }
            }
          },
          children
    ...
    import_fs14.default.writeFileSync(outputPath, buffer);
    return {
      success: true,
      file_path: outputPath,
  ...
  await new Promise((resolve6, reject) => {
    ...
    webContents.once("did-finish-load", () => {
      ...
      void webContents.executeJavaScript(
        ...
        clearTimeout(timeout);
        reject(
          err instanceof Error ? err : new Error(`PDF export font/layout prep failed: ${String(err)}`)
        );
      });
    });
  });
}
async function exportHtmlFileToPdf(htmlFilePath, pdfFilePath) {
  ...
  try {
    ...
    await (0, import_promises2.writeFile)(pdfFilePath, data);
  } catch (err) {
...
// skills/documents/actions/template-core/load-manifest.ts
var MANIFEST_PATH = "templates/manifest.json";
...
function getTemplateById(manifest, templateId) {
  const id = templateId.trim();
  return manifest.templates.find((t) => t.id === id) ?? null;
}

// skills/documents/actions/template-core/normalize-data.ts
function parseSectionsOutline(raw) {
  return raw.trim().split("\n").filter(Boolean).map((line) => {
    const colonIdx = line.indexOf(":");
    if (colonIdx < 0) return { heading: line.trim(), body: "", level: 1 };
    return {
...
function parseTableData2(raw) {
  ...
  if (raw && typeof raw === "object") {
    ...
    const rows = Array.isArray(o.rows) ? o.rows.map(
      (r) => Array.isArray(r) ? r.map(String) : [String(r)]
    ) : [];
...
function normalizeRawToDocumentData(template, raw, docTitle) {
  if (!raw || typeof raw !== "object") {
  ...
  if (template.renderer === "excel" || o.columns) {
    return {
      ...
      sheet: {
        ...
        rows: parseRows2(o.rows),
        sheet_name: o.sheet_name ? String(o.sheet_name) : void 0
      }
    };
  }
  if (template.renderer === "powerpoint" || o.slides) {
    return {
      ...
      meta: { presenter: o.presenter ? String(o.presenter) : void 0 },
      slides: Array.isArray(o.slides) ? o.slides : []
    };
  }
  return {
    title: String(o.title ?? docTitle),
    meta: { author: o.author ? String(o.author) : void 0 },
    sections: Array.isArray(o.sections) ? o.sections : typeof o.sections === "string" ? parseSectionsOutline(o.sections) : [],
    table: parseTableData2(o.table_data ?? o.table)
  };
}
function validateDocumentData(template, data) {
...
// skills/documents/actions/template-core/map-to-tool-input.ts
function mapToSpreadsheetInput(template, data, outputFilename) {
  ...
  return {
    ...
    chart_column: chart?.column ?? (data.sheet?.columns?.[1] ?? data.sheet?.columns?.[0] ?? "Revenue")
  };
}
function mapToPresentationInput(template, data, outputFilename) {
  const style = template.style ?? {};
...
function mapToWordDocInput(template, data, outputFilename) {
  const style = template.style ?? {};
  return {
    output_filename: outputFilename,
    title: data.title,
    ...
    table_data: data.table ?? void 0
  };
}
function mapToHtmlRenderContext(template, data) {
  return {
    title: data.title,
    meta: data.meta ?? {},
    sections: data.sections ?? [],
    table: data.table ?? void 0,
    style: template.style ?? {}
  };
}

// skills/documents/actions/render-document.ts
var DEFAULT_DATA_PATH = "output/toolLoop/step-2b/results/data.json";
function slugify4(name) {
  return name.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "") || "document";
}
function readDataJson(sandboxRoot, dataPath) {
  const abs = import_path15.default.isAbsolute(dataPath) ? dataPath : import_path15.default.join(sandboxRoot, dataPath);
  if (!import_fs15.default.existsSync(abs)) {
    throw new Error(`data file not found: ${dataPath}`);
  }
  const raw = JSON.parse(import_fs15.default.readFileSync(abs, "utf-8"));
  return raw;
}
function loadHtmlTemplate(relativePath) {
  const { content } = readSkillAttachment(DOCUMENTS_SKILL_ID, `templates/${relativePath}`);
  return content;
}
async function renderHtmlBranch(sandboxRoot, template, data, outputFilename, outputFormat) {
  const resultsDir = import_path15.default.join(sandboxRoot, getOutputResultsRelPrefix());
  import_fs15.default.mkdirSync(resultsDir, { recursive: true });
  const slug = slugify4(outputFilename);
  const htmlPath = import_path15.default.join(resultsDir, `${slug}.html`);
  ...
  const html = import_mustache.default.render(templateSrc, context);
  import_fs15.default.writeFileSync(htmlPath, html, "utf-8");
  ...
  const pdfPath = import_path15.default.join(resultsDir, `${slug}.pdf`);
...
var renderDocument = {
  ...
  description: "Render a document from a template manifest entry and normalized data.json. Supports PDF/HTML reports, Excel, PowerPoint, and Word. Preferred step-3 entry point for the documents skill.",
  async execute(input) {
    const sandbox = requireActiveSandbox();
    if (!sandbox.ok) return { error: sandbox.message };
    ...
    const style = template.style ?? {};
    const outputFormat = outputFormatRaw === "html" || outputFormatRaw === "pdf" ? outputFormatRaw : style.output_format === "html" ? "html" : "pdf";
    switch (template.renderer) {
      case "html": {
...
//# sourceMappingURL=ffb65d480f5c22ae76c66714b43bf67739153935.js.map
