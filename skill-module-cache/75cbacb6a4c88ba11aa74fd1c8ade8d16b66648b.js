...
function bindSubAgentDelegation(ctx) {
  if (!ctx) return;
  delegationStack().push(ctx);
}
function clearSubAgentDelegation() {
  const stack = delegationStack();
  if (stack.length > 0) stack.pop();
}
function resetSubAgentDelegationStack() {
...
function resolveBundledSkillsDirectory() {
  if (import_electron.app?.isPackaged) {
    return (0, import_path3.join)(import_electron.app.getAppPath(), "skills");
  }
...
function resolveBundledToolSetDirectory() {
  if (import_electron.app?.isPackaged) {
    return (0, import_path3.join)(import_electron.app.getAppPath(), SKILL_FILES.TOOL_SET_DIR);
  }
...
var import_fs4, import_path3, import_electron, RESERVED_SKILL_DIR_NAMES, PROPERTY_KEY_ORDER;
  ...
  if (!view) {
    ...
    if (!view.webContents.isDestroyed()) {
      view.webContents.on("did-fail-load", (_event, errorCode, errorDesc, url) => {
        log7.error("Sandbox output view failed to load", {
          errorCode,
          errorDesc,
          url
        });
...
var import_electron2, sandboxResultsViews, closedHandlersRegistered, log7, syncSandboxOutputView;
var init_output_view = __esm({
  "src/main/agent/sandbox/output-view.ts"() {
    ...
    log7 = createLogger("sandbox.output-view");
...
function realpathSafe(p) {
  const r = (0, import_path8.resolve)(p.trim());
  try {
    return (0, import_fs8.realpathSync)(r);
  } catch {
    return r;
  }
}
function isRemovableopenfdeSandboxPathImpl(candidatePath) {
  ...
  let name;
  try {
    const absReal = (0, import_fs8.realpathSync)((0, import_path8.resolve)(candidatePath.trim()));
    parentReal = (0, import_path8.dirname)(absReal);
    name = (0, import_path8.basename)(absReal);
  } catch {
    const abs = (0, import_path8.resolve)(candidatePath.trim());
    name = (0, import_path8.basename)(abs);
    parentReal = realpathSafe((0, import_path8.dirname)(abs));
  }
  if (parentReal === openfdeSandboxReal) return true;
  try {
    const absReal = (0, import_fs8.realpathSync)((0, import_path8.resolve)(candidatePath.trim()));
    const rel = (0, import_path8.relative)(openfdeSandboxReal, absReal);
    if (rel && !rel.startsWith("..") && !rel.startsWith(`..${import_path8.sep}`) && (rel.startsWith("sub-agents") || rel.includes("/sub-agents/"))) {
      return true;
    }
  } catch {
...
async function removeSandboxDirectoriesImpl(paths) {
  for (const p of paths) {
    const abs = (0, import_path8.resolve)(p.trim());
    if (!isRemovableopenfdeSandboxPath(abs)) {
      log8.warn("Skipped sandbox removal outside allowed sandbox roots", {
        path: p,
    ...
    try {
      ...
      log8.info("Removed sandbox directory", { path: abs });
    } catch (err) {
      log8.error("Failed to remove sandbox directory", {
        path: abs,
        err
      });
    }
...
var import_fs8, import_promises, import_os2, import_path8, log8, isRemovableopenfdeSandboxPath, removeSandboxDirectories;
var init_cleanup = __esm({
  "src/main/agent/sandbox/cleanup.ts"() {
    ...
    init_logger();
    log8 = createLogger("sandbox.cleanup");
...
var init_sandbox_globals_lock = __esm({
  "src/main/agent/sandbox/sandbox-globals-lock.ts"() {
    ...
    init_paths();
    init_run_context();
    log9 = createLogger("sandbox.globals-lock");
...
var log10, PlanModeStateMachine;
var init_plan_mode_state_machine = __esm({
  "src/main/agent/coding/plan-mode-state-machine.ts"() {
    ...
    log10 = createLogger("agent.plan-mode.state");
    PlanModeStateMachine = class _PlanModeStateMachine {
      ...
      consumeEnterReminder(meta = defaultMeta("injection:consumeEnterReminder")) {
        ...
        if (consumed) {
          log10.debug("Consumed enter plan reminder", {
      ...
      consumeExecuteReminder(meta = defaultMeta("injection:consumeExecuteReminder")) {
        ...
        if (consumed) {
          log10.debug("Consumed execute plan reminder", {
            conversationId: this.conversationId,
            trigger: meta.trigger
          });
        }
      ...
      logUpdate(from, fromStatus, toStatus, meta) {
        ...
        if (statusChanged) {
          log10.info("Explore mode status transition", payload);
        } else {
          log10.info("Explore mode state updated", payload);
        }
      }
    };
  }
...
var log11, READ_FILE_PRUNE_PREVIEW_CHARS;
var init_context_overflow_guard = __esm({
  "src/main/agent/expr/context-overflow-guard.ts"() {
    init_logger();
    init_thread_tagger();
    log11 = createLogger("agent.expr.context-overflow-guard");
...
var import_fs9, import_path9, log12;
...
function serializeToolInputSchema(schema) {
  ...
  if (typeof import_zod3.z.toJSONSchema === "function")
    return import_zod3.z.toJSONSchema(schema);
...
var log13, getSkillsDir, parseSkillMarkdown2, skillToAgent2;
var init_skills = __esm({
  "src/main/skills/skills.ts"() {
    ...
    skillToAgent2 = traceFunction2(log13, "skillToAgent", skillToAgent);
  }
});
...
async function loadEngineAgentsFromDisk(userId) {
  ...
  const skillAgents = skills.map(skillToAgent2);
...
function getMcpServerManager() {
  if (!_manager) {
    _manager = instrumentInstanceMethods2(new McpServerManager(), log14);
  }
  return _manager;
}
var log14, McpServerManager, _manager;
var init_mcp_server_manager = __esm({
  "src/main/services/mcp-server-manager.ts"() {
    init_mcp();
    init_logger();
    init_mcp_server_runtime();
    log14 = createLogger("services.mcp-server-manager");
...
var init_thread_context_builder = __esm({
  "src/main/agent/expr/thread-context-builder.ts"() {
    ...
    init_conversation_store();
    init_thread_tagger();
    log15 = createLogger("agent.expr.thread-context-builder");
  }
});
...
var init_llm_debug_writer = __esm({
  "src/main/agent/llm/llm-debug-writer.ts"() {
    ...
    log16 = createLogger("agent.llm.debug");
...
var log17, ProviderAdapter, OllamaAdapter, LlamaCppAdapter, OpenAIAdapter, AnthropicAdapter, GeminiAdapter, DeepSeekAdapter, ZhipuAdapter, MoonshotAdapter, QwenAdapter, HuggingFaceAdapter, OpenAiCompatibleProviderAdapter, PROVIDER_ADAPTERS;
var init_adapters = __esm({
  "src/main/agent/providers/adapters.ts"() {
    init_openfde_ai();
    init_logger();
    log17 = createLogger("agent.providers.adapters");
    ...
    PROVIDER_ADAPTERS = {
      ollama: instrumentInstanceMethods2(new OllamaAdapter(), log17.child({ provider: "ollama" })),
      llamacpp: instrumentInstanceMethods2(
        new LlamaCppAdapter(),
        log17.child({ provider: "llamacpp" })
      ),
      openai: instrumentInstanceMethods2(new OpenAIAdapter(), log17.child({ provider: "openai" })),
      anthropic: instrumentInstanceMethods2(
        new AnthropicAdapter(),
        log17.child({ provider: "anthropic" })
      ),
      gemini: instrumentInstanceMethods2(new GeminiAdapter(), log17.child({ provider: "gemini" })),
      deepseek: instrumentInstanceMethods2(
        new DeepSeekAdapter(),
        log17.child({ provider: "deepseek" })
      ),
      zhipu: instrumentInstanceMethods2(new ZhipuAdapter(), log17.child({ provider: "zhipu" })),
      moonshot: instrumentInstanceMethods2(new MoonshotAdapter(), log17.child({ provider: "moonshot" })),
      qwen: instrumentInstanceMethods2(new QwenAdapter(), log17.child({ provider: "qwen" })),
      bytedance: openAiCompatibleAdapter("bytedance", "bytedance"),
      huggingface: instrumentInstanceMethods2(
        new HuggingFaceAdapter(),
        log17.child({ provider: "huggingface" })
      ),
  ...
  "toolSet/sub-agents/invoke-agent.ts"() {
    ...
    invokeAgent = {
      ...
      inputSchema: import_zod4.z.object({
        agentId: import_zod4.z.string().min(1).describe("Catalog agent id to run"),
        task: import_zod4.z.string().min(1).describe("Task for the sub-agent"),
        wait: import_zod4.z.boolean().optional().describe("When false, start in background and return runId immediately")
      }),
      needsApproval: false,
      async execute(input) {
        const parsed = import_zod4.z.object({
          ...
          wait: import_zod4.z.boolean().optional()
        }).safeParse(input);
        if (!parsed.success) {
...
var import_zod5, invokeAgentsSchema, invokeAgents;
var init_invoke_agents = __esm({
  "toolSet/sub-agents/invoke-agents.ts"() {
    import_zod5 = require("zod");
    init_delegation_context();
    init_constants();
    invokeAgentsSchema = import_zod5.z.object({
      runs: import_zod5.z.array(
        import_zod5.z.object({
          agentId: import_zod5.z.string().min(1),
          task: import_zod5.z.string().min(1)
        })
      ).min(1),
      wait: import_zod5.z.boolean().optional().describe("When false, return runIds immediately without blocking")
    });
    invokeAgents = {
...
var import_zod6, waitForSubAgentRunsTool;
  ...
  "toolSet/sub-agents/wait-for-sub-agent-runs.ts"() {
    ...
    waitForSubAgentRunsTool = {
      ...
      inputSchema: import_zod6.z.object({
        runIds: import_zod6.z.array(import_zod6.z.string().min(1)).min(1)
      }),
      needsApproval: false,
      async execute(input) {
        const parsed = import_zod6.z.object({ runIds: import_zod6.z.array(import_zod6.z.string()) }).safeParse(input);
        if (!parsed.success) {
          return {
...
// toolSet/sub-agent-tools.ts
var sub_agent_tools_exports = {};
__export(sub_agent_tools_exports, {
  INVOKE_AGENTS_TOOL_NAME: () => INVOKE_AGENTS_TOOL_NAME,
  INVOKE_AGENT_TOOL_NAME: () => INVOKE_AGENT_TOOL_NAME,
  SUB_AGENT_TAG: () => SUB_AGENT_TAG,
  SUB_AGENT_TOOL_NAMES: () => SUB_AGENT_TOOL_NAMES,
  UNIVERSAL_SUB_AGENT_TOOL_NAMES: () => UNIVERSAL_SUB_AGENT_TOOL_NAMES,
  WAIT_FOR_SUB_AGENT_RUNS_TOOL_NAME: () => WAIT_FOR_SUB_AGENT_RUNS_TOOL_NAME,
  bindSubAgentDelegation: () => bindSubAgentDelegation,
  buildSubAgentChildParams: () => buildSubAgentChildParams,
  clearSubAgentDelegation: () => clearSubAgentDelegation,
  getSubAgentDelegation: () => getSubAgentDelegation,
  invokeAgent: () => invokeAgent,
  invokeAgents: () => invokeAgents,
  isSubAgentIdAllowed: () => isSubAgentIdAllowed,
  requireSubAgentDelegation: () => requireSubAgentDelegation,
  resetSubAgentDelegationStack: () => resetSubAgentDelegationStack,
  subAgentTools: () => subAgentTools,
  waitForSubAgentRunsTool: () => waitForSubAgentRunsTool
});
module.exports = __toCommonJS(sub_agent_tools_exports);
init_sub_agents();
...
//# sourceMappingURL=75cbacb6a4c88ba11aa74fd1c8ade8d16b66648b.js.map
