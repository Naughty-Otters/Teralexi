...
var PLANNING_TAG, ENTER_PLAN_MODE_TOOL_NAME, EXIT_PLAN_MODE_TOOL_NAME, PLAN_MODE_TOOL_NAMES, PLAN_FILE_WRITE_TOOLS, PLAN_MODE_FILE_IO_TOOLS, PLAN_MODE_RESEARCH_TOOLS, PLAN_MODE_GIT_READ_TOOLS, PLAN_MODE_ALWAYS_IN_CATALOG_TOOL_NAMES;
var init_constants = __esm({
  "toolSet/planning/constants.ts"() {
    ...
    PLAN_FILE_WRITE_TOOLS = /* @__PURE__ */ new Set([
      "write_file",
      "edit_file",
      "apply_patch"
    ]);
...
var log2, DEFAULT_RESPONSE_LANGUAGE, DEFAULT_USER_ID, ANTHROPIC_MODELS, SYSTEM_PROP_KEYS, buildToolPromptDescription, withResponseLanguageInstruction, normalizeBaseURL, todoStatusIcon;
var init_config = __esm({
  "src/main/agent/config/config.ts"() {
    init_logger();
    init_constants4();
    init_llm_constants2();
    log2 = createLogger("agent.config");
...
var import_fs6, import_path5, ConversationStore;
  ...
  "src/main/services/conversation-store/store.ts"() {
    ...
    ConversationStore = class {
      ...
      saveDataToFile(userId, relativePath, data) {
        ...
        const workspaceRoot = (0, import_path5.resolve)(workspacePath);
        const targetPath = (0, import_path5.resolve)(workspaceRoot, cleanRelativePath);
        const workspacePrefix = workspaceRoot.endsWith(import_path5.sep) ? workspaceRoot : `${workspaceRoot}${import_path5.sep}`;
        ...
        (0, import_fs6.mkdirSync)((0, import_path5.dirname)(targetPath), { recursive: true });
...
var import_node_fs2, FILESYSTEM_PREVIEW_CONVERSATION_ID;
...
function getMcpServerManager() {
  if (!_manager) {
    _manager = instrumentInstanceMethods2(new McpServerManager(), log5);
  }
  return _manager;
}
var log5, McpServerManager, _manager;
var init_mcp_server_manager = __esm({
  "src/main/services/mcp-server-manager.ts"() {
    init_mcp();
    init_logger();
    init_mcp_server_runtime();
    log5 = createLogger("services.mcp-server-manager");
...
var init_thread_context_builder = __esm({
  "src/main/agent/expr/thread-context-builder.ts"() {
    ...
    init_conversation_store();
    init_thread_tagger();
    log6 = createLogger("agent.expr.thread-context-builder");
  }
});
...
var init_llm_debug_writer = __esm({
  "src/main/agent/llm/llm-debug-writer.ts"() {
    ...
    log7 = createLogger("agent.llm.debug");
...
var log8, ProviderAdapter, OllamaAdapter, LlamaCppAdapter, OpenAIAdapter, AnthropicAdapter, GeminiAdapter, DeepSeekAdapter, ZhipuAdapter, MoonshotAdapter, QwenAdapter, HuggingFaceAdapter, OpenAiCompatibleProviderAdapter, PROVIDER_ADAPTERS;
var init_adapters = __esm({
  "src/main/agent/providers/adapters.ts"() {
    init_openfde_ai();
    init_logger();
    log8 = createLogger("agent.providers.adapters");
    ...
    PROVIDER_ADAPTERS = {
      ollama: instrumentInstanceMethods2(new OllamaAdapter(), log8.child({ provider: "ollama" })),
      llamacpp: instrumentInstanceMethods2(
        new LlamaCppAdapter(),
        log8.child({ provider: "llamacpp" })
      ),
      openai: instrumentInstanceMethods2(new OpenAIAdapter(), log8.child({ provider: "openai" })),
      anthropic: instrumentInstanceMethods2(
        new AnthropicAdapter(),
        log8.child({ provider: "anthropic" })
      ),
      gemini: instrumentInstanceMethods2(new GeminiAdapter(), log8.child({ provider: "gemini" })),
      deepseek: instrumentInstanceMethods2(
        new DeepSeekAdapter(),
        log8.child({ provider: "deepseek" })
      ),
      zhipu: instrumentInstanceMethods2(new ZhipuAdapter(), log8.child({ provider: "zhipu" })),
      moonshot: instrumentInstanceMethods2(new MoonshotAdapter(), log8.child({ provider: "moonshot" })),
      qwen: instrumentInstanceMethods2(new QwenAdapter(), log8.child({ provider: "qwen" })),
      bytedance: openAiCompatibleAdapter("bytedance", "bytedance"),
      huggingface: instrumentInstanceMethods2(
        new HuggingFaceAdapter(),
        log8.child({ provider: "huggingface" })
      ),
  ...
  "toolSet/sub-agents/invoke-agent.ts"() {
    ...
    invokeAgent = {
      ...
      inputSchema: import_zod.z.object({
        agentId: import_zod.z.string().min(1).describe("Catalog agent id to run"),
        task: import_zod.z.string().min(1).describe("Task for the sub-agent"),
        wait: import_zod.z.boolean().optional().describe("When false, start in background and return runId immediately")
      }),
      needsApproval: false,
      async execute(input) {
        const parsed = import_zod.z.object({
          ...
          wait: import_zod.z.boolean().optional()
        }).safeParse(input);
        if (!parsed.success) {
...
var import_zod2, invokeAgentsSchema, invokeAgents;
var init_invoke_agents = __esm({
  "toolSet/sub-agents/invoke-agents.ts"() {
    import_zod2 = require("zod");
    init_delegation_context();
    init_constants3();
    invokeAgentsSchema = import_zod2.z.object({
      runs: import_zod2.z.array(
        import_zod2.z.object({
          agentId: import_zod2.z.string().min(1),
          task: import_zod2.z.string().min(1)
        })
      ).min(1),
      wait: import_zod2.z.boolean().optional().describe("When false, return runIds immediately without blocking")
    });
    invokeAgents = {
...
var import_zod3, waitForSubAgentRunsTool;
  ...
  "toolSet/sub-agents/wait-for-sub-agent-runs.ts"() {
    ...
    waitForSubAgentRunsTool = {
      ...
      inputSchema: import_zod3.z.object({
        runIds: import_zod3.z.array(import_zod3.z.string().min(1)).min(1)
      }),
      needsApproval: false,
      async execute(input) {
        const parsed = import_zod3.z.object({ runIds: import_zod3.z.array(import_zod3.z.string()) }).safeParse(input);
        if (!parsed.success) {
          return {
...
async function loadSkillsFromDirectory(skillsDir, options) {
  if (!(0, import_fs7.existsSync)(skillsDir)) {
    (0, import_fs7.mkdirSync)(skillsDir, { recursive: true });
    return [];
  }
  const skills = [];
  let entries;
  try {
    entries = (0, import_fs7.readdirSync)(skillsDir);
  } catch {
    return [];
  }
  ...
  for (const entry of entries) {
    ...
    const skillFolder = (0, import_path6.join)(skillsDir, entry);
    const skillFile = (0, import_path6.join)(skillFolder, SKILL_FILES.SKILL_MD);
    try {
      let skillRaw = (0, import_fs7.readFileSync)(skillFile, "utf-8");
      ...
      log9.warn(SKILL_LOADER_LOG.SKIPPED_INVALID, {
        skillId: entry,
        folder: skillFolder
      });
    } catch (err) {
      log9.warn(SKILL_LOADER_LOG.SKIPPED_FAILED, {
        skillId: entry,
        folder: skillFolder,
        err
      });
  ...
  log9.info(SKILL_LOADER_LOG.LOADED, {
    skillsDir,
    count: skills.length,
...
var import_fs7, import_path6, log9;
var init_skills_directory_loader = __esm({
  "src/main/skills/skills-directory-loader.ts"() {
    ...
    init_skill_workspace_tool_defaults();
    init_conversation_store();
    log9 = createLogger("skills.loader");
  }
});
...
function serializeToolInputSchema(schema) {
  ...
  if (typeof import_zod4.z.toJSONSchema === "function")
...
var log10, getSkillsDir, parseSkillMarkdown2, skillToAgent;
var init_skills = __esm({
  "src/main/skills/skills.ts"() {
    ...
    init_actions();
    init_skill_visibility();
    log10 = createLogger("skills.skills");
    ...
    skillToAgent = traceFunction2(log10, "skillToAgent", skillToAgent2);
  }
});
...
function toolLoopOutputRelBase(scopeOrStepKey) {
  ...
  if (!pathScope) {
    return (0, import_path7.join)("output", TOOL_LOOP_OUTPUT_SEGMENT);
  }
  ...
  return (0, import_path7.join)("output", TOOL_LOOP_OUTPUT_SEGMENT, ...instanceSegments);
}
function getOutputResultsRelPrefix(scope) {
  const active = scope?.trim() || getSandboxOutputScopeFromEnv();
  if (active) {
    return (0, import_path7.join)(toolLoopOutputRelBase(active), "results");
  }
  return (0, import_path7.join)("output", "results");
}
function getOutputScriptsRelPrefix(scope) {
  const active = scope?.trim() || getSandboxOutputScopeFromEnv();
  if (active) {
    return (0, import_path7.join)(toolLoopOutputRelBase(active), "scripts");
  }
  return (0, import_path7.join)("output", "scripts");
}
function remapLegacyPlanRelativePath(userPath) {
  ...
  if (seg.length >= 2 && seg[0] === "output" && seg[1] === "plans") {
    return (0, import_path7.join)("plans", ...seg.slice(2));
  }
  return stripped;
}
function remapLegacySharedOutputPath(userPath) {
  ...
  if (seg.length >= 2 && seg[0] === "output" && seg[1] === "results") {
    return (0, import_path7.join)(getOutputResultsRelPrefix(), ...seg.slice(2));
  }
  if (seg.length >= 2 && seg[0] === "output" && seg[1] === "scripts") {
    return (0, import_path7.join)(getOutputScriptsRelPrefix(), ...seg.slice(2));
  }
  return stripped;
}
var import_path7, OTTER_AGENT_SANDBOX_OUTPUT_SCOPE_ENV, SANDBOX_OUTPUT_SCOPE_GLOBAL_KEY, TOOL_LOOP_OUTPUT_SEGMENT;
...
var init_run_context = __esm({
  "src/main/agent/sandbox/run-context.ts"() {
    ...
    log12 = createLogger("sandbox.run-context");
  ...
  if (!view) {
    ...
    if (!view.webContents.isDestroyed()) {
      view.webContents.on("did-fail-load", (_event, errorCode, errorDesc, url) => {
        log13.error("Sandbox output view failed to load", {
          errorCode,
          errorDesc,
          url
        });
...
var import_electron2, sandboxResultsViews, closedHandlersRegistered, log13, syncSandboxOutputView;
var init_output_view = __esm({
  "src/main/agent/sandbox/output-view.ts"() {
    ...
    log13 = createLogger("sandbox.output-view");
...
async function removeSandboxDirectoriesImpl(paths) {
  for (const p of paths) {
    ...
    if (!isRemovableopenfdeSandboxPath(abs)) {
      log14.warn("Skipped sandbox removal outside allowed sandbox roots", {
        path: p,
    ...
    try {
      ...
      log14.info("Removed sandbox directory", { path: abs });
    } catch (err) {
      log14.error("Failed to remove sandbox directory", {
        path: abs,
        err
      });
    }
...
var import_fs9, import_promises, import_os2, import_path9, log14, isRemovableopenfdeSandboxPath, removeSandboxDirectories;
var init_cleanup = __esm({
  "src/main/agent/sandbox/cleanup.ts"() {
    ...
    init_logger();
    log14 = createLogger("sandbox.cleanup");
...
var init_sandbox_globals_lock = __esm({
  "src/main/agent/sandbox/sandbox-globals-lock.ts"() {
    ...
    init_paths();
    init_run_context();
    log15 = createLogger("sandbox.globals-lock");
...
var log16, PlanModeStateMachine;
var init_plan_mode_state_machine = __esm({
  "src/main/agent/coding/plan-mode-state-machine.ts"() {
    ...
    log16 = createLogger("agent.plan-mode.state");
    PlanModeStateMachine = class _PlanModeStateMachine {
      ...
      consumeEnterReminder(meta = defaultMeta("injection:consumeEnterReminder")) {
        ...
        if (consumed) {
          log16.debug("Consumed enter plan reminder", {
      ...
      consumeExecuteReminder(meta = defaultMeta("injection:consumeExecuteReminder")) {
        ...
        if (consumed) {
          log16.debug("Consumed execute plan reminder", {
            conversationId: this.conversationId,
            trigger: meta.trigger
          });
        }
      ...
      logUpdate(from, fromStatus, toStatus, meta) {
        ...
        if (statusChanged) {
          log16.info("Explore mode status transition", payload);
        } else {
          log16.info("Explore mode state updated", payload);
        }
      }
    };
  }
...
function normalizePlanModeRelativePath(userPath) {
...
function pruneStalePlanMarkdownFiles(plansDirAbs, canonicalPlanAbs) {
  if (!(0, import_node_fs3.existsSync)(plansDirAbs)) return;
  const canonical = (0, import_node_path2.resolve)(canonicalPlanAbs);
  let entries;
  try {
    entries = (0, import_node_fs3.readdirSync)(plansDirAbs);
  } catch {
    return;
  }
  for (const name of entries) {
    ...
    try {
      (0, import_node_fs3.unlinkSync)(abs);
    } catch {
    }
  }
}
function isCanonicalPlanMarkdownPath(storage, userPath) {
  const norm = normalizePlanModeRelativePath(userPath).replace(/^\/+/, "");
  if (norm === storage.planFile.displayPath) return true;
  try {
    return (0, import_node_path2.resolve)(storage.sandboxRoot, norm) === (0, import_node_path2.resolve)(storage.planFile.absolutePath);
  } catch {
    return false;
  }
}
...
function ensurePlanModePlansDir(plansDirAbs) {
  if (!(0, import_node_fs3.existsSync)(plansDirAbs)) {
    (0, import_node_fs3.mkdirSync)(plansDirAbs, { recursive: true });
  }
}
...
function migrateLegacyPlanArtifacts(storage) {
  ...
  if (!(0, import_node_fs3.existsSync)(legacyDir)) return;
  ...
  if (!(0, import_node_fs3.existsSync)(storage.planFile.absolutePath) && (0, import_node_fs3.existsSync)(legacyPlan)) {
    try {
      (0, import_node_fs3.renameSync)(legacyPlan, storage.planFile.absolutePath);
    } catch {
      (0, import_node_fs3.copyFileSync)(legacyPlan, storage.planFile.absolutePath);
    }
  }
  if (!(0, import_node_fs3.existsSync)(storage.todosFile.absolutePath) && (0, import_node_fs3.existsSync)(legacyTodos)) {
    try {
      (0, import_node_fs3.renameSync)(legacyTodos, storage.todosFile.absolutePath);
    } catch {
      (0, import_node_fs3.copyFileSync)(legacyTodos, storage.todosFile.absolutePath);
    }
  }
}
function readLegacySandboxTodoList(sandboxRoot) {
  const legacy = (0, import_node_path2.join)(sandboxRoot, "todos.json");
  if (!(0, import_node_fs3.existsSync)(legacy)) return null;
  try {
    return parseTodoList(JSON.parse((0, import_node_fs3.readFileSync)(legacy, "utf8")));
  } catch {
    return null;
  }
}
function readPlanModeTodoList(conversationId, options) {
  ...
  if ((0, import_node_fs3.existsSync)(file)) {
    try {
      return parseTodoList(JSON.parse((0, import_node_fs3.readFileSync)(file, "utf8")));
    } catch {
...
function syncPlanMarkdownFromTodoList(storage, list) {
  ...
  (0, import_node_fs3.writeFileSync)(storage.planFile.absolutePath, markdown2, "utf8");
}
...
function bootstrapPlanModeStorage(conversationId, title, options) {
  ...
  ensurePlanModePlansDir(storage.plansDirAbs);
  pruneStalePlanMarkdownFiles(storage.plansDirAbs, storage.planFile.absolutePath);
  if (!(0, import_node_fs3.existsSync)(storage.planFile.absolutePath)) {
...
function seedTodosFromPlanMarkdown(conversationId, planPath, options) {
  let content = "";
  try {
    content = (0, import_node_fs3.readFileSync)(planPath, "utf8");
  } catch {
    return { seeded: 0 };
  }
...
var import_node_fs3, import_node_path2, import_node_crypto2, PLAN_MODE_PLANS_DIR;
...
function wrapPlanModeFileToolExecutes(toolSet, conversationId) {
  const id = conversationId?.trim();
  if (!id) return;
...
var log17, READ_FILE_PRUNE_PREVIEW_CHARS;
var init_context_overflow_guard = __esm({
  "src/main/agent/expr/context-overflow-guard.ts"() {
    init_logger();
    init_thread_tagger();
    log17 = createLogger("agent.expr.context-overflow-guard");
...
var import_node_fs4, MAX_MANIFEST_FILE_ENTRIES, MAX_MANIFEST_SEARCH_ENTRIES, MAX_MANIFEST_RESOURCE_ENTRIES, MANIFEST_TOOLS;
  ...
  "toolSet/planning/enter-plan-mode.ts"() {
    ...
    enterPlanMode = {
      ...
      inputSchema: import_zod5.z.object({
        title: import_zod5.z.string().optional().describe("Short title for the plan (used in plan filename slug).")
      }),
      needsApproval: true,
      async execute(input) {
        const conversationId = getConversationIdFromEnv();
        ...
        const parsed = import_zod5.z.object({ title: import_zod5.z.string().optional() }).safeParse(input);
...
var import_node_fs5, import_zod6, exitPlanMode;
  ...
  "toolSet/planning/exit-plan-mode.ts"() {
    ...
    exitPlanMode = {
      ...
      inputSchema: import_zod6.z.object({
        summary: import_zod6.z.string().optional().describe("One-paragraph summary of the plan for the approval prompt.")
      }),
      needsApproval: true,
      async execute(input) {
        const conversationId = getConversationIdFromEnv();
        ...
        const parsed = import_zod6.z.object({ summary: import_zod6.z.string().optional() }).safeParse(input);
        ...
        const planExists = storage != null && (0, import_node_fs5.existsSync)(storage.planFile.absolutePath);
        let planContent = "";
        if (planExists && storage) {
          try {
            planContent = (0, import_node_fs5.readFileSync)(storage.planFile.absolutePath, "utf8");
          } catch {
            planContent = "";
          }
        }
...
var init_planning = __esm({
  "toolSet/planning/index.ts"() {
    ...
    init_enter_plan_mode();
    init_exit_plan_mode();
    planningTools = [enterPlanMode, exitPlanMode];
  }
});

// toolSet/planning-tools.ts
var planning_tools_exports = {};
__export(planning_tools_exports, {
  ...
  PLAN_FILE_WRITE_TOOLS: () => PLAN_FILE_WRITE_TOOLS,
  PLAN_MODE_TOOL_NAMES: () => PLAN_MODE_TOOL_NAMES,
  enterPlanMode: () => enterPlanMode,
  exitPlanMode: () => exitPlanMode,
  parsePlanStepsFromMarkdown: () => parsePlanStepsFromMarkdown,
  planningTools: () => planningTools,
  seedTodosFromPlan: () => seedTodosFromPlan,
  wrapPlanModeFileToolExecutes: () => wrapPlanModeFileToolExecutes
});
module.exports = __toCommonJS(planning_tools_exports);
init_planning();
...
//# sourceMappingURL=2061daa4628aa530f79fc83f54e54c6e9ee131e0.js.map
