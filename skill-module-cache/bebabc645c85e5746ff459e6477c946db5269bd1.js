...
function getopenfdeDbPath() {
  const path3 = (0, import_path.join)(getopenfdeDbDir(), openfde_DB_FILENAME);
  ensureParentDirForFile(path3);
  return path3;
}
...
function ensureDir(path3) {
  (0, import_fs2.mkdirSync)(redirectLegacyChannelDataPath(path3), { recursive: true });
}
...
function resolvePath(ctx, path3) {
...
function normalizeToolInputForDedupeKey(toolName, input, ctx) {
  ...
  switch (toolName) {
    case "read_file": {
      const path3 = typeof raw.path === "string" ? raw.path : "";
      return {
        path: normalizeToolPathForKey(path3, ctx),
...
function loadConversationWorkspace(conversationId) {
  ...
  const path3 = row?.workspacePath?.trim() || null;
  cache.set(id, path3);
  return path3;
}
...
function readFileCacheKey(path3, offset, limit) {
  return `${path3}\0${offset ?? 1}\0${limit ?? ""}`;
}
...
function isSubAgentSandboxRoot(path3) {
...
function stripTerminalCaptureHeaders(text) {
...
function stampCommandToolResult(result) {
  const stdout = typeof result.stdout === "string" ? result.stdout : "";
  const stderr = typeof result.stderr === "string" ? result.stderr : "";
  const output = formatCommandOutput(stdout, stderr);
  const existing = typeof result.resultContent === "string" ? stripTerminalCaptureHeaders(result.resultContent) : "";
...
// src/main/agent/workspace/git-service.ts
...
async function ghCreatePr(cwd, options) {
  ...
  try {
    ...
    const url = stdout.trim().split("\n").reverse().find((l) => l.startsWith("http")) ?? stdout.trim();
    return { ok: true, url, error: stderr.trim() || void 0 };
  } catch (err) {
    const e = err;
    return {
      ok: false,
...
var MAX_WORKSPACE_EDITOR_BYTES = 2 * 1024 * 1024;
var WORKSPACE_TERMINAL_MAX_BUFFER = 10 * 1024 * 1024;
...
init_sandbox_paths();
function resolveGitWorkingDirectory(baseRoot, workingDirectory) {
  const raw = workingDirectory?.trim() || ".";
  try {
    const cwd = import_path10.default.isAbsolute(raw) || /^[A-Za-z]:[\\/]/.test(raw) ? resolvePathAllowingOutside(baseRoot, raw) : resolvePathMustBeInside(baseRoot, raw);
    return { ok: true, cwd };
  } catch (err) {
    return { ok: false, error: err instanceof Error ? err.message : String(err) };
  }
}
function resolveActiveGitCwd(workingDirectory) {
  const sandboxRoot = getSandboxRootFromEnv();
  const workspacePath = getWorkspacePathFromEnv();
  const wd = workingDirectory?.trim();
  if (wd) {
    const base = sandboxRoot ?? workspacePath ?? void 0;
    if (!base) {
      return {
        ok: false,
        error: "workingDirectory was provided but no active sandbox or workspace folder is set for this conversation."
      };
    }
  ...
  return {
    ok: false,
    error: "No workspace folder or active sandbox is set for this conversation. Select a workspace folder, or run inside an agent sandbox, before using git tools."
  };
}
...
var MAX_BUFFER = 1024 * 1024 * 50;
var workingDirectoryField = import_zod7.z.string().optional().describe(
  ...
  try {
    ...
    if (execErr.code === "ENOENT") {
      return {
        ...
        command: formatGitCommand(args),
        args,
        error: "git executable not found on PATH. Install Git and ensure it is available to the app."
      };
    }
    ...
    return stampCommandToolResult({
      ...
      command: formatGitCommand(args),
      args,
      error: execErr.killed ? "git command timed out" : void 0
    });
  }
}
function defineGitTool(config) {
  const tool = {
    name: config.name,
    tags: ["git"],
    description: config.description,
    ...
    async execute(input) {
      ...
      const timeoutMs = typeof config.timeoutMs === "function" ? config.timeoutMs(parsed.data) : config.timeoutMs;
      const data = parsed.data;
      const workingDirectory = config.workingDirectory ? config.workingDirectory(parsed.data) : data.workingDirectory;
...
var cwdOnly = import_zod7.z.object({ workingDirectory: workingDirectoryField });
var gitStatus = defineGitTool({
  name: "git_status",
  description: "Show working tree status (porcelain v2 with branch info). Read-only.",
  inputSchema: cwdOnly.extend({
    includeUntracked: import_zod7.z.boolean().optional().default(true)
  }),
  buildArgs: (input) => {
...
var gitDiff = defineGitTool({
  name: "git_diff",
  description: "Show changes. Use staged=true for index vs HEAD, paths to limit files. Read-only.",
  inputSchema: cwdOnly.extend({
    staged: import_zod7.z.boolean().optional().default(false),
    stat: import_zod7.z.boolean().optional().default(false),
    paths: import_zod7.z.array(import_zod7.z.string().min(1)).optional().default([])
  }),
  buildArgs: (input) => {
    const args = ["diff"];
...
var gitLog = defineGitTool({
  name: "git_log",
  description: "Show commit history. Read-only. Supports oneline format and optional path limits.",
  inputSchema: cwdOnly.extend({
    maxCount: import_zod7.z.number().int().min(1).max(500).optional().default(30),
    oneline: import_zod7.z.boolean().optional().default(true),
    ref: import_zod7.z.string().optional().describe("Start at this ref (e.g. branch, HEAD~3)."),
    paths: import_zod7.z.array(import_zod7.z.string().min(1)).optional().default([])
  }),
  buildArgs: (input) => {
...
var gitShow = defineGitTool({
  name: "git_show",
  description: "Show a commit or object (patch included). Read-only.",
  inputSchema: cwdOnly.extend({
    ref: import_zod7.z.string().optional().default("HEAD").describe("Commit, tag, or object to show."),
    stat: import_zod7.z.boolean().optional().default(false)
  }),
  buildArgs: (input) => {
...
var gitAdd = defineGitTool({
  name: "git_add",
  description: 'Stage paths for commit. Use all=true to stage everything, or paths such as ["."] for specific files.',
  inputSchema: cwdOnly.extend({
    paths: import_zod7.z.array(import_zod7.z.string().min(1)).optional().default([]),
    all: import_zod7.z.boolean().optional().default(false),
    update: import_zod7.z.boolean().optional().default(false)
  }),
  buildArgs: (input) => {
    const args = ["add"];
...
var gitReset = defineGitTool({
  name: "git_reset",
  description: "Unstage or reset HEAD. mode soft|mixed|hard (hard rewrites tree \u2014 requires approval).",
  inputSchema: cwdOnly.extend({
    mode: import_zod7.z.enum(["soft", "mixed", "hard"]).optional(),
    ref: import_zod7.z.string().optional().describe("Commit to reset to (default HEAD)."),
    paths: import_zod7.z.array(import_zod7.z.string().min(1)).optional().default([])
  }),
...
var gitCommit = defineGitTool({
  name: "git_commit",
  description: "Create a commit with the given message. Requires approval.",
  inputSchema: cwdOnly.extend({
    message: import_zod7.z.string().min(1),
    amend: import_zod7.z.boolean().optional().default(false),
    allowEmpty: import_zod7.z.boolean().optional().default(false),
    all: import_zod7.z.boolean().optional().default(false).describe("Stage all tracked changes (-a).")
  }),
  needsApproval: true,
  buildArgs: (input) => {
...
var gitBranch = defineGitTool({
  name: "git_branch",
  description: "List, create, delete, or rename branches. delete is destructive and requires approval.",
  inputSchema: cwdOnly.extend({
    action: import_zod7.z.enum(["list", "create", "delete", "rename", "show-current"]),
    name: import_zod7.z.string().optional(),
    newName: import_zod7.z.string().optional(),
    force: import_zod7.z.boolean().optional().default(false),
    remote: import_zod7.z.boolean().optional().default(false).describe("Include remote-tracking branches when listing.")
  }),
...
var gitCheckout = defineGitTool({
  name: "git_checkout",
  description: "Switch branches or restore paths. Branch checkout may require approval when creating a new branch.",
  inputSchema: cwdOnly.extend({
    branch: import_zod7.z.string().optional(),
    create: import_zod7.z.boolean().optional().default(false),
    paths: import_zod7.z.array(import_zod7.z.string().min(1)).optional().default([])
  }),
  needsApproval: false,
  buildArgs: (input) => {
...
var gitMerge = defineGitTool({
  name: "git_merge",
  description: "Merge a branch into the current branch. Requires approval.",
  inputSchema: cwdOnly.extend({
    branch: import_zod7.z.string().min(1),
    noFf: import_zod7.z.boolean().optional().default(false),
...
var gitRebase = defineGitTool({
  name: "git_rebase",
  description: "Rebase current branch onto another ref. Requires approval.",
  inputSchema: cwdOnly.extend({
    onto: import_zod7.z.string().min(1),
    abort: import_zod7.z.boolean().optional().default(false),
    continue: import_zod7.z.boolean().optional().default(false)
  }),
  needsApproval: true,
  buildArgs: (input) => {
...
var gitCherryPick = defineGitTool({
  name: "git_cherry_pick",
  description: "Apply an existing commit on the current branch. Requires approval.",
  inputSchema: cwdOnly.extend({
    commit: import_zod7.z.string().min(1),
    noCommit: import_zod7.z.boolean().optional().default(false)
  }),
  needsApproval: true,
  buildArgs: (input) => {
...
var gitStash = defineGitTool({
  name: "git_stash",
  description: "Stash working tree changes: list, push, pop, apply, drop, or show.",
  inputSchema: cwdOnly.extend({
    action: import_zod7.z.enum(["list", "push", "pop", "apply", "drop", "show"]),
    message: import_zod7.z.string().optional(),
    index: import_zod7.z.number().int().min(0).optional()
  }),
  needsApproval: false,
  buildArgs: (input) => {
    const ref = input.index != null ? `stash@{${input.index}}` : void 0;
    switch (input.action) {
      case "list":
        return ["stash", "list"];
      case "push": {
...
var gitPull = defineGitTool({
  name: "git_pull",
  description: "Fetch and integrate from a remote. Requires approval.",
  inputSchema: cwdOnly.extend({
    remote: import_zod7.z.string().optional().default("origin"),
    branch: import_zod7.z.string().optional(),
    rebase: import_zod7.z.boolean().optional().default(false)
  }),
  needsApproval: true,
  buildArgs: (input) => {
    const args = ["pull"];
...
var gitPush = defineGitTool({
  name: "git_push",
  description: "Push commits to a remote. force=true requires approval and uses --force-with-lease when possible.",
  inputSchema: cwdOnly.extend({
    remote: import_zod7.z.string().optional().default("origin"),
    branch: import_zod7.z.string().optional(),
    force: import_zod7.z.boolean().optional().default(false),
    setUpstream: import_zod7.z.boolean().optional().default(false)
  }),
  needsApproval: true,
  buildArgs: (input) => {
    const args = ["push"];
...
var gitFetch = defineGitTool({
  name: "git_fetch",
  description: "Download objects from a remote without merging. Read-only for local branches.",
  inputSchema: cwdOnly.extend({
    remote: import_zod7.z.string().optional(),
    prune: import_zod7.z.boolean().optional().default(false),
    all: import_zod7.z.boolean().optional().default(false)
  }),
  buildArgs: (input) => {
    const args = ["fetch"];
...
var gitClone = defineGitTool({
  name: "git_clone",
  description: "Clone a repository into the sandbox (or a subdirectory). Requires approval. Network access needed.",
  inputSchema: import_zod7.z.object({
    url: import_zod7.z.string().url(),
    directory: import_zod7.z.string().optional().describe("Sandbox-relative destination directory (default: repo name from URL)."),
    branch: import_zod7.z.string().optional(),
    depth: import_zod7.z.number().int().min(1).optional()
  }),
  needsApproval: true,
...
var gitRemote = defineGitTool({
  name: "git_remote",
  description: "List or manage remotes (add, remove, rename, get-url, set-url).",
  inputSchema: cwdOnly.extend({
    action: import_zod7.z.enum(["list", "add", "remove", "rename", "get-url", "set-url"]),
    name: import_zod7.z.string().optional(),
...
var gitTag = defineGitTool({
  name: "git_tag",
  description: "List, create, or delete tags. delete requires approval.",
  inputSchema: cwdOnly.extend({
    action: import_zod7.z.enum(["list", "create", "delete"]),
    name: import_zod7.z.string().optional(),
    message: import_zod7.z.string().optional().describe("Annotated tag message."),
    commit: import_zod7.z.string().optional(),
    force: import_zod7.z.boolean().optional().default(false)
  }),
  needsApproval: (input) => input.action === "delete",
  buildArgs: (input) => {
...
var gitClean = defineGitTool({
  name: "git_clean",
  description: "Remove untracked files. dryRun defaults true; force=true deletes files and requires approval.",
  inputSchema: cwdOnly.extend({
    dryRun: import_zod7.z.boolean().optional().default(true),
    force: import_zod7.z.boolean().optional().default(false),
    directories: import_zod7.z.boolean().optional().default(false)
  }),
...
var gitInit = defineGitTool({
  name: "git_init",
  description: "Initialize a new git repository at the working directory.",
  inputSchema: cwdOnly.extend({
    bare: import_zod7.z.boolean().optional().default(false)
  }),
  buildArgs: (input) => {
    const args = ["init"];
...
var gitCreatePrSchema = import_zod7.z.object({
  workingDirectory: workingDirectoryField,
  title: import_zod7.z.string().min(1).describe("PR title (short, under 70 chars)."),
  body: import_zod7.z.string().describe("PR description in markdown. Include a summary and test plan."),
  base: import_zod7.z.string().optional().describe("Base branch (defaults to the repo default branch, usually main/master)."),
  draft: import_zod7.z.boolean().optional().default(false).describe("Open as a draft PR.")
});
var gitCreatePr = {
  name: "git_create_pr",
  tags: ["git"],
  description: "Create a GitHub pull request using the `gh` CLI. Requires `gh` to be installed and authenticated (`gh auth status`). Push the branch first with `git_push`. Returns the PR URL on success.",
...
//# sourceMappingURL=bebabc645c85e5746ff459e6477c946db5269bd1.js.map
