...
function extractYamlFrontmatterBlock(markdown4) {
  const match = markdown4.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?/);
  if (!match?.[1]?.trim()) return null;
  return match[1].trim();
}
function stripYamlFrontmatter(markdown4) {
  return markdown4.replace(/^---\r?\n[\s\S]*?\r?\n---\r?\n?/, "");
}
...
function syncPlanMarkdownFromTodoList(storage, list) {
  ...
  const markdown4 = renderPlanMarkdownFromTodoList(list);
  (0, import_node_fs2.writeFileSync)(storage.planFile.absolutePath, markdown4, "utf8");
}
...
function bootstrapPlanModeStorage(conversationId, title, options) {
  ...
  if (!(0, import_node_fs2.existsSync)(storage.planFile.absolutePath)) {
    const markdown4 = renderPlanModeMarkdown(planContextFromTodoList(emptyTodoList()));
    (0, import_node_fs2.writeFileSync)(storage.planFile.absolutePath, markdown4, "utf8");
  }
  return storage;
}
...
// skills/research/actions/index.ts
var index_exports = {};
__export(index_exports, {
  exportResearchPdf: () => exportResearchPdf,
  tools: () => tools
});
...
// src/main/agent/sandbox/markdown-to-pdf.ts
...
// src/main/agent/sandbox/research-report-html.ts
var import_markdown_it2 = __toESM(require("markdown-it"));

// src/main/agent/sandbox/pdf-print-styles.ts
var PDF_SERIF_FONT_STACK = [
  "PdfSerif",
  "Noto Serif",
  "Times New Roman",
  "Times",
...
function pdfDocumentFontFaceCss() {
  return `
      @font-face {
...
// src/main/agent/sandbox/research-report-html.ts
var markdown2 = new import_markdown_it2.default({
  html: false,
  breaks: true,
  linkify: true
});
function renderResearchReportHtmlDocument(markdownBody) {
  const bodyHtml = markdown2.render(markdownBody);
...
function buildResearchReportHtmlDocument(bodyHtml) {
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
// src/main/agent/sandbox/result-document-html.ts
var import_markdown_it3 = __toESM(require("markdown-it"));
var markdown3 = new import_markdown_it3.default({
  html: false,
  breaks: true,
  linkify: true
});
function renderMarkdownToHtmlDocument(markdownBody) {
  return buildHtmlDocument(markdown3.render(markdownBody));
}
...
// src/main/agent/sandbox/markdown-to-pdf.ts
async function exportMarkdownBodyToPdf(markdownBody, pdfPath, kind = "default") {
  const html = kind === "research-report" ? renderResearchReportHtmlDocument(markdownBody) : renderMarkdownToHtmlDocument(markdownBody);
  const tmpHtml = `${pdfPath}.render-tmp.html`;
  await (0, import_promises3.writeFile)(tmpHtml, html, "utf8");
  try {
    await exportHtmlFileToPdf(tmpHtml, pdfPath);
  } finally {
    await (0, import_promises3.unlink)(tmpHtml).catch(() => void 0);
  }
}

// skills/research/actions/export-research-pdf.ts
var pathField = (label) => import_zod7.z.preprocess((val) => {
  ...
  return val;
}, import_zod7.z.string().min(1, `${label} is required`));
var inputSchema = import_zod7.z.object({
  ...
  pdf_path: pathField("pdf_path").optional().describe(
    "Sandbox-relative PDF output path. Defaults to the markdown path with a .pdf extension."
  )
});
var exportResearchPdf = {
  name: "export_research_pdf",
  description: "Export a research paper markdown file to a print-ready PDF under output/results/.",
  inputSchema,
  execute: async (raw) => {
    ...
    if (!import_fs10.default.existsSync(markdownAbs)) {
      throw new Error(`markdown file not found: ${input.markdown_path}`);
    }
    const markdownBody = import_fs10.default.readFileSync(markdownAbs, "utf-8");
    const pdfRel = input.pdf_path?.trim() || input.markdown_path.replace(/\.md$/i, ".pdf");
    const pdfAbs = resolveSandboxRelativePath(sandboxRoot, pdfRel);
    const resultsDir = import_path10.default.join(sandboxRoot, getOutputResultsRelPrefix());
    import_fs10.default.mkdirSync(resultsDir, { recursive: true });
    import_fs10.default.mkdirSync(import_path10.default.dirname(pdfAbs), { recursive: true });
    await exportMarkdownBodyToPdf(markdownBody, pdfAbs, "research-report");
    return {
      success: true,
      markdown_path: input.markdown_path,
      pdf_path: pdfRel,
      filename: import_path10.default.basename(pdfAbs),
...
// skills/research/actions/index.ts
var tools = [exportResearchPdf];
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  exportResearchPdf,
  tools
});
//# sourceMappingURL=6400a93722a6b3985c84a6a5b610f65014ecf6ad.js.map
