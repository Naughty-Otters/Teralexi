...
function getopenfdeDbPath() {
  const path6 = (0, import_path.join)(getopenfdeDbDir(), openfde_DB_FILENAME);
  ensureParentDirForFile(path6);
  return path6;
}
...
function ensureDir(path6) {
  (0, import_fs2.mkdirSync)(redirectLegacyChannelDataPath(path6), { recursive: true });
}
...
function ensureToolLoopStepOutputDirs(sandboxRoot, scope) {
  const base = (0, import_path5.join)(sandboxRoot, toolLoopOutputRelBase(scope));
  (0, import_fs6.mkdirSync)((0, import_path5.join)(base, "results"), { recursive: true });
  (0, import_fs6.mkdirSync)((0, import_path5.join)(base, "scripts"), { recursive: true });
}
...
var import_fs6, import_path5, OTTER_AGENT_SANDBOX_OUTPUT_SCOPE_ENV, SANDBOX_OUTPUT_SCOPE_GLOBAL_KEY, TOOL_LOOP_OUTPUT_SEGMENT;
...
function resolvePath(ctx, path6) {
  const parts = path6.trim().split(".");
  let cur = ctx;
...
var import_fs7, import_path6, ConversationStore;
  ...
  "src/main/services/conversation-store/store.ts"() {
    ...
    ConversationStore = class {
      ...
      saveDataToFile(userId, relativePath, data) {
        ...
        (0, import_fs7.mkdirSync)((0, import_path6.dirname)(targetPath), { recursive: true });
        (0, import_fs7.writeFileSync)(targetPath, content, "utf-8");
        return targetPath;
      }
      // ── Token usage ──────────────────────────────────────────────────────────
...
function resolvePathForContainment(filePath) {
  ...
  for (; ; ) {
    try {
      const real = (0, import_fs8.realpathSync)(current);
...
function resolveUserProjectPath(workspacePath, userPath) {
  const trimmed = userPath.trim();
  if (!trimmed) {
...
function normalizeToolInputForDedupeKey(toolName, input, ctx) {
  ...
  switch (toolName) {
    case "read_file": {
      const path6 = typeof raw.path === "string" ? raw.path : "";
      return {
        path: normalizeToolPathForKey(path6, ctx),
...
function loadConversationWorkspace(conversationId) {
  ...
  const path6 = row?.workspacePath?.trim() || null;
  cache.set(id, path6);
  return path6;
}
...
function readFileCacheKey(path6, offset, limit) {
  return `${path6}\0${offset ?? 1}\0${limit ?? ""}`;
}
...
async function loadSkillsFromDirectory(skillsDir, options) {
  if (!(0, import_fs9.existsSync)(skillsDir)) {
    (0, import_fs9.mkdirSync)(skillsDir, { recursive: true });
    return [];
  }
  const skills = [];
  let entries;
  try {
    entries = (0, import_fs9.readdirSync)(skillsDir);
  } catch {
    return [];
  }
  ...
  for (const entry of entries) {
    ...
    try {
      let skillRaw = (0, import_fs9.readFileSync)(skillFile, "utf-8");
...
var import_fs9, import_path8, log12;
...
function isSubAgentSandboxRoot(path6) {
...
function realpathSafe(p) {
  ...
  try {
    return (0, import_fs10.realpathSync)(r);
  } catch {
    return r;
  }
}
function isRemovableopenfdeSandboxPathImpl(candidatePath) {
  ...
  let name;
  try {
    const absReal = (0, import_fs10.realpathSync)((0, import_path9.resolve)(candidatePath.trim()));
...
var import_fs10, import_promises, import_os2, import_path9, log16, isRemovableopenfdeSandboxPath, removeSandboxDirectories;
...
async function snapshotFilesUnderDirWithExclusions(dir, options) {
  ...
  const normalizedRoot = import_path10.default.normalize(dir);
  async function walk(current, depth) {
    if (depth > options.maxDepth) return;
    let entries;
    try {
      entries = await import_fs11.promises.readdir(current, { withFileTypes: true });
    } catch {
      return;
    }
    for (const ent of entries) {
      if (ent.name.startsWith(".")) continue;
      const full = import_path10.default.join(current, ent.name);
      if (ent.isDirectory()) {
      ...
      try {
        const st = await import_fs11.promises.stat(full);
        map.set(import_path10.default.normalize(full), {
          mtimeMs: st.mtimeMs,
          size: st.size
        });
      } catch {
...
function filterDeliverableChangedPaths(changedPaths, watchRoot, excludeSubdirs = STEP_NON_DELIVERABLE_SUBDIRS) {
  const blockedRoots = excludeSubdirs.map(
    (name) => import_path10.default.normalize(import_path10.default.join(watchRoot, name))
  );
  return changedPaths.filter((filePath) => {
    const norm = import_path10.default.normalize(filePath);
    for (const blocked of blockedRoots) {
      if (norm === blocked) return false;
      const rel = import_path10.default.relative(blocked, norm);
      if (!rel.startsWith("..") && !import_path10.default.isAbsolute(rel)) return false;
    }
    return true;
  });
}
...
function toPosixRel(sandboxRoot, absPath) {
  return import_path10.default.relative(sandboxRoot, absPath).split(import_path10.default.sep).join("/");
}
function isCaptureLike(filePath, captureAbs) {
  if (captureAbs && import_path10.default.normalize(filePath) === import_path10.default.normalize(captureAbs)) {
    return true;
  }
  const base = import_path10.default.basename(filePath).toLowerCase();
  return base.startsWith("capture-") && base.endsWith(".txt");
}
function sandboxRelativePosix(sandboxRoot, absPath) {
  return import_path10.default.relative(import_path10.default.resolve(sandboxRoot), import_path10.default.resolve(absPath)).split(import_path10.default.sep).join("/");
}
...
function dispositionForArtifact(options) {
  ...
  if (primaryAbs && import_path10.default.normalize(absPath) === import_path10.default.normalize(primaryAbs)) {
    return "deliverable";
  }
...
function pickPrimaryPath(options) {
  ...
  if (declaredPrimaryAbs) {
    try {
      return import_path10.default.normalize(declaredPrimaryAbs);
    } catch {
    }
  }
  const candidates = changedPaths.map((p) => import_path10.default.normalize(p)).filter((p) => !isCaptureLike(p, captureAbs)).filter((p) => !scriptAbs || p !== import_path10.default.normalize(scriptAbs));
...
function buildScriptArtifacts(options) {
  ...
  for (const abs of changedPaths) {
    const normalized = import_path10.default.normalize(abs);
    if (isCaptureLike(normalized, captureAbsolutePath)) continue;
    if (normalized === import_path10.default.normalize(scriptPath)) continue;
  ...
  if (declaredPrimaryAbs && import_path10.default.normalize(declaredPrimaryAbs) !== import_path10.default.normalize(captureAbsolutePath) && !artifacts.some((a) => a.role === "primary")) {
    const normalizedPrimary = import_path10.default.normalize(declaredPrimaryAbs);
    push({
      role: "primary",
...
async function readPrimaryArtifactPreview(artifacts, sandboxRoot) {
  const primary = artifacts.find((a) => a.role === "primary") ?? artifacts.find((a) => a.role === "sidecar" && TEXT_EXTENSIONS.has(import_path10.default.extname(a.path).toLowerCase()));
  if (!primary) return null;
  const abs = import_path10.default.isAbsolute(primary.path) ? primary.path : sandboxRoot ? import_path10.default.join(sandboxRoot, primary.relPath) : primary.path;
  try {
    const buf = await import_fs11.promises.readFile(abs);
...
var import_fs11, import_path10, TEXT_EXTENSIONS, STEP_NON_DELIVERABLE_SUBDIRS, MAX_PRIMARY_PREVIEW_BYTES;
var init_run_script_artifacts = __esm({
  "toolSet/run-script-artifacts.ts"() {
    ...
    STEP_NON_DELIVERABLE_SUBDIRS = ["scripts"];
...
async function snapshotWorkspaceGuard(workspaceRoot, maxDepth = 6) {
  const map = /* @__PURE__ */ new Map();
  const normalizedRoot = import_path11.default.normalize(workspaceRoot);
  async function walk(current, depth) {
    if (depth > maxDepth) return;
    let entries;
    try {
      entries = await import_fs12.promises.readdir(current, { withFileTypes: true });
    } catch {
      return;
    }
    for (const ent of entries) {
      if (ent.name.startsWith(".")) continue;
      const full = import_path11.default.join(current, ent.name);
      if (ent.isDirectory()) {
      ...
      if (!ent.isFile()) continue;
      try {
        const st = await import_fs12.promises.stat(full);
        map.set(import_path11.default.normalize(full), {
          mtimeMs: st.mtimeMs,
          size: st.size
        });
      } catch {
...
function detectWorkspaceWrites(options) {
  ...
  return changed.map((abs) => {
    const rel = import_path11.default.relative(workspaceRoot, abs);
    return rel.split(import_path11.default.sep).join("/");
  });
}
var import_fs12, import_path11, WORKSPACE_SNAPSHOT_SKIP_DIRS, WORKSPACE_WRITE_WARNING;
var init_run_script_workspace_guard = __esm({
  "toolSet/run-script-workspace-guard.ts"() {
    import_fs12 = require("fs");
    import_path11 = __toESM(require("path"));
    init_run_script_artifacts();
    WORKSPACE_SNAPSHOT_SKIP_DIRS = /* @__PURE__ */ new Set([
      "node_modules",
      ".git",
      "dist",
      "build",
    ...
    WORKSPACE_WRITE_WARNING = "Script created or modified files in the user workspace. Write generated outputs under OTTER_RESULTS_DIR, ./results/, or results/scratch/ in the sandbox step folder. Use promote_artifact to copy deliverables into the workspace when intentional.";
  }
});
...
async function runScriptPreflight(options) {
  const issues = [];
  try {
    await import("fs/promises").then((fs4) => fs4.access(options.scriptPath));
  } catch {
    issues.push({
      code: "script_missing",
  ...
  if (resultRel && options.resolveResultFileAbs) {
    ...
    if (!abs) {
      ...
      const parent = import_path12.default.dirname(abs);
      try {
        await import("fs/promises").then(
          (fs4) => fs4.mkdir(parent, { recursive: true })
        );
      } catch (err) {
        issues.push({
          code: "result_path",
...
var import_child_process, import_util, import_path12, execFileAsync, PREFLIGHT_TIMEOUT_MS;
var init_run_script_preflight = __esm({
  "toolSet/run-script-preflight.ts"() {
    import_child_process = require("child_process");
    import_util = require("util");
    import_path12 = __toESM(require("path"));
    execFileAsync = (0, import_util.promisify)(import_child_process.execFile);
    PREFLIGHT_TIMEOUT_MS = 15e3;
  }
});
...
var shell_command_exports = {};
__export(shell_command_exports, {
  buildCreateScriptInstruction: () => buildCreateScriptInstruction,
  buildRunScriptInstruction: () => buildRunScriptInstruction,
  runScript: () => runScript,
  runScriptFile: () => runScriptFile
});
module.exports = __toCommonJS(shell_command_exports);
function normalizeScriptType(t) {
...
function getSandboxOrError() {
  ...
  if (!root) {
    return {
      ok: false,
      message: "No active agent sandbox. Shell execution is only available during an agent run with a sandbox."
    };
  }
  return { ok: true, root: import_path13.default.resolve(root) };
}
function ensureAbsoluteScriptPath(sandboxRoot, scriptPath) {
  const trimmed = scriptPath.trim();
  if (!trimmed) return trimmed;
  const normalized = import_path13.default.normalize(trimmed);
  return import_path13.default.isAbsolute(normalized) ? normalized : import_path13.default.resolve(sandboxRoot, normalized);
}
function scriptSearchRoots(sandboxRoot) {
  const roots = [];
  const push = (dir) => {
    const normalized = import_path13.default.normalize(dir);
    if (!roots.includes(normalized)) roots.push(normalized);
  };
  push(import_path13.default.join(sandboxRoot, getOutputScriptsRelPrefix()));
  push(import_path13.default.join(sandboxRoot, "scripts"));
  return roots;
}
function tryResolveUnderScriptsRoot(scriptsRoot, userPath) {
  try {
    const stripped = userPath.trim().replace(/^[/\\]+/, "");
    const segments = stripped.split(/[/\\]+/).filter(Boolean);
    const relativeSegments = segments[0] === "scripts" ? segments.slice(1) : segments;
    if (relativeSegments.length === 0) return null;
    const resolved = import_path13.default.normalize(
      import_path13.default.resolve(scriptsRoot, ...relativeSegments)
    );
    const relToScripts = import_path13.default.relative(scriptsRoot, resolved);
    if (relToScripts.startsWith("..") || import_path13.default.isAbsolute(relToScripts)) {
      return null;
    }
    return resolved;
  } catch {
...
function collectScriptPathCandidates(root, userPath) {
  const ordered = [];
  const pushRel = (rel, scriptsRoot) => {
  ...
  for (const scriptsRoot of scriptSearchRoots(root)) {
    ...
    if (segments[0] === "scripts" && segments.length >= 2) {
      pushRel(import_path13.default.join(...segments), scriptsRoot);
    }
...
function resolveToolLoopStepBaseDir(sandboxRoot) {
  const scope = getSandboxOutputScopeFromEnv();
  if (!scope) return null;
  ensureToolLoopStepOutputDirs(sandboxRoot, scope);
  return import_path13.default.join(sandboxRoot, toolLoopOutputRelBase(scope));
}
function resolveDeliverableWatchRoot(sandboxRoot) {
  const stepBase = resolveToolLoopStepBaseDir(sandboxRoot);
  if (stepBase) return stepBase;
  return import_path13.default.join(sandboxRoot, getOutputResultsRelPrefix());
}
function resolveScriptProcessCwd(sandboxRoot) {
  const stepBase = resolveToolLoopStepBaseDir(sandboxRoot);
  if (stepBase) {
...
function looksLikeSandboxPathArg(arg) {
  const t = arg.trim();
  if (!t || t.startsWith("-")) return false;
  if (t.includes("/") || t.includes("\\")) return true;
  return /\.(py|sh|js|mjs|cjs|ts|json|md|txt|csv)$/i.test(t);
}
function resolveScriptArgs(sandboxRoot, workspacePath, scriptArgs) {
  return scriptArgs.map((arg) => {
    ...
    try {
      ...
      if ((0, import_fs13.existsSync)(sandboxAbs)) return sandboxAbs;
    } catch {
    }
    if (workspacePath) {
      try {
...
function buildScriptProcessEnv(options) {
  ...
  return {
    ...
    OTTER_STEP_CWD: cwd,
    OTTER_RESULTS_DIR: import_path13.default.join(sandboxRoot, getOutputResultsRelPrefix()),
    OTTER_SCRIPTS_DIR: import_path13.default.join(sandboxRoot, getOutputScriptsRelPrefix()),
    OTTER_REFERENCE_SCRIPTS_DIR: import_path13.default.join(sandboxRoot, "scripts"),
    ...stepBase ? { OTTER_TOOL_LOOP_STEP_DIR: stepBase } : {},
    ...workspacePath ? { OTTER_WORKSPACE_PATH: import_path13.default.resolve(workspacePath) } : {}
  };
}
async function resolveExistingScriptPath(root, userPath) {
  const candidates = collectScriptPathCandidates(root, userPath);
  for (const p of candidates) {
    try {
      await import_fs14.promises.access(p);
  ...
  for (const scriptsRoot of scriptSearchRoots(root)) {
    try {
      const names = await import_fs14.promises.readdir(scriptsRoot);
      const files = names.filter((n) => n && !n.startsWith(".")).slice(0, 8);
      if (files.length > 0) {
        const rel = import_path13.default.relative(root, scriptsRoot).split(import_path13.default.sep).join("/") || "scripts";
...
async function ensureDir2(dir) {
  await import_fs14.promises.mkdir(dir, { recursive: true });
}
function getShellCommand() {
  if (process.platform === "win32") {
    const shell2 = process.env["ComSpec"] || "cmd.exe";
    return { shell: shell2, shellLabel: "cmd" };
  }
  const shell = process.env["SHELL"] || "/bin/bash";
  return { shell, shellLabel: import_path13.default.basename(shell) };
}
function getScriptRuntime(kind) {
  if (kind === "bash") {
    ...
    try {
      const result = await new Promise((resolve6, reject) => {
        (0, import_child_process2.execFile)(
          ...
          (error, stdout, stderr) => {
            if (!error) {
              resolve6({ stdout, stderr, exitCode: 0, signal: null });
              return;
            }
            const execError2 = error;
            const errorCode = typeof execError2.code === "string" ? execError2.code : void 0;
            if (errorCode === "ENOENT") {
              reject(execError2);
              return;
            }
            resolve6({
              stdout,
              stderr,
              exitCode: typeof execError2.code === "number" ? execError2.code : 1,
              signal: execError2.signal ?? null
            });
          }
        );
      });
...
function formatCaptureBody(stdout, stderr) {
  const parts = [];
  if (stdout.trim()) {
    parts.push("--- stdout ---\n" + stdout.trimEnd());
  }
...
function normalizeCaptureRelativePath(userCapture) {
  const fallback = `capture-${Date.now()}-${import_crypto2.default.randomBytes(4).toString("hex")}.txt`;
  const resultsPrefix = getOutputResultsRelPrefix();
  if (!userCapture?.trim()) {
    return import_path13.default.join(resultsPrefix, fallback);
  }
  ...
  const underPrefix = seg.length >= prefixSeg.length && prefixSeg.every((part, i) => seg[i] === part);
  if (underPrefix) {
    return import_path13.default.join(...seg);
  }
  if (seg.length === 1) {
    return import_path13.default.join(resultsPrefix, seg[0]);
  }
  return import_path13.default.join(resultsPrefix, import_path13.default.basename(remapped));
}
function normalizeResultFileRelativePath(userPath) {
...
async function writeScriptToSandbox(options) {
  ...
  const scriptsDir = import_path13.default.join(root, getOutputScriptsRelPrefix());
  await ensureDir2(scriptsDir);
  const baseFromUser = `script-${Date.now()}-${import_crypto2.default.randomBytes(4).toString("hex")}`;
  const ensuredBase = baseFromUser.endsWith(runtime.extension) ? baseFromUser : `${baseFromUser}${runtime.extension}`;
  let scriptPath = import_path13.default.join(scriptsDir, ensuredBase);
  if ((0, import_fs13.existsSync)(scriptPath)) {
    let n = 1;
    const stem = ensuredBase.endsWith(runtime.extension) ? ensuredBase.slice(0, -runtime.extension.length) : ensuredBase;
    while ((0, import_fs13.existsSync)(scriptPath)) {
      scriptPath = import_path13.default.join(scriptsDir, `${stem}-${n}${runtime.extension}`);
      n += 1;
    }
  }
  try {
    await import_fs14.promises.writeFile(scriptPath, scriptContent, "utf8");
  } catch (err) {
    return {
      ok: false,
  ...
  if (scriptType === "bash" && process.platform !== "win32") {
    try {
      await import_fs14.promises.chmod(scriptPath, 448);
    } catch {
    }
  }
  let verified;
  try {
    verified = await import_fs14.promises.readFile(scriptPath, "utf8");
  } catch (err) {
    return {
      ok: false,
  ...
  return {
    ...
    relToSandbox: import_path13.default.relative(root, scriptAbsPath),
    scriptRelativePathPosix: import_path13.default.relative(root, scriptAbsPath).split(import_path13.default.sep).join("/")
  };
}
function stripLeadingShebang(content) {
  return content.replace(/^#!.*(?:\r?\n|$)/, "");
}
function getScriptCommentPrefix(scriptType) {
...
async function runScriptFileAndCapture(options) {
  ...
  const resultsAbsDir = import_path13.default.join(root, getOutputResultsRelPrefix());
  await ensureDir2(resultsAbsDir);
  ...
  const workspaceBefore = workspacePath ? await snapshotWorkspaceGuard(workspacePath) : null;
  ...
  if (resultFileRelativePath?.trim()) {
    try {
      ...
      await import_fs14.promises.access(candidate);
  ...
  if (success) {
    ...
    if (primaryEntry) {
      const abs = import_path13.default.isAbsolute(primaryEntry.path) ? primaryEntry.path : import_path13.default.join(root, primaryEntry.relPath);
      try {
        const raw = await import_fs14.promises.readFile(abs, "utf8");
        if (raw.trim()) {
          resultContent = raw;
          resultReadFrom = primaryEntry.relPath;
        }
      } catch {
        resultContent = void 0;
      }
    }
    ...
    if (!resultContent?.trim()) {
      if (consoleOutput.trim()) {
        ...
        try {
          const captureText = await import_fs14.promises.readFile(captureAbs, "utf8");
          if (captureText.trim() && captureText.trim().toLowerCase() !== "(no stdout/stderr)") {
            resultContent = captureText;
            resultReadFrom = import_path13.default.relative(root, captureAbs).split(import_path13.default.sep).join("/");
          }
        } catch {
          resultContent = void 0;
        }
  ...
  return {
    ...
    cwd,
    scriptPath,
    ...workspaceWrites?.length ? { workspaceWrites, workspaceWriteWarning } : {},
    ...result.error ? { error: result.error } : {}
  };
}
function buildRunScriptInstruction(scriptType, scriptContent) {
  const st = scriptType === "javascript" ? "javascript" : scriptType === "nodejs" ? "nodejs" : scriptType;
  return [
    ...
    `Optional: \`referencedScriptFiles\` (paths to files already in \`scripts/\`; their contents are prepended before \`scriptContent\`), \`scriptArgs\`, \`captureRelativePath\`, \`resultFileRelativePath\`. The tool creates the script file internally. Paths in args are relative to SANDBOX ROOT (\u{1F4E6}).`,
    ``,
    `\`\`\`${scriptType === "nodejs" || scriptType === "javascript" ? "javascript" : scriptType}`,
    scriptContent,
    `\`\`\``
  ].join("\n");
}
var import_child_process2, import_fs13, import_fs14, import_crypto2, import_path13, import_zod7, scriptTypeEnum, scriptTypeRequired, sharedRunScriptFieldsRest, runSandboxScriptContentInput, runSandboxScriptFileInput, runScript, runScriptFile, buildCreateScriptInstruction;
var init_shell_command = __esm({
  "toolSet/shell-command.ts"() {
    ...
    init_sandbox_paths();
    init_run_script_workspace_guard();
    init_run_script_artifacts();
    init_run_script_preflight();
    init_terminal_capture();
    scriptTypeEnum = import_zod7.z.enum(["bash", "python", "nodejs", "javascript"]);
    scriptTypeRequired = scriptTypeEnum.describe(
    ...
    sharedRunScriptFieldsRest = {
      scriptArgs: import_zod7.z.array(import_zod7.z.string()).optional().default([]).describe(
      ...
      captureRelativePath: import_zod7.z.string().min(1).optional().describe(
      ...
      resultFileRelativePath: import_zod7.z.string().min(1).optional().describe(
      ...
      timeoutMs: import_zod7.z.number().int().min(100).max(12e4 * 5).optional().default(12e4 * 5).describe(
        "Maximum time in ms to wait for the interpreter process (default 10 minutes)."
      ),
      env: import_zod7.z.record(import_zod7.z.string(), import_zod7.z.string()).optional().describe(
       ...
       */
      workingDirectory: import_zod7.z.enum(["sandbox"]).optional().default("sandbox").describe(
    ...
    runSandboxScriptContentInput = import_zod7.z.object({
       ...
       */
      referencedScriptFiles: import_zod7.z.array(import_zod7.z.string().min(1)).optional().default([])
    });
    runSandboxScriptFileInput = import_zod7.z.object({
      scriptType: scriptTypeRequired,
      ...sharedRunScriptFieldsRest,
      /**
    ...
    runScript = {
      ...
      async execute(input) {
        ...
        const resolvedReferencedScripts = [];
        for (const referencedScriptFile of referencedScriptFiles) {
          ...
          try {
            const content = await import_fs14.promises.readFile(resolvedPath.scriptPath, "utf8");
            resolvedReferencedScripts.push({
              scriptRelativePathPosix: import_path13.default.relative(sb.root, resolvedPath.scriptPath).split(import_path13.default.sep).join("/"),
              content
            });
          } catch (err) {
            return {
    ...
    runScriptFile = {
      ...
      async execute(input) {
        ...
        const scriptRelativePathPosix = import_path13.default.relative(sb.root, resolvedPath.scriptPath).split(import_path13.default.sep).join("/");
        try {
          const runOut = await runScriptFileAndCapture({
            root: sb.root,
            cwd: scriptCwd,
    ...
    buildCreateScriptInstruction = buildRunScriptInstruction;
  }
});
init_shell_command();
...
//# sourceMappingURL=e587d9ac2d1bc44e258a1e60e189cec8c45140cc.js.map
