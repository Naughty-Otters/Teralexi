...
function redirectLegacyChannelDataPath(targetPath) {
  ...
  for (const name of openfde_CHANNEL_DATA_DIRS) {
    ...
    if (resolved === legacyRoot || resolved.startsWith(`${legacyRoot}${import_path.sep}`)) {
      const relative4 = resolved.slice(legacyRoot.length).replace(/^[/\\]+/, "");
      return relative4 ? (0, import_path.join)(channelsRoot, name, relative4) : (0, import_path.join)(channelsRoot, name);
    }
  }
  return targetPath;
}
...
// src/main/agent/lsp/json-rpc.ts
...
var HEADER_SEPARATOR, MessageBuffer;
var init_json_rpc = __esm({
  "src/main/agent/lsp/json-rpc.ts"() {
    HEADER_SEPARATOR = "\r\n\r\n";
    MessageBuffer = class {
      constructor() {
...
// src/main/agent/lsp/language-servers.ts
function bundledBinDir() {
  return process.env[OTTER_LSP_BUNDLED_BIN_ENV]?.trim() || null;
}
function binFileName(command) {
  return process.platform === "win32" ? `${command}.cmd` : command;
}
function resolveServerCommand(def, workspaceRoot) {
  const binName = binFileName(def.command);
  const local = (0, import_node_path3.join)(workspaceRoot, "node_modules", ".bin", binName);
  if ((0, import_node_fs6.existsSync)(local)) return local;
  const bundledDir = bundledBinDir();
  if (bundledDir) {
    const bundled = (0, import_node_path3.join)(bundledDir, binName);
    if ((0, import_node_fs6.existsSync)(bundled)) return bundled;
  }
  return def.command;
}
function buildServerPath(workspaceRoot) {
  const bundledDir = bundledBinDir();
  const extra = [
    (0, import_node_path3.join)(workspaceRoot, "node_modules", ".bin"),
    ...bundledDir ? [bundledDir] : [],
    "/usr/local/bin",
    "/opt/homebrew/bin",
    "/usr/bin",
    (0, import_node_path3.dirname)(process.execPath)
  ];
  const existing = process.env.PATH ?? "";
  return [...extra, existing].filter(Boolean).join(import_node_path3.delimiter);
}
function detectWorkspaceServers(workspaceRoot) {
  if (!workspaceRoot?.trim()) return [];
  return LANGUAGE_SERVERS.filter(
    (server) => (server.markers ?? []).some(
      (marker) => (0, import_node_fs6.existsSync)((0, import_node_path3.join)(workspaceRoot, marker))
    )
  );
}
function matchLanguageServer(filePath) {
...
function isLspSupportedFile(filePath) {
  return matchLanguageServer(filePath) !== null;
}
var import_node_fs6, import_node_path3, LANGUAGE_SERVERS, OTTER_LSP_BUNDLED_BIN_ENV;
var init_language_servers = __esm({
  "src/main/agent/lsp/language-servers.ts"() {
    ...
    OTTER_LSP_BUNDLED_BIN_ENV = "OTTER_LSP_BUNDLED_BIN";
  }
});

// src/main/agent/lsp/lsp-client.ts
var import_node_child_process, import_node_url, log18, INITIALIZE_TIMEOUT_MS, DIAGNOSTICS_TIMEOUT_MS, SYMBOL_TIMEOUT_MS, NULL_REPLY_METHODS, LspClient;
var init_lsp_client = __esm({
  "src/main/agent/lsp/lsp-client.ts"() {
    import_node_child_process = require("node:child_process");
    import_node_url = require("node:url");
    init_logger();
    init_json_rpc();
    init_language_servers();
    log18 = createLogger("agent.lsp.client");
    INITIALIZE_TIMEOUT_MS = 15e3;
    DIAGNOSTICS_TIMEOUT_MS = 6e3;
    SYMBOL_TIMEOUT_MS = 8e3;
    NULL_REPLY_METHODS = /* @__PURE__ */ new Set([
    ...
    LspClient = class {
      constructor(def, workspaceRoot) {
        this.def = def;
        this.workspaceRoot = workspaceRoot;
        this.proc = null;
        this.reader = new MessageBuffer();
        this.nextId = 1;
        this.pending = /* @__PURE__ */ new Map();
        this.diagnosticsByUri = /* @__PURE__ */ new Map();
        this.publishWaiters = /* @__PURE__ */ new Map();
        this.openVersions = /* @__PURE__ */ new Map();
      ...
      async start() {
        ...
        const proc = (0, import_node_child_process.spawn)(command, this.def.args, {
        ...
        await new Promise((resolve6, reject) => {
          const onSpawnError = (err) => reject(err);
        ...
        proc.on("exit", (code) => {
          if (!this.disposed) {
            log18.warn("Language server exited unexpectedly", {
              server: this.def.id,
              code
            });
          }
      ...
      async initialize() {
        const result = this.request(
          "initialize",
          {
            ...
            rootUri: (0, import_node_url.pathToFileURL)(this.workspaceRoot).toString(),
            workspaceFolders: [
              {
                uri: (0, import_node_url.pathToFileURL)(this.workspaceRoot).toString(),
                name: "workspace"
              }
            ],
            capabilities: {
       ...
       */
      async getDiagnostics(absPath, text, languageId) {
        ...
        try {
          ...
          log18.debug("getDiagnostics failed", { server: this.def.id, err });
          return [];
        }
      }
      // ── Public: symbol intelligence ────────────────────────────────────────────
      /** Open (or update) a document so the server can answer queries about it. */
      async openDocument(absPath, text, languageId) {
      ...
      syncDocument(absPath, text, languageId) {
      ...
      closeDocument(absPath) {
        if (this.disposed || !this.proc) return;
        const uri = (0, import_node_url.pathToFileURL)(absPath).toString();
      ...
      async lspRequest(method, params, timeoutMs = SYMBOL_TIMEOUT_MS) {
        if (this.disposed || !this.proc) return null;
        return this.request(method, params, timeoutMs);
      }
      ensureDocumentOpen(absPath, text, languageId) {
        const uri = (0, import_node_url.pathToFileURL)(absPath).toString();
      ...
      handleMessage(message) {
        if (message.id != null && (message.result !== void 0 || message.error)) {
      ...
      send(message) {
        ...
        try {
          ...
          log18.debug("LSP write failed", { server: this.def.id, err });
        }
      }
...
// src/main/agent/lsp/workspace-seed.ts
function findWorkspaceSeedFile(workspaceRoot, server) {
  const root = workspaceRoot.trim();
  if (!root) return null;
  const exts = new Set(Object.keys(server.extensions));
  for (const rel of PREFERRED_RELATIVE_PATHS[server.id] ?? []) {
    const candidate = (0, import_node_path4.join)(root, rel);
    if ((0, import_node_fs7.existsSync)(candidate) && exts.has((0, import_node_path4.extname)(candidate).toLowerCase())) {
      return candidate;
    }
  }
  let scanned = 0;
  const walk = (dir, depth) => {
    ...
    let entries;
    try {
      entries = (0, import_node_fs7.readdirSync)(dir, { withFileTypes: true });
    } catch {
      return null;
    }
    for (const ent of entries) {
      ...
      scanned += 1;
      const ext = (0, import_node_path4.extname)(name).toLowerCase();
...
var import_node_fs7, import_node_path4, MAX_FILES_SCANNED, MAX_DEPTH, SKIP_DIRS, PREFERRED_RELATIVE_PATHS;
var init_workspace_seed = __esm({
  "src/main/agent/lsp/workspace-seed.ts"() {
...
// src/main/agent/lsp/diagnostic-format.ts
...
function buildDiagnosticReport(absFilePath, workspaceRoot, diagnostics, options = {}) {
  ...
  const displayPath = workspaceRoot ? (0, import_node_path5.relative)(workspaceRoot, absFilePath) || absFilePath : absFilePath;
...
var import_node_path5, SEVERITY_LABEL, DEFAULT_MAX_DIAGNOSTICS;
var init_diagnostic_format = __esm({
  "src/main/agent/lsp/diagnostic-format.ts"() {
...
// src/main/agent/lsp/symbol-format.ts
function symbolKindName(kind) {
  return SYMBOL_KIND_NAMES[kind] ?? `kind-${kind}`;
}
function uriToDisplayPath(uri, workspaceRoot) {
  let abs;
  try {
    abs = (0, import_node_url2.fileURLToPath)(uri);
  } catch {
    return uri;
  }
  if (workspaceRoot) {
    const rel = (0, import_node_path6.relative)(workspaceRoot, abs);
    if (rel && !rel.startsWith("..")) return rel;
  }
  return abs;
}
function rangeToDisplay(uri, range, workspaceRoot) {
  return {
...
function isLocationLink(value) {
  return Boolean(value) && typeof value.targetUri === "string";
}
...
function hoverToText(result) {
  if (!result || typeof result !== "object") return "";
  const contents = result.contents;
  return markedContentToText(contents).trim();
}
function markedContentToText(contents) {
  ...
  if (isHierarchical) {
    const walk = (nodes, depth) => {
      for (const node of nodes) {
        ...
        out.push({
          ...
          detail: node.detail?.trim() || void 0,
          depth
        });
    ...
    for (const sym of result) {
      ...
      out.push({
        ...
        line: pos.line + 1,
        character: pos.character + 1,
        detail: sym.containerName?.trim() || void 0,
        depth: 0
      });
    }
  }
...
function normalizeWorkspaceSymbols(result, workspaceRoot) {
  if (!Array.isArray(result)) return [];
  const out = [];
  for (const sym of result) {
    ...
    out.push({
      ...
      container: sym.containerName?.trim() || void 0
    });
  }
  return out;
}
var import_node_url2, import_node_path6, SYMBOL_KIND_NAMES;
var init_symbol_format = __esm({
  "src/main/agent/lsp/symbol-format.ts"() {
...
// src/main/agent/lsp/editor-lsp-bridge.ts
var editor_lsp_bridge_exports = {};
__export(editor_lsp_bridge_exports, {
  SYNC_DEBOUNCE_MS: () => SYNC_DEBOUNCE_MS,
  absPathFromDiagnosticUri: () => absPathFromDiagnosticUri,
  getEditorLspBridge: () => getEditorLspBridge,
  relativePathFromAbs: () => relativePathFromAbs,
  resolveEditorRelativePath: () => resolveEditorRelativePath
});
function getEditorLspBridge() {
  const g = globalThis;
  let bridge = g[EDITOR_LSP_BRIDGE_GLOBAL_KEY];
  if (!bridge) {
    bridge = new EditorLspBridge();
    g[EDITOR_LSP_BRIDGE_GLOBAL_KEY] = bridge;
    const manager = getLspManager();
    manager.setEditorContentResolver(
      (workspaceRoot, absPath) => bridge.getOwnedContent(workspaceRoot, absPath)
    );
    manager.setDiagnosticsHandler(
      (workspaceRoot, absPath, diagnostics) => bridge.publishDiagnostics(workspaceRoot, absPath, diagnostics)
    );
  }
  return bridge;
}
function resolveEditorRelativePath(workspaceRoot, relativePath) {
  return resolveUserProjectPath(workspaceRoot, relativePath);
}
function relativePathFromAbs(workspaceRoot, absPath) {
  const root = (0, import_node_path7.normalize)(workspaceRoot);
  const abs = (0, import_node_path7.normalize)(absPath);
  if (!abs.startsWith(root)) return null;
  const rel = abs.slice(root.length).replace(/^[/\\]/, "");
  return rel || null;
}
function absPathFromDiagnosticUri(uri) {
  try {
    if (uri.startsWith("file://")) return (0, import_node_path7.normalize)((0, import_node_url3.fileURLToPath)(uri));
    return (0, import_node_path7.normalize)(uri);
  } catch {
    return null;
  }
}
var import_node_path7, import_node_url3, log19, SYNC_DEBOUNCE_MS, EditorLspBridge, EDITOR_LSP_BRIDGE_GLOBAL_KEY;
var init_editor_lsp_bridge = __esm({
  "src/main/agent/lsp/editor-lsp-bridge.ts"() {
    ...
    init_lsp_manager();
    init_language_servers();
    log19 = createLogger("agent.lsp.editor-bridge");
    SYNC_DEBOUNCE_MS = 200;
    EditorLspBridge = class {
      constructor() {
        this.sessions = /* @__PURE__ */ new Map();
        /** workspaceRoot::relativePath → latest editor buffer (any session). */
        this.ownedContent = /* @__PURE__ */ new Map();
      }
      startSession(conversationId, workspaceRoot, webContents) {
        const id = conversationId.trim();
        const root = workspaceRoot.trim();
        ...
        this.sessions.set(id, {
          conversationId: id,
          workspaceRoot: (0, import_node_path7.normalize)(root),
          webContents,
          documents: /* @__PURE__ */ new Map()
        });
        getLspManager().prewarm((0, import_node_path7.normalize)(root));
        return { ok: true };
      }
      ...
      queueSyncDocument(conversationId, relativePath, content, languageId) {
        const session = this.sessions.get(conversationId.trim());
      ...
      closeDocument(conversationId, relativePath) {
        const session = this.sessions.get(conversationId.trim());
        if (!session) return { ok: false, error: "Editor LSP session not started." };
        const rel = relativePath.replace(/\\/g, "/").trim();
      ...
      async request(conversationId, relativePath, method, params) {
        const session = this.sessions.get(conversationId.trim());
        if (!session) return { ok: false, error: "Editor LSP session not started." };
        const rel = relativePath.replace(/\\/g, "/").trim();
        const doc = session.documents.get(rel);
      ...
      listOpenDocumentsForWorkspace(workspaceRoot) {
        const root = (0, import_node_path7.normalize)(workspaceRoot);
        const out = [];
      ...
      getOwnedContent(workspaceRoot, absPath) {
        const root = (0, import_node_path7.normalize)(workspaceRoot);
        const normalizedAbs = (0, import_node_path7.normalize)(absPath);
        for (const session of this.sessions.values()) {
          if (session.workspaceRoot !== root) continue;
          for (const doc of session.documents.values()) {
            if ((0, import_node_path7.normalize)(doc.absPath) === normalizedAbs) {
              return doc.content;
            }
          }
        }
      ...
      publishDiagnostics(workspaceRoot, absPath, diagnostics) {
        const root = (0, import_node_path7.normalize)(workspaceRoot);
        const normalizedAbs = (0, import_node_path7.normalize)(absPath);
        for (const session of this.sessions.values()) {
          if (session.workspaceRoot !== root) continue;
          for (const doc of session.documents.values()) {
            if ((0, import_node_path7.normalize)(doc.absPath) !== normalizedAbs) continue;
      ...
      trackOwnedContent(workspaceRoot, relativePath, content) {
        this.ownedContent.set(`${(0, import_node_path7.normalize)(workspaceRoot)}::${relativePath}`, content);
      }
      untrackOwnedContent(workspaceRoot, relativePath) {
        this.ownedContent.delete(`${(0, import_node_path7.normalize)(workspaceRoot)}::${relativePath}`);
      }
    };
    EDITOR_LSP_BRIDGE_GLOBAL_KEY = "__OTTER_EDITOR_LSP_BRIDGE__";
  }
});

// src/main/agent/lsp/lsp-manager.ts
function withTimeout(promise, ms, fallback) {
  return new Promise((resolve6) => {
    const timer = setTimeout(() => resolve6(fallback), ms);
    promise.then(
      (v) => {
        clearTimeout(timer);
        resolve6(v);
...
function getLspManager() {
  const g = globalThis;
  let manager = g[LSP_MANAGER_GLOBAL_KEY];
  if (!manager) {
...
var import_promises2, import_node_url4, import_node_path8, POSITION_OPS, LSP_METHOD, log20, OVERALL_TIMEOUT_MS, LspManager, LSP_MANAGER_GLOBAL_KEY;
var init_lsp_manager = __esm({
  "src/main/agent/lsp/lsp-manager.ts"() {
    ...
    init_language_servers();
    init_workspace_seed();
    init_diagnostic_format();
    init_symbol_format();
    POSITION_OPS = /* @__PURE__ */ new Set([
      "definition",
      "references",
      "hover",
      "implementation"
    ...
    log20 = createLogger("agent.lsp.manager");
    OVERALL_TIMEOUT_MS = 15e3;
    LspManager = class {
      constructor() {
        this.clients = /* @__PURE__ */ new Map();
        this.starting = /* @__PURE__ */ new Map();
        /** Commands known to be missing on PATH — don't retry spawning them. */
        this.unavailableCommands = /* @__PURE__ */ new Set();
        this.editorContentResolver = null;
        this.diagnosticsHandler = null;
        this.wiredClients = /* @__PURE__ */ new WeakSet();
        this.clientWorkspaceRoots = /* @__PURE__ */ new WeakMap();
        /** Servers warmed with at least one open document for workspace/symbol. */
        this.symbolSearchWarmed = /* @__PURE__ */ new Set();
      }
      key(workspaceRoot, serverId) {
        return `${workspaceRoot}::${serverId}`;
      }
      setEditorContentResolver(resolver) {
      ...
      wireClientNotifications(client, workspaceRoot) {
        ...
        client.onNotification((method, params) => {
          ...
          try {
            const absPath = (0, import_node_path8.normalize)((0, import_node_url4.fileURLToPath)(payload.uri));
            this.diagnosticsHandler(workspaceRoot, absPath, payload.diagnostics ?? []);
          } catch {
          }
        });
      }
      async readDocumentText(absFilePath, workspaceRoot) {
        const owned = this.editorContentResolver?.(workspaceRoot, absFilePath);
        if (owned != null) return owned;
        try {
          return await (0, import_promises2.readFile)(absFilePath, "utf8");
        } catch {
          return null;
        }
      }
      async syncEditorDocument(workspaceRoot, absFilePath, text, languageId) {
      ...
      async closeEditorDocument(workspaceRoot, absFilePath) {
        const match = matchLanguageServer(absFilePath);
        if (!match) return;
        const client = this.clients.get(this.key(workspaceRoot, match.server.id));
        client?.closeDocument(absFilePath);
      }
      async editorLspRequest(workspaceRoot, absFilePath, text, languageId, method, params) {
        const match = matchLanguageServer(absFilePath);
        if (!match) {
      ...
      async getClient(workspaceRoot, serverDef, create) {
        ...
        const startPromise = (async () => {
          ...
          try {
            ...
            this.wireClientNotifications(client, workspaceRoot);
            log20.info("Language server started", { server: serverDef.id, workspaceRoot });
            return client;
          } catch (err) {
            const code = err?.code;
            if (code === "ENOENT") {
              this.unavailableCommands.add(serverDef.command);
              log20.info("Language server not installed; skipping", {
                server: serverDef.id,
                command: serverDef.command
              });
            } else {
              log20.warn("Language server failed to start", { server: serverDef.id, err });
            }
            client.dispose();
            return null;
          } finally {
       ...
       */
      async getDiagnosticReport(absFilePath, workspaceRoot) {
        const empty = { errorCount: 0, warningCount: 0, block: "" };
        const match = matchLanguageServer(absFilePath);
      ...
      async querySymbols(params) {
        ...
        return withTimeout(
          (async () => {
            ...
            const requestParams = operation === "workspace_symbols" ? { query: params.query ?? "" } : operation === "references" ? {
              textDocument: { uri },
              position: { line: params.line - 1, character: params.character - 1 },
              context: { includeDeclaration: true }
            } : operation === "document_symbols" ? { textDocument: { uri } } : {
              textDocument: { uri },
              position: { line: params.line - 1, character: params.character - 1 }
            };
       ...
       */
      async queryWorkspaceSymbols(workspaceRoot, query, options) {
        const root = workspaceRoot.trim();
        if (!root) return { ok: false, error: "workspaceRoot is required." };
        const detected = detectWorkspaceServers(root);
        const servers = detected.length > 0 ? detected : LANGUAGE_SERVERS.filter((s) => s.id === "typescript" || s.id === "pyright");
        const merged = [];
        const seen = /* @__PURE__ */ new Set();
        ...
        for (const server of servers) {
          ...
          try {
            ...
            log20.debug("workspace/symbol failed", { server: server.id, err });
          }
        }
        if (!startedAny) {
          return {
       ...
       */
      async warmForWorkspaceSymbolSearch(client, server, workspaceRoot, conversationId) {
      ...
      async collectSymbolWarmCandidates(workspaceRoot, server, conversationId) {
        const root = (0, import_node_path8.normalize)(workspaceRoot);
        const exts = new Set(Object.keys(server.extensions));
        const out = [];
        const seen = /* @__PURE__ */ new Set();
        const add = (absPath) => {
          const normalized = (0, import_node_path8.normalize)(absPath);
          if (seen.has(normalized)) return;
          if (!exts.has((0, import_node_path8.extname)(normalized).toLowerCase())) return;
          seen.add(normalized);
          out.push(normalized);
        };
        if (conversationId?.trim()) {
          try {
            const { getEditorLspBridge: getEditorLspBridge2 } = await Promise.resolve().then(() => (init_editor_lsp_bridge(), editor_lsp_bridge_exports));
            for (const doc of getEditorLspBridge2().listOpenDocumentsForWorkspace(root)) {
              add(doc.absPath);
            }
          } catch {
          }
      ...
      async waitForWorkspaceSymbolReady(client, timeoutMs = 6e3) {
      ...
      prewarm(workspaceRoot) {
        ...
        for (const server of detectWorkspaceServers(workspaceRoot)) {
          void this.getClient(
            ...
            (client) => {
              if (client) {
                log20.info("Pre-warmed language server", { server: server.id, workspaceRoot });
              }
            },
            () => {
            }
    ...
    LSP_MANAGER_GLOBAL_KEY = "__OTTER_LSP_MANAGER__";
  }
});

// src/main/agent/lsp/apply-lsp-diagnostics.ts
var log21;
var init_apply_lsp_diagnostics = __esm({
  "src/main/agent/lsp/apply-lsp-diagnostics.ts"() {
    init_logger();
    init_sandbox();
    init_lsp_manager();
    init_language_servers();
    log21 = createLogger("agent.lsp.wrapper");
  }
});

// src/main/agent/lsp/index.ts
var init_lsp = __esm({
  "src/main/agent/lsp/index.ts"() {
    init_lsp_manager();
    init_editor_lsp_bridge();
    init_apply_lsp_diagnostics();
    init_symbol_format();
    init_language_servers();
    init_diagnostic_format();
    init_json_rpc();
...
var lspTool = {
  name: "lsp",
  tags: [...LSP_TAG],
  description: "Code intelligence via the language server. Operations: definition (where a symbol is defined), references (all uses), implementation (impls of an interface/abstract), hover (type/doc at a position), document_symbols (outline of one file), workspace_symbols (project-wide symbol search by name). Position ops (definition/references/hover/implementation) need 1-based line & character. workspace_symbols needs query. Requires a workspace folder. Prefer this over grep for navigating code by symbol.",
  inputSchema: import_zod7.z.object({
    operation: import_zod7.z.enum(OPERATIONS).describe("Which code-intelligence query to run."),
    path: import_zod7.z.string().min(1).describe("File path (workspace-relative or absolute). For workspace_symbols, used to pick the language server."),
    line: import_zod7.z.number().int().min(1).optional().describe("1-based line (required for definition/references/hover/implementation)."),
    character: import_zod7.z.number().int().min(1).optional().describe("1-based column (required for definition/references/hover/implementation)."),
    query: import_zod7.z.string().optional().describe("Symbol name to search for (required for workspace_symbols).")
  }),
  needsApproval: false,
  async execute(input) {
    const parsed = import_zod7.z.object({
      operation: import_zod7.z.enum(OPERATIONS),
      path: import_zod7.z.string().min(1),
      line: import_zod7.z.number().optional(),
      character: import_zod7.z.number().optional(),
    ...
    if (!workspaceRoot) {
      return {
        error: "Code intelligence requires a workspace folder. Ask the user to select one for this conversation."
      };
    }
    const sandbox = requireActiveSandbox();
    if (!sandbox.ok) return { error: sandbox.message };
    const { operation, path: path2, line, character, query } = parsed.data;
    if (operation === "workspace_symbols" && !query?.trim()) {
      return { error: "workspace_symbols requires a non-empty query." };
    }
    let absFilePath;
    try {
      absFilePath = resolvePathInContext(sandbox.root, workspaceRoot, path2);
    } catch (err) {
...
var lspTools = [lspTool];
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  lspTool,
  lspTools
});
//# sourceMappingURL=d3a8db59cb3c761e0658ae4aa086ab1938425aac.js.map
