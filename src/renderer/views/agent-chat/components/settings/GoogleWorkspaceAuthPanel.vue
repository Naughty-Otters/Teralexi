<template>
  <section class="gw-auth">
    <div class="gw-auth__divider" aria-hidden="true" />

    <h3 class="gw-auth__title">{{ p.googleWorkspace.signInSectionTitle }}</h3>
    <p class="gw-auth__intro">{{ p.googleWorkspace.signInHint }}</p>
    ...
    <template v-if="account">
      ...
      <button
        ...
        :disabled="signInLoading || disabled"
        @click="signOut"
      >
        {{ p.actions.signOut }}
      </button>
    ...
    <template v-else>
      ...
      <button
        ...
        :disabled="signInLoading || disabled || !oauthConfigured"
        @click="signIn"
      >
...
import { googleWorkspaceOAuthConfiguredFromSpecs } from '@shared/skills/skill-system-property-io'
...
function onGoogleWorkspaceAccountChanged(payload: {
  account: GoogleWorkspaceAccountInfo | null
}) {
...
watch(
  () => props.propertyValues,
  () => {
    if (oauthConfigured.value) error.value = null
  },
  { deep: true },
)
