<template>
  <div
    class="chat-pane"
    :class="{ 'chat-pane--focused': isFocused }"
    @click.capture="onPaneActivate($event)"
  >
    <ChatPanelHeader
      v-if="isFocused"
    ...
    <section class="chat-main">
      ...
      <ChatComposer
        ...
        :conversation-id="paneConversationId"
...
const emit = defineEmits<{ activate: []; 'open-preview': [url: string] }>()

const isFocused = computed(() => props.isFocused !== false)
const paneConversationId = computed(
  () => props.conversationId?.trim() || null,
)
...
provide('chatConversationId', paneConversationId)
...
const showWorkspaceSplitPanel = computed({
  get: () =>
    workspaceNavStore.isWorkspacePanelOpen(paneConversationId.value),
  set: (open: boolean) => {
    const conversationId = paneConversationId.value?.trim()
...
const currentPreviewLinkTabs = computed(() => {
  const cid = paneConversationId.value
  if (!cid) return []
...
function onUpdateActivePreviewLinkTabId(tabId: string | null) {
  const cid = paneConversationId.value?.trim()
  if (!cid) return
  activePreviewLinkTabIdByConversation.value = {
    ...activePreviewLinkTabIdByConversation.value,
    [cid]: tabId,
  ...
  if (useFull) {
    ...
    if (isChatUiWorkerAvailable()) {
      ...
      void workerNormalizeChatMessages(raw).then((next) => {
        ...
        if (paneConversationId.value == null) return
...
function scheduleScrollToBottom(behavior: ScrollBehavior = 'auto'): void {
  scheduleUiFlush(
    ...
    {
      conversationId: paneConversationId.value ?? undefined,
      priority: 'normal',
    },
  )
}
    ...
    if (phase === 'end') {
      ...
      scheduleUiFlush(
        ...
        () => {
          ...
          if (!chat) return
          if (paneConversationId.value !== conversationId) return
      ...
      evictIdleConversationChats({
        ...
        keepIds: useConversationLayoutStore().visibleConversationIdList,
      })
    }
  },
  onStreamUiChunk(conversationId, meta) {
    const isVisible = paneConversationId.value === conversationId
...
const contextWindowUsage = computed(() => {
  ...
  if (!paneConversationId.value) return null
...
async function loadOlderMessagesForScroll(): Promise<boolean> {
  const conversationId = paneConversationId.value
...
watch(
  ...
  (v) => {
    ...
    const conversationId = paneConversationId.value ?? undefined
    scheduleUiFlush(
      'messages-sync',
...
function resolveActiveConversationId(): string | undefined {
  return paneConversationId.value ?? undefined
}
...
function conversationHasPendingHitl(
  ...
  if (
    paneConversationId.value === conversationId &&
...
watch(isBusy, (busy, wasBusy) => {
  ...
  if (wasBusy && !busy) {
    ...
    if (!hasPendingHitl.value) {
      void loadFollowUpSuggestions(paneConversationId.value ?? undefined)
    } else {
...
function onConversationFollowUpsChanged(
  ...
  const conversationId = payload?.conversationId?.trim()
  if (!conversationId || conversationId !== paneConversationId.value) {
    return
  }
...
watch(
  ...
  (focused) => {
    if (!focused) return
    registerStressChatDriver({
      isReady: () => Boolean(chatInst.value && paneConversationId.value),
      sendAndWait: stressSendAndWait,
      stopCurrentTurn: () => {
        const conversationId =
          paneConversationId.value?.trim() ||
          (typeof chatInst.value?.id === 'string'
            ? chatInst.value.id.trim()
            : '')
        if (!conversationId) return
        agentStore.markUiChatInFlight(conversationId, false)
        const chat = getConversationChat(conversationId) ?? chatInst.value
...
onUnmounted(() => {
  ...
  if (isFocused.value) registerStressChatDriver(null)
})
...
async function loadFollowUpSuggestions(conversationId: string | undefined) {
  ...
  try {
    ...
    if (paneConversationId.value !== id) return
    if (result.ok) {
...
async function ensureConversationForModeCommands(): Promise<string | null> {
  const existing = paneConversationId.value
...
async function runWorkspaceCommand(action: WorkspaceSlashAction) {
  const hasConversation = Boolean(paneConversationId.value?.trim())
...
async function onSubmit() {
  ...
  await clearFollowUpSuggestions(paneConversationId.value ?? undefined)
  ...
  const agentId = agentStore.selectedAgentId
  let conversationId = paneConversationId.value ?? undefined
...
function onPaneActivate(event?: MouseEvent) {
  if (!isFocused.value) emit('activate')
  if (event) handleChatPanelLinkClick(event, openSandboxPreview)
}
</script>
...
.chat-pane--focused {
  /* focus ring owned by ConversationSplitLayout leaf chrome */
}
.chat-main {
  flex: 1;
  min-width: 0;
