...
/** True when an approved plan is executing and todos remain in `plans/todos.json`. */
...
export async function runApprovedPlanTodoForeach(
  ...
  const [{ runForEachItemBatch }, { createPlannedTodoStrategy }] =
    await Promise.all([
      import('../steps/foreach-item/batch-runner'),
